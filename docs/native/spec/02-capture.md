...
As built in WP-B2: `CaptureEngineTests` is one partial class in five files (`.Sessions`, `.Grabs`, `.Persistence`, `.Screenshot`, `.Events`) over `EngineHarness`, which has a fake for every seam, a manual clock, and the real `ProjectStore` in a temp folder. `OwnWindowClickDoesNotQueryElement` is `PillClicksCreateNoSteps`, which asserts that no element query ran. Beyond the list: `AStaleClickLandsInNoLaterSession` (EDGE-CAP-28 for a replaced session), `CaptureBailsWhenSessionClearedDuringTheElementQuery`, `AFailedStartReleasesTheReservation`, `ShotsSwappedMidSessionRefusesWrite`, `PauseAndResumeReportTheState`, `IdleStopAndDiscardOnlyReportIdle`, `DiscardWithNothingCapturedDeletesNothing`, `ADiscardCleanupFailureIsSwallowed`, `NoSessionAfterTeardown`, `ListTargetsFiltersAndNames`, `InsertAtIsClamped`, `HostileOrphansDoNotBreakTheCounter`, `RapidClicksLandInOrder`, `ASuppressedCaptureBurnsNoNumber`, `CaptureSettingsIsNeverWritten` (Q-CAP-12), `ScreenshotOfAnAreaOverlappingAMonitor`, `ScreenshotIndexIsClampedAndSeeded`, `ASlowCaptureLogsItsTiming`, `DisposeAloneTearsDown`, `EverySeamIsRequired` and `TheMessagesAreElectrons`, plus new `TimeProviderCaptureClockTests`. The menu cases of the list (`DoubleClickCollapsesChained` to `ScreenModeMenuUsesChosenMonitor`, `MenuSelectionTakesPolledFrameOwnership`) and `MenuPollTests` are WP-B3's.
