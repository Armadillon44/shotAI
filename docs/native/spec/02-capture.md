...
- `round(x)` everywhere below means **JavaScript `Math.round`**: the nearest integer, ties toward positive infinity (informally `floor(x + 0.5)`). It is NOT C# `Math.Round` (banker's rounding) and NOT `MidpointRounding.AwayFromZero` (which differs for negative halves). The literal `Math.Floor(x + 0.5)` is also wrong at `0.49999999999999994` (it returns 1, JavaScript returns 0); INV-CAP-18 gives the exact port.
...
- `Date.now()` is read separately at each check (`:348`, `:369`, `:397`, `:426`, `:454`), not once per event. The difference is sub-millisecond; the native engine may read the clock once per event.
...
Both use the scale factor of the monitor containing the NEW click point (`Monitor.fromPoint(point)`), `1` when none or on error.
...
**INV-CAP-18. Every `Math.round` in this spec is ported as `JsMath.Round`, never `Math.Round`.** Exact port of ECMAScript `Math.round` for finite doubles: `double f = Math.Floor(x); return (x - f >= 0.5) ? f + 1 : f;` (the naive `Math.Floor(x + 0.5)` returns 1 for `0.49999999999999994` because the addition rounds up; JavaScript returns 0). Non-finite input is returned unchanged. Why: .NET's default is banker's rounding, which moves crops and click markers by one pixel at halves. Citation: formulas in 2.8.2, 2.11, 2.6. Test: `JsMathTests.RoundMatchesJavaScript` (cases 0.5, 1.5, 2.5, -0.5, -1.5, -2.5, 0.49999999999999994, 4503599627370497).
...
**INV-CAP-24. ShotAI.Core capture code references no Windows API.** Every OS dependency is behind an interface implemented in ShotAI.Platform. Why: the engine tests run on Linux CI (feasibility doc, "What gets harder" 8). Test: the Core project builds as `net10.0` without Windows targeting; `ArchitectureTests.CoreHasNoWindowsReferences` (reflection over referenced assemblies).
...
**INV-CAP-29 [SECURITY]. No UI-thread code path waits on the capture shield's lock, and every own window is excluded from capture before the shield learns of it.** Why: a cross-thread `SetWindowDisplayAffinity` serviced by the UI thread would otherwise deadlock (Q-CAP-22), and a window must never be capturable for a frame (fail-closed, `src/main/main.ts:496-502`). Citation: 7.8 deadlock rule. Test: `ShieldDeadlockTests.RegisterAndToggleWhileShieldHeldCompletes`, `AllWindowsRegisteredTests`.
...
|---|---|---|
| ShotAI.Core | `ShotAI.Core.Capture` | `CaptureConstants`, `JsMath`, `CaptureGeometry`, `AutoClassifier`, `ClickCaptions`, `DownscalePolicy`, `ShotNaming`, `UiaControlTypes`, `CaptureShield`, `ShieldedScreenCapture` (implements `IScreenCapture` over an `IMonitorCapture` and the `CaptureShield`; in Core so `ShieldedCaptureTests` run on Linux), `CaptureEngine`, `MenuArm` (internal), `CaptureSession` (internal), event arg records, and the seam interfaces `ITriggerSource`, `IMonitorCapture` (raw), `IScreenCapture` (shielded), `IWindowInfoProvider`, `IElementLocator`, `IOwnWindows`, `IWindowProtection`, `IImageCodec`, `ICaptureClock`, `ICaptureSettings` |
| ShotAI.Platform | `ShotAI.Platform.Capture` | `Win32TriggerSource` (hook thread, hotkey, watchdog), `GdiMonitorCapture` (implements `IMonitorCapture`, `internal sealed`), `Win32WindowInfoProvider`, `UiaElementLocator`, `WicImageCodec`, `DisplayAffinityProtection` (uses the existing `CaptureExclusion.Apply`), `OwnWindowRegistry` (implements `IOwnWindows`), `PlatformCaptureRegistration` (DI extension) |
| ShotAI.App | `ShotAI.App.Capture` | `CaptureViewModel` (subscribes, marshals to the Dispatcher), the `RecordingChanged` handler (hide main, show pill; spec 03), registration of every window with `OwnWindowRegistry` on `SourceInitialized` |

Model types (`CaptureTarget`, `Rect`, `Point`, `ProjectStep`, `StepClick`, `StepElement`, `CapturedWindow`, `CapturedMonitor`, `WindowInfo`, `MonitorInfo`, `ProjectManifest`) come from spec 01. Core may reference `Microsoft.Extensions.Logging.Abstractions` (platform neutral). It is NOT yet in `dotnet/Directory.Packages.props` (which today pins only `Microsoft.Windows.CsWin32` and `xunit.v3`); the first capture PR adds its `PackageVersion` there (spec 10 owns logging).
...
// Events (raised on engine threads; the App marshals to the UI thread, spec 11).
...
Job failure: any exception except `OperationCanceledException` during teardown is logged at error (`capture failed:`) and raised as `CaptureFailed(ex.Message)` with the queue continuing (INV-CAP-21). An empty message is passed through; the pill substitutes its own text (spec 03).
...
2. The Settings toggle calls `ApplyRemoteVisibility` through `Task.Run`, never inline on the UI thread.
3. No UI-thread code path takes `CaptureShield._lock`. `AllWindowsRegisteredTests` asserts the order; a `ShieldDeadlockTests` case (Windows) holds a shield on a worker while the UI thread registers a new window and toggles the setting, and must finish within 2 s.
...
| Class | Cases |
|---|---|
| `JsMathTests` | `Round(0.5)=1`, `(1.5)=2`, `(2.5)=3`, `(-0.5)=0`, `(-1.5)=-1`, `(-2.5)=-2`, `(1.4999)=1`, `(0.49999999999999994)=0`, `(4503599627370497)=4503599627370497`; `NoMathRoundInCapture` (source scan, AC-CAP-4) |
...
| `ArchitectureTests` | `CoreHasNoWindowsReferences` |
...
**ShotAI.App.Tests (Windows only):** `AllWindowsRegisteredTests` (every `Window` created by the app is in `OwnWindowRegistry` before `Show`, including the Discard dialog); `RecordingChangedHidesAndShows` (spec 03 owns the behavior; this test pins the event contract).

**Diagnostics:** `dotnet/tools/ShotAI.ProtectionProbe` (console, Windows): a C# port of `scripts/protection-probe.cjs` with the same delays, reps, tolerance and verdict text, run for a normal WPF window AND an `AllowsTransparency` window. Not in CI (it needs an interactive desktop). The capture self-test ports to a `--capture-selftest` switch (spec 10) with the macOS-corrected size assertions (EDGE-CAP-34).
...
**AC-CAP-2.** `CaptureEngineTests` and `MenuPollTests` pass on Linux (every case in 8.4).
...
**AC-CAP-4.** `JsMathTests` passes; no `Math.Round(` call exists in `ShotAI.Core/Capture` (source scan in the same test class).
...
|---|---|---|
| 01 Model and store | `ProjectStep`, `StepClick`, `StepElement`, `CapturedWindow`, `CapturedMonitor`, `CaptureTarget`, `Rect`, `Point`, `ProjectManifest`; `OpenProjectAsync`, `AddStepAsync`, `InsertStepAtAsync`, `DeleteStepsAsync`, `DeleteProjectAsync` (serialized write queue, renumbering, clamping); `PathConfine.ConfineNoLinks` with reparse-point refusal | new steps (written through the store), `shots/step-NNNN.png` files; the rule that capture never writes `captureSettings` |
...
| 10 Settings and infra | `ICaptureSettings.CaptureScaleNow()` and `RemoteVisibleNow()` synchronous caches; `ILogger` category `capture`; the `--capture-selftest` switch | log lines (2.6 step 18 and section 2 messages) |
| 11 Service boundary | UI-thread marshaling of events | `ICaptureService` (7.2) mapped to the IPC channels `capture:start`, `capture:single` (dropped), `capture:screenshot`, `capture:pause`, `capture:resume`, `capture:stop`, `capture:discard`, `capture:get-state`, `capture:list-targets`, and the events `capture:state-changed`, `capture:step-added`, `capture:error` |
...
**Q-CAP-1. Port `captureSingle`?** It has no UI caller. Recommended default: do not port; drop `capture:single` in spec 11; keep 2.2.4 as documentation only.

**Q-CAP-2. Hotkey rebinding?** Electron has none and four UI strings hard-code `Ctrl+Shift+S`. Recommended default: no rebinding at 2.0.0; revisit as a feature after cutover (it would add a settings key, spec 10).

**Q-CAP-3. Does Chromium's `globalShortcut` pass `MOD_NOREPEAT`?** Unverified. If it does not, holding the chord in Electron repeats captures. Recommended default: use `MOD_NOREPEAT` (IMPROVEMENT) regardless.
...
**Q-CAP-5. Raw Input instead of `WH_MOUSE_LL`.** Recommended default: the hook plus watchdog; switch to Raw Input only if field logs show watchdog reinstalls.
...
**Q-CAP-7. Wording of the new grab-failure error (D6).** Recommended default: `A screenshot could not be captured. If this keeps happening, make sure the target is visible, then try again.`
...
**Q-CAP-11. Monitor id.** Electron used node-screenshots' id (believed to be the `HMONITOR` value, unverified). It is persisted only as metadata and used within a session. Recommended default: `(uint)HMONITOR`, re-resolved each step; never compare ids across launches.

**Q-CAP-12. `captureSettings` in `project.json`.** Electron writes `null` always; macOS writes the target. Recommended default: Electron parity (never write it); spec 01 must preserve a value written by macOS.

**Q-CAP-13. `CAPTUREBLT` cursor flicker and layered menus.** Recommended default: `SRCCOPY | CAPTUREBLT`; if manual testing shows cursor flicker without a menu benefit, drop `CAPTUREBLT` and re-run AC-CAP-7.

**Q-CAP-14. DXGI Desktop Duplication for the menu frame.** Recommended default: not in 2.0.0; BitBlt polling is proven. Revisit only if poll CPU on 4K monitors is a complaint.

**Q-CAP-15. Display affinity on WPF `AllowsTransparency` (layered, `UpdateLayeredWindow`) windows, and from a non-UI thread.** Neither is documented (Microsoft Learn requires only a top-level window of the current process and DWM composition). Whatever the answer, the 7.8 deadlock rule applies (Q-CAP-22). Recommended default: verify with `ShotAI.ProtectionProbe` and `GdiMonitorCaptureTests.LayeredWindowAcceptsAffinity` before building the pill and overlay; if a worker-thread call fails or blocks, marshal `SetAllExcluded` to the UI thread with `Dispatcher.Invoke` and never block the UI thread on capture work (7.3 rule).

**Q-CAP-16. Interpolation mode.** Electron's `'good'` resize is not reproducible exactly. Recommended default: WIC `Fant`; compare legibility of small text against Electron output at 0.5 and 0.85.

**Q-CAP-17. Element names in logs.** Electron logs `el='<name>'` at info (`CaptureController.ts:1455`); names can be sensitive labels and logs travel with support tickets. Recommended default: log the element name at debug only (IMPROVEMENT), controlType at info.

**Q-CAP-18. PNG pixel format.** 32 bpp RGBA (parity) versus 24 bpp RGB (about 25 percent smaller). Recommended default: RGBA for parity; revisit after the AVIF encoder (spec 09) is measured.

**Q-CAP-19. GC pauses on the hook thread.** A long blocking GC could exceed the hook timeout. Recommended default: the watchdog; keep the hook proc allocation-free; if reinstalls appear in logs, move the hook to Raw Input (Q-CAP-5).

**Q-CAP-20. `lastLeftClick` persistence across sessions.** Electron never resets it. Recommended default: reset on `StartAsync` (no observable difference in practice).
