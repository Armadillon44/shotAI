...
As built in WP-B6: Core has get-windows' rules as `WindowDescriber` (the foreground window, the window list and a window target resolved against it), `WindowListFilter` and `VersionInfoKeys`, over the seam `IWindowFacts` (with `WindowListFacts`), which Platform's internal `Win32WindowFacts` answers with the calls get-windows makes; `Win32WindowInfoProvider` is the describer over those facts. Core also has the element locator's queue, `ElementQueryPool`, over the seam `IElementReader`, which Platform's internal `UiaElementReader` implements for one query thread; `UiaElementLocator` is the pool over UI Automation readers on MTA threads. `JsString.ToWellFormed` (JavaScript's `toWellFormed`) makes the strings read from Win32 and UI Automation well formed. `PlatformCaptureRegistration` registers `IWindowInfoProvider` and `IElementLocator`, and `AddShotAICore` registers `ICaptureClock` as `TimeProviderCaptureClock` and `ICaptureService` as `CaptureEngine`, now that every seam its constructor needs is registered (AC-CAP-35).
...
As built in WP-B6: `IWindowInfoProvider` and `IElementLocator` are as above. Their Platform implementations are thin: the rules are Core's, over two Core seams that are never registered in the container, `IWindowFacts` (get-windows' Win32 reads: the foreground and top-level windows, a window's descendants, the list facts, the process id, the image path, the FileDescription, the window and client rectangles, the frame bounds, the title and whether the window is minimized) and `IElementReader` (one query thread's `Read(x, y)`, made, used and disposed on that thread).
...
As built in WP-B6: the rules above are Core's `WindowDescriber` over `IWindowFacts`, tested on Linux (`WindowDescriberTests`, `WindowListFilterTests`), and Platform's `Win32WindowFacts` makes the Win32 calls (`WindowInfoTests`). `ListedWindow.FrameBounds` is the DWM frame bounds, or the window rectangle when DWM has none; `Focused` is whether the window is the foreground window; `Resolve` runs 2.10.3 against a fresh `ListWindows()`. Every string read (the image path, the FileDescription, the title) is made well formed, a lone surrogate becoming U+FFFD, as get-windows' UTF-8 conversion leaves it. The styles are `GetWindowInfo`'s, zero when it fails (so the filter drops the window), and a window whose cloaking DWM cannot report counts as not cloaked (get-windows leaves the value unset then). The title of a window of this process is read with `InternalGetWindowText`, because `GetWindowTextW` and `GetWindowTextLengthW` send such a window a message and would wait for its UI thread, which a capture thread must never do (ARCHITECTURE DL2); the capture never uses its own windows' titles (INV-CAP-6). Corrected in WP-B6, from get-windows' source: the fallback name is get-windows' `getFileName`, the text after the last backslash (empty when there is none), not `Path.GetFileName`; the UWP walk stops at the first descendant whose process opens with an image path other than the frame host's, and adopts that descendant's name only when it is not empty (`newOwner.name.size()`), the frame host keeping its own otherwise; the translation table's first pair is used only when the table holds a whole pair. Q-CAP-4's fallback (`WindowFromPoint`, `GetAncestor`) is not built: parity first, decided with AC-CAP-27 in WP-B11.
...
As built in WP-B6: the queue, the two threads (`shotAI.Uia.0` and `shotAI.Uia.1`, background threads), the 600 ms cap and the deadline are Core's `ElementQueryPool`, tested on Linux (`ElementQueryPoolTests`). Each thread makes its `IElementReader` first and disposes it when the queue ends; a thread whose reader cannot be made answers null and logs `element locator: UI Automation could not be started on <thread>; element names disabled there:` at warning, and a started one `element locator: UI Automation ready on <thread>` at information. A failed read logs only its exception's type and HRESULT, at debug, since the message could describe what was under the point; a query past its cap and a request dropped at its deadline log at debug. Platform's `UiaElementReader` is one thread's reader. Corrected: the automation object is `CUIAutomation8`'s, the class Windows 8 added with `IUIAutomation2`, and `CUIAutomation`'s only when that class cannot be made; both timeouts are 500 ms (`TheTimeoutsAre500Ms`). There is no explicit `CoInitializeEx`, since each thread is put in the MTA before it starts. The climb reads an element's control type only when its name is not empty, as the Rust test short-circuits; names are made well formed, as Rust's lossy decode leaves them; every COM object a read takes is released when the read ends, and each `BSTR` is freed. Found on both runners: within one process, UI Automation holds up a read of one window while another read waits on a hung provider of that process (on x64 the second read failed with a COM exception after 580 ms), while across processes a read of one app answers while another app hangs (`AHungAppLeavesTheOtherThread`). That is what D15 needs, because the own-window gate keeps every query off shotAI's own windows (D1).
...
As built in WP-B6, for the divergences this WP owns:

- D15: two MTA query threads with 500 ms UI Automation timeouts and the queue's deadline (7.6 as built).
- D17: UI Automation through CsWin32 COM in the process; no DLL, no DLL path search, no platform check.
...
As built in WP-B6: `WindowInfoTests` has `ExplorerFileDescriptionIsWindowsExplorer`, `FallsBackToExeName` (a file without a version resource has no FileDescription; the file-name fallback is Core's), `ForegroundOfOwnWindowHasOwnPid` (an own window describes with the own pid, title and app; the test never takes the foreground, since the App's tests run focus tests in another process meanwhile), `TheForegroundIsTheForegroundWindowDescribed`, `ListFiltersToolWindowsAndCloaked`, `ListDropsDisabledAndCaptionlessWindows` (with a popup kept, and an owned window kept only as an app window), `TheListIsTopFirst`, `AMinimizedWindowIsListedMinimized`, `TheListCarriesTheFrameBounds`, `ResolveByIdThenPidTitleThenPid`, `AProcessImageIsItsWin32Path` and `AnOwnWindowsTitleIsReadWhileItsThreadIsBlocked` (DL2); `IsOwnWindowUsesOutPid` is `OwnWindowsTests`' (WP-B5). `UwpAppKeepsFrameHostPid` is Core's, over scripted facts (`WindowDescriberTests`), since the tests launch no Store app; AC-CAP-32 checks a real one. `UiaElementLocatorTests` has the list's three resolutions (`ClickingTheTextResolvesTheButton`, `ATextBoxResolvesEdit`, `PlainTextIsUnavailable`), `HungProviderTimesOut`, `AHungAppLeavesTheOtherThread` (two Windows PowerShell WinForms apps, one of which stops pumping), `TheTimeoutsAre500Ms`, `ReadersAreMadeOnMtaThreads` and `APointOffTheScreenNeverThrows`; `StaleRequestsAreDroppedWhenBothThreadsHang` is Core's (`ElementQueryPoolTests`, 15 cases). Both classes run in the screen collection, alone. New Core `WindowDescriberTests` (26 cases), `WindowListFilterTests` (11), `JsHelpersTests.ToWellFormedReplacesLoneSurrogatesOnly` and `AddShotAICoreTests.RegistersTheEngineAndItsClock`; the App's `ContainerTests` resolves `ICaptureService`, with no `ResolvableFrom` entry left.
...
**AC-CAP-5.** All ShotAI.Platform.Tests capture classes pass on a Windows x64 runner and on a Windows ARM64 machine. Met in WP-B6, on the x64 and arm64 runners.
...
**AC-CAP-35.** `CaptureEngineTests.DisposeIsSynchronousAndIdempotent`, `.EventsGoThroughEventRaiser`, `.JobFailureMessageUsesUserMessage` and `.ThrownMessagesAreShotAIExceptions` pass on Linux, and `Composition.ContainerTests.NoAsyncOnlyDisposables` (spec 11) passes with `CaptureEngine` registered as `ICaptureService` (R-ARCH-10, ARCHITECTURE C5). Met in WP-B6: the engine is registered, and `NoAsyncOnlyDisposables` resolves and checks it.
...
**Q-CAP-8. Which window rect node-screenshots used for window crops** (`GetWindowRect`, client rect, or DWM extended frame bounds). The package ships no source. Recommended default: `DWMWA_EXTENDED_FRAME_BOUNDS` (includes the title bar, excludes invisible borders, matching the comment at `CaptureController.ts:1253-1259`); decided by AC-CAP-34 (Window mode, normal, maximized and 150% cases; AC-CAP-6 covers only auto mode): switch to `GetWindowRect` if Electron's PNGs are 14 to 16 px wider. As built in WP-B6: `DWMWA_EXTENDED_FRAME_BOUNDS`, the window rectangle when DWM has none (`TheListCarriesTheFrameBounds`); the comparison stays with AC-CAP-34 (WP-B11).

**Q-CAP-9. node-screenshots' `Window.all()` filter** (which windows it lists and reports as focused) is not inspectable. Recommended default: the get-windows filter (7.5); compare the Window chooser lists of both builds on one machine. As built in WP-B6: the get-windows filter (`WindowListFilter`, `ListFiltersToolWindowsAndCloaked`, `ListDropsDisabledAndCaptionlessWindows`); the comparison stays with WP-B11.
...
**Risk R4 (medium).** App-name derivation drift (FileDescription) changes captions, the auto classifier and SOP prompts. Mitigated by `WindowInfoTests` and AC-CAP-30. As built in WP-B6: the derivation is Core's, tested against get-windows' source on Linux (`WindowDescriberTests`), and `ExplorerFileDescriptionIsWindowsExplorer` reads a real FileDescription on both runners.
