...
**AC-CAP-2.** `CaptureEngineTests` and `MenuPollTests` pass on Linux (every case in 8.4). `MenuPollTests.StaleFrameIsDisposed` waits for D24's disposable frames and lands with them in WP-B5 (moved in WP-B3).
