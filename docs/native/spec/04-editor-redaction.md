...
| Concern | Owner |
|---|---|
| The `project.json` codec, `ProjectStep` views, the write queue, `AtomicFile`, `PathConfine` and `IPathProbe`, `ProjectSession` and `ApplyDurable`, the transaction around `UpdateStepAsync` and `MergeStepsAsync` | 01 |
...
Errors thrown in the MAIN process (the store's `step ... not found`, the render-write refusal, the known-project gate of the scan handler) reach the renderer through `ipcMain.handle`, and Electron prefixes their message, so the editor notice actually reads `` Error invoking remote method 'projects:update-step': Error: step abc not found `` (and the same prefix with `projects:redact-scan`). The prefix is ELECTRON-ONLY: natively these are direct calls and the notice shows the exception message alone. Errors raised in the renderer (the flatten's `E_UNBAKEABLE`, the load failure, the `ensureFlattened` wrapper) never carried the prefix, although the `ensureFlattened` wrapper can embed a prefixed inner message from `updateStep`.
...
| `ShotAI.Platform.Imaging` | ShotAI.Platform | `WicImageCodec` (spec 02's class) also implements `IRenderCodec` |
...
| `ShotAI.App.Editor` | ShotAI.App | `EditorOverlayView` (XAML), `EditorViewModel`, `EditorCanvas`, `AnnotationPainter`, `SelectionAdorner`, `InlineTextBox`, `BlurPreviewCache`, `WpfOverlayRasterizer`, `StaRenderThread`, `EditorRegistration` (DI) |

Model types (`ProjectStep`, `StepClick`, `Rect`, `Point`, `ProjectManifest`) and `JsMath`, `JsString`, `JsNumber`, `PathConfine`, `IPathProbe`, `AtomicFile`, `ProjectStore`, `ProjectSession` come from spec 01. Spec 11 lists `IStepFlattener` under `ShotAI.Core.Editor`; this spec places it in `ShotAI.Core.Rendering` (Q-EDIT-20). Core references no Windows API; everything Windows-specific is behind `IOverlayRasterizer`, `IRenderCodec` and `IOcrEngine`.
...
public sealed class EditorDocument
{
    ...
    public Rect? Crop { get; }                          // Geometry.ParseRect of the raw crop (EDGE-EDIT-28)
...
{
    PremultipliedImage Decode(ReadOnlySpan<byte> encoded);   // PNG or JPEG; EXIF orientation respected; sRGB; premultiplied BGRA; synchronous (WIC COM, 7.13), call off the UI thread
...
public sealed class FlattenException : Exception
{
...
The rasterizer is called only when `Items.Count > 0`; an empty scene skips step 6's rasterize and composite. Threading: steps 0 to 3 and 6 to 7 run on the thread pool; step 6's rasterization runs on the STA render thread (7.10.6).
...
The store job (spec 01 7.8 `UpdateStepAsync` and `MergeStepsAsync`), natively:

```
resolved = gate; manifest = read; step = find (else throw InvalidOperationException($"step {stepId} not found"))
...
public sealed class RenderGateException(string message) : Exception(message);
...
Spec 07 calls the static form with its probe, spec 09 the injected `IRenderGate`; both run the one algorithm below. Spec 09 names the exception `RenderRefusedException`; this spec's name is `RenderGateException` (Q-EDIT-20).
...
```csharp
public interface IStepFlattener
{
    /// ensureFlattened (2.19). Returns the last manifest written, or null when nothing changed.
    /// Without a session (Home exports, 06): each re-bake calls ProjectStore.UpdateStepAsync directly.
    ...
    /// With the open project's ProjectSession (05, 07): each re-bake persists through session.ApplyDurable,
    /// so the session's Current adopts every written manifest. 05 and 07 write the four-argument call while
    /// saying "persisting through the session"; they must use this overload (Q-EDIT-20).
    Task<ProjectManifest?> EnsureFlattenedAsync(string projectPath, string projectDir,
                                                IReadOnlyList<ProjectStep> steps, ProjectSession session,
                                                CancellationToken ct);
    /// The editor and merge entry point: decode the ORIGINAL, flatten, return PNG bytes.
...
`StepFlattener` implements 2.19 exactly (skip rules, `shotNo` counting, one step at a time, `ct.ThrowIfCancellationRequested()` before each step, `OperationCanceledException` rethrown unwrapped), with: the source read via `PathConfine.ConfineNoLinks(projectDir, step.Screenshot, probe)` (D-EDIT-4) and `File.ReadAllBytesAsync` (a null path or any IO or decode failure becomes `$"Could not load a screenshot to flatten ({step.Screenshot})."`, ELECTRON-ONLY URL replaced by the relative path); the marker `new FlattenMarker(click.Image.X, click.Image.Y, AnnotationStyle.MarkerColorFor(step), click.Radius)` (D-EDIT-18); persistence through `session.ApplyDurable(s => s.UpdateStepAsync(projectPath, step.Id, StepPatchValidator.MarkerBakedOnly(), png))` in the session overload, else `ProjectStore.UpdateStepAsync` directly (Home export, 06); a click that spec 01's lenient `ProjectStep.Click` cannot parse gives no marker instead of Electron's `TypeError` (EDGE-EDIT-68); the source decode and the flatten run on the thread pool, one step at a time; and the error wrap `$"Step {shotNo} (\"{caption}\") couldn't be prepared: {ex.Message}"` with `caption = JsString.Trim(step.Caption)` or `"untitled"` when empty.
...
The editor's scan entry point (App) replaces `projects:redact-scan` (ELECTRON-ONLY IPC): `EditorViewModel.AutoRedactAsync()` calls `SensitiveRegionScanner.ScanAsync(document source image, Path.GetFileName(step.Screenshot), editorCts.Token)` and then `EditorDocument.AddAutoRedactions(rects)`. Notices:
...
`EditorOverlayView` is a `UserControl` placed in the project window's top overlay layer (03 provides the layer), with the 2.2 backdrop (`#8C0F172A`, a 24 DIP margin) and a rounded panel; `AutomationProperties.Name="Edit screenshot"`, and the panel is the keyboard focus scope while open. `EditorViewModel` (CommunityToolkit.Mvvm `ObservableObject`, `[RelayCommand]` for every button) wraps one `EditorDocument` and exposes its state; it is created by an `EditorFactory` resolved from DI with `Flattener`, `IStepFlattener`, `ISensitiveRegionScanner`, `IRenderCodec`, `IPathProbe`, `ILogger<EditorViewModel>`; the open project's `ProjectSession` is not a DI service (one per project window), so 05 passes it to `EditorFactory.Create(session, projectPath, projectDir, step)`. Rail, top bar, properties bar and hint line are XAML bound to the view model with the exact strings of 2.22 (non-ASCII characters as in the Electron source). The tool buttons are `RadioButton`s styled as rail buttons (`AutomationProperties.Name` = the label, `ToolTip` = the tooltip); the redaction mode is a two-button segmented `RadioButton` group named `How to hide this region`. Sliders: WPF `Slider` with `Minimum`, `Maximum`, `SmallChange = 1`, `IsSnapToTickEnabled = true`, `TickFrequency = 1`. The Color control is a swatch button that opens `Win32ColorDialog` (7.13) seeded with the current color and applies the result via `ChangeColor` (parity with Chromium's Windows color input, which opens the same system dialog).

#### 7.10.2 Loading

On open: `abs = PathConfine.ConfineNoLinks(projectDir, step.Screenshot, probe)` (D-EDIT-4); read the bytes on the thread pool; `IRenderCodec.Decode` (thread pool) into the view model's `source` (`PremultipliedImage`, kept for the preview tiles, OCR and the save's flatten); construct the `EditorDocument` (which assigns a fresh id to any `blur` element without a string `id`, D-EDIT-2); create a `WriteableBitmap` (`PixelFormats.Pbgra32`, 96 DPI) from the pixels for display. Any failure: notice error `Could not load the screenshot.`, the canvas shows `Loading screenshot\u2026`, Save and Auto-redact disabled (EDGE-EDIT-44). The file is not kept open (spec 01 EDGE-MODEL-39).
...
`EditorViewModel : IDisposable` cancels its token, drops its references to the `WriteableBitmap` and the preview cache (neither has a `Dispose`), and detaches document events. `StaRenderThread` is a DI singleton disposed at exit. `WindowsOcrEngine` is a singleton; the engine object has no disposal.

### 7.12 Persistence and error handling

| Operation | Path | Error surface |
|---|---|---|
| Editor save | `ProjectSession.ApplyDurable` then `ProjectStore.UpdateStepAsync` (render then manifest in one job, rollback on manifest failure) | notice error with the exception message; editor stays open |
...
| Egress preparation (06, 07, 09) | `IStepFlattener.EnsureFlattenedAsync` then the gate inside 07 or 09 | the caller's notice with the 2.19 or 2.20 message |
...
| `Editor/EditorSaveTests` (fake flattener and session) | `BakedListEqualsPersistedList` (a blur without an id is rekeyed on open; `null`, `42` and `{"type":5}` are absent from both the flatten request and the patch); `PrepareSaveNeverThrows` (a text with numeric `text`, a `null` element); `CancellationClearsSaving`; `NonHexMarkerColorNotWritten` (seeded `"red"` stays on disk); `PatchHasExactlyTheFiveKeys`; `RemovedMarkerPersistsNullClick`; `MovedMarkerKeepsGlobalButtonAndImageScale`; `ReportsSuccessOnlyAfterBothFilesAreWritten`; `FailureKeepsEditorOpenWithState`; `FlattensFromTheOriginal`; `UnknownAnnotationSurvivesSave`; `MarkerBakedTrueWithoutClick`; `MarkerColorAlwaysWritten` |
...
| `Imaging/RenderCodecTests` | PNG round trip is lossless (premultiplied opaque pixels byte-equal); EXIF orientation 6 JPEG decodes rotated (width and height swapped); a transparent PNG keeps alpha through decode and encode |
...
|---|---|---|
| 01 Model and store | `ProjectStep` views (`Raw`, `Annotations`, `Crop`, `Click`, `Flattened`, `RenderRev`, `MarkerBaked`, `MarkerColor`), `StepClick`, `Rect`, `Point`, `Geometry.ParseRect`, `JsMath`, `JsNumber`, `JsString`, `JsJson`, `PathConfine.Confine`/`ConfineNoLinks`, `IPathProbe`, `AtomicFile`, `ProjectStore.UpdateStepAsync`/`MergeStepsAsync`/`GetProjectForReadAsync`, `ProjectSession.ApplyDurable` | `StepPatch`, `Optional<T>`, `StepPatchValidator`, `StepPatchApplier.ApplyAndInvalidate`, `IStepRenderWriter` and `RenderWriteReceipt` (the rollback hook the store job calls), the render path constant |
| 02 Capture | `click.image` in stored-PNG px (after downscale), `click.imageScale`, `click.button`; `WicImageCodec` | `IRenderCodec` requirements (orientation, premultiplied, sRGB) added to `WicImageCodec` |
| 03 Windows and shell | the project window's overlay layer, focus scope, the shared notice control, `HwndSource` access for `WM_MOUSEHWHEEL`, the owner HWND for the color dialog, app-exit hook to stop `StaRenderThread` | `EditorOverlayView`; the rule that the editor cannot be closed while saving |
| 05 Report | the Edit entry point (`OpenEditor(step)`), the merge flow (origin recovery, captions) | `IStepFlattener.FlattenStepAsync` for merges, `AnnotationStyle.MarkerColorFor`, `AnnotationFactory.Marker`, the `markerBaked` overlay rule, the image cache key `(flattened, renderRev)` |
| 06 Home | row and bulk export flows | `IStepFlattener.EnsureFlattenedAsync` (without a session: direct store) |
| 07 SOP | the review-before-send flow and cancel token | `EnsureFlattenedAsync` before estimate and send; `RenderGate.ResolveSendableRender(..., EgressVerb.Send, probe)` as the only image source; the refusal messages |
| 09 Export | export flows, package safe mode | `EnsureFlattenedAsync`; `IRenderGate.Resolve(..., EgressVerb.Export)` (the DI wrapper over `RenderGate`); `RenderGateException` (09 calls it `RenderRefusedException`, Q-EDIT-20); the rule that exports never flatten themselves |
| 10 Settings and infra | `ILogger` categories `ocr`, `editor`, `render`; theme tokens for the editor chrome | log lines of 2.23 and 7.8 |
| 11 Service boundary | nothing | the deletion of `projects:update-step`, `projects:merge-steps`, `projects:redact-scan` (replaced by direct calls); `StepPatchValidator.Parse` as the validation any remaining untrusted-patch entry point must use; `ISensitiveRegionScanner` and `IStepFlattener` (namespace per 7.1, Q-EDIT-20) |
...
**Q-EDIT-20.** Cross-spec names to align (this spec is the owner): spec 09 injects `IRenderGate` and catches `RenderRefusedException`, spec 07 calls `RenderGate.ResolveSendableRender` and catches `RenderGateException`; spec 11 places `IStepFlattener` in `ShotAI.Core.Editor` and names `ISensitiveRegionScanner`; specs 05 and 07 call a four-argument `EnsureFlattenedAsync` while persisting through the session; spec 05 EDGE-REP-40 cites the codec as `ExifOrientationMode.RespectExifOrientation` (WinRT), which 7.13 replaces with spec 02's WIC COM codec. Recommended default: keep this spec's names (`IRenderGate` plus static `RenderGate`, `RenderGateException`, `ShotAI.Core.Rendering.IStepFlattener`, `ISensitiveRegionScanner`, the session overload of `EnsureFlattenedAsync`) and have 05, 07, 09 and 11 follow them when they are next edited.
