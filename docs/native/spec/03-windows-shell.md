...
| `ShellStringsTests` | every constant equals the 2.11 text; the three em-dash strings contain U+2014 exactly once and no U+2013 (each string arrives with the WP of the window that shows it; WP-A13 has `AppName` and `AppNameMatchesTheElectronTitle`); added in WP-A13: `PixelRectTests` (exclusive edges, touching rectangles do not intersect) |
