...
| 2.10.9 | `appIconPath()` resources-then-dev lookup | find the PNG icon inside or outside the asar | `src/main/paths.ts:38-44` | The exe's `ApplicationIcon` is already set in the scaffold (`dotnet/src/ShotAI.App/ShotAI.App.csproj`, `$(RepoRoot)assets/shotAI_icon.ico`), and a WPF window whose `Icon` is unset shows the exe's icon. The About dialog's image and `Window.Icon` read it as the resource `/shotAI;component/Assets/shotAI_icon.ico`, which `ShotAI.App.csproj` adds as `<Resource Include="$(RepoRoot)assets/shotAI_icon.ico" Link="Assets/shotAI_icon.ico" />`. Corrected in WP-A15: the form `pack://application:,,,/Assets/shotAI_icon.ico` names the entry assembly, which is the test host in App.Tests; the component form names the App assembly in every host. The Electron About icon was the PNG (`shotAI_icon.png`); the `.ico` carries the same artwork. |
...
| Native: overlay hit-test fill (new) | `#01000000` | ARGB | alpha 1/255 so a layered overlay receives the mouse everywhere | 7.4.4, EDGE-SHELL-28 |
...
**EDGE-SHELL-46. An owned WPF window is hidden, minimized and closed with its owner and always stays above it.** Setting `Owner = mainWindow` on the pill or an overlay would make `main.Hide()` hide the pill for the whole recording. Required: the pill and the overlays have no `Owner`; only the About dialog (owner main) and the Discard dialog (owner pill) are owned. Electron creates both without a parent (`src/main/main.ts:312-332`, `src/main/RegionService.ts:55-79`). Native pitfall. As built in WP-B7: WPF gives a window whose `ShowInTaskbar` is false a hidden owner of its own, which never shows (7.6.2); `CapturePillWindowTests.HasNoOwner` checks that the pill has no `Owner` and no visible owner, and stays visible when the main window hides.
...
**EDGE-SHELL-52. Closing the overlays at a press of another button hands that button's release to the window beneath them.** Once the overlay is destroyed, Windows sends the release to the window under the cursor, where a right release can open that window's context menu and a side button's release can go Back or Forward. Required natively: from such a press the overlay keeps the capture until every button is up, then ends the selection with `null` (D25). Found in WP-B8's review of its own code; whether Electron's overlay, which closes a few milliseconds after the renderer's `region:cancel`, still takes the release is UNVERIFIED.
...
| ShotAI.App | `ShotAI.App.Shell` | `ShotAIWindow` (base class, 7.4.7), `ShotAIPopup`, `MainWindow`, `MainWindowSizer` (implements `IMainWindowLayout`), `IMainWindowLayout`, `IAreaSelectionService`, `CapturePillWindow`, `CapturePillViewModel`, `DiscardConfirmWindow`, `AreaOverlayWindow`, `AreaSelectionService` (implements `IAreaSelectionService`), `RecordingVisibilityController`, `AppMenuViewModel`, `AboutWindow`, `ActivationListener`, `PopupExclusion`, `WindowRegistration`, `CrashLogging`; added in WP-A15: `ShellCommands` (the window's own menu items as routed commands), `IShellNavigationState` (spec 11 7.3), implemented by 06's `NavigationState` |
...
`DocScale.DetailWindowWidth` is the Core port of `detailWindowWidth` (owned by spec 05 with the rest of `doc-scale.ts`); its formula is fixed in 2.3.2 and pinned by `WindowLayoutTests` regardless of owner. As built in WP-A15, it landed with the window sizing, with the five constants it needs (`HtmlColumnBase`, `DocPadding`, `ReportFrameBase`, `DetailWindowBase` and `WindowChrome`); the detents and `Widths` join with the report (WP-A17). `current.Width == newW` compares doubles that are integral in practice (the App rounds physical sizes back to DIP with the same scale); the App passes widths rounded to 1/1000 DIP to avoid a false inequality from floating error.

Added in WP-A15, so Linux tests pin the unit conversions of 7.4.2 as well: `WindowLayout.ToDip(PixelRect, double scale)` divides each value by the scale and rounds it to 1/1000 DIP with `JsMath.Round`; `WindowLayout.ToPixels(DipRect, double scale)` scales each edge and rounds it on its own, and the size is the distance between the rounded edges, so adjacent rectangles stay adjacent. A rectangle read from Windows survives the trip to DIP and back unchanged at any real scale (the 1/1000 rounding moves an edge by less than half a pixel). Both refuse a scale that is not positive and finite. `ShellConstants` holds, as built, only the main window's four constants; the pill's and the overlay's join with their windows. As built in WP-B7: the pill's six joined (`PillWidth`, `PillHeight`, `PillDockGap`, `FlashMs`, `PulseMs`, `PillTooltipDelayMs`), and `ShellStrings` has the pill's strings and the Discard confirmation's `DiscardConfirm` (`Discard`) and `Cancel` (7.6.3). `PillActiveLabel` writes the count with the invariant culture, digits with no grouping, as JavaScript's template literal writes an integer (`TheCountIsPlainDigits`).
...
As built in WP-B7: as above; `TopCenter` also refuses a scale that is not positive and finite, as `WindowLayout` does, and `NeedsRedock` counts a pill that only touches a work area's edge as off it, since `PixelRect.Intersects` excludes the edges (`TouchingAnEdgeIsOffTheMonitor`).
...
As built in WP-B7: there is no `PillStatus`: `PillViewState.Status` is the engine's own `CaptureStatus`, which has the same three values, and the pill's styles compare with it. `Hint` is null while no session exists, since row 2 then shows nothing. `OnSessionShown` also enables the controls. `Changed` is raised only when the recomputed view differs from the last (record equality). `OnError` keeps a message that is not all white space exactly as it came, its leading and trailing spaces included (`MessageKeptUntrimmed`).
...
As built in WP-B7: `RecordingVisibilityPlanner` and `ShellAction` as written (`RecordingVisibilityPlannerTests`).
...
As built in WP-B8: as written; `ToPhysical` also refuses a scale that is not positive and finite, as `WindowLayout` does, and one rule joins: `OverlayUnder(monitors, x, y)`, the index of the first monitor whose `rcMonitor` contains the cursor, right and bottom edges excluded, or -1 when none does (EDGE-SHELL-27; the focus then stays with the last overlay shown, as Electron's stays with the last overlay ready). `ShellConstants` has the overlay's four constants and `ShellStrings` its title, the hint's two lines and `Badge`, which writes the numbers with the invariant culture. Every file of `ShotAI.Core/Shell` is read for the three Windows namespaces, comments aside, since Core's framework has `System.Windows.Input` and `Microsoft.Win32` (`CoreReferencesTests.TheShellFolderNamesNoWindowsNamespace`, AC-SHELL-29).
...
**UiZoom**: `Factor(level) = Math.Clamp(Math.Pow(1.2, level), 0.25, 5.0)`; `ZoomIn(level) = level + 0.5`, `ZoomOut(level) = level - 0.5`, each then clamped to the level range whose factor stays in `[0.25, 5.0]`; `ActualSize = 0`. Corrected in WP-A15: the ends are the last half steps inside that range, `MinLevel = -7.5` (factor 0.2548) and `MaxLevel = 8.5` (factor 4.7102), not the range's own ends (-7.60 and 8.83): a press that stopped at those would leave the next press back off the half-step grid, and the same number of presses back would no longer return to factor 1. Chromium clamps the factor it draws; whether Electron's stored level also stops at the ends is UNVERIFIED.
...
All of these are public helpers the App calls directly (ARCHITECTURE INV-ARCH-4); CsWin32 output stays `internal` behind them (ARCHITECTURE 14.7). `MonitorQueries` shares its enumeration code with spec 02's `GdiMonitorCapture.Monitors()`; if spec 02 lands first, extend its descriptor with `WorkArea` instead of adding a second enumerator. (Spec 02's `MonitorDescriptor` is `(uint Id, string Name, Rect Bounds, double ScaleFactor, bool IsPrimary)`, with `Rect` of doubles; `MonitorDescriptorEx` here uses integer `PixelRect` and adds `Handle` and `WorkArea`, `WorkArea = MONITORINFO.rcWork`.) `ExistingInstance.Activate` must not block: `PostMessageW`, never `SendMessage`, so a hung first instance cannot hang the second launch; it returns whether the message was posted.

As built in WP-A15: `MonitorQueries` came first, so spec 02's enumerator (WP-B3) extends it rather than the reverse. `Scale` is `GetDpiForMonitor(MDT_EFFECTIVE_DPI)` over 96, which is the real DPI only in a per-monitor aware process (the app's manifest declares PerMonitorV2; an unaware test host reads 96 everywhere). `CursorPosition` throws `Win32Exception` when Windows gives no position (a desktop that is not the input desktop); the main window then opens on the primary monitor. `Handle` is the `HMONITOR` as a `nint`.

Added in WP-A13: `public static class WindowMessages { public static uint Register(string name); }` (`RegisterWindowMessageW`, throws `Win32Exception` on failure), through which the App's `ActivationListener` gets the activation message id, since the App has no interop of its own; and `public sealed class WindowShowHook : IDisposable { public static WindowShowHook Install(Action<nint> showing, Action<nint> destroyed); }`, the Q-SHELL-3 catch-all of 7.4.7, which reports on the installing thread only and never lets a callback's exception leave the hook procedure. Corrected in WP-A13: `WindowStyles` reads and writes `GWL_EXSTYLE` with `GetWindowLong` and `SetWindowLong`, because CsWin32 cannot generate the `Ptr` variants, which exist only in 64-bit `user32.dll`, for an AnyCPU assembly, and the extended style is a 32-bit value; `GetWindowRect` throws `Win32Exception` for a handle that is not a window.
...
As built in WP-B7: exit step 2 calls `ICaptureService.Teardown()`, and `SessionEnding` sets `IsShuttingDown` and calls `Teardown()` (`App.EndSession`). `IsShuttingDown` is `ShellShutdown`, an App singleton that File, Exit and the main window's `OnClosed` set before the shutdown goes on, and that the pill's `Closing` reads. Step 8 creates the pill's HWND with `EnsureHandle()`, closes the pill when the main window closes (`App.ClosePillWithMain`, so the pill's veto never keeps the run alive) and attaches both windows to `RecordingVisibilityController`; step 9 starts the controller between `RemoteVisibilityApplier` and `ThemeManager`.
...
- `MainWindowSizer.SetDetailView(bool open, double scale)` (the implementation of spec 11's catalog interface `IMainWindowLayout`, `ShotAI.App.Shell`, singleton): UI thread only. `hwnd`; `m = MonitorQueries.ForWindow(hwnd)`; `s = m.Scale`; `cur = GetWindowRect(hwnd) / s` (DIP, rounded to 1/1000); `wa = m.WorkArea / s`; `r = WindowLayout.DetailResize(cur, wa, open, scale, WindowState == Maximized, _fullScreen)`; if not null, `SetBoundsNoActivate(hwnd, (r * s) with JsMath.Round per edge, topmost: false)`. The outer window rect (`GetWindowRect`, with the invisible resize borders on Windows 10 and 11) is used on both sides, and WPF's `Width` means the same outer size, so the 720 and 1010 constants keep one meaning in the native app. Whether Electron's `getBounds` and `BrowserWindow` `width` include those invisible borders is UNVERIFIED (spec 02 7.9 records the same gap), so the visible width may differ from Electron's by about 14 px (Q-SHELL-20). Callers: spec 05 and 06 on entering or leaving a project and on every committed scale change, exactly like the Electron effect (2.3.2). As built in WP-A15: the main window attaches itself to the sizer when it is made; before that, after it closed (no handle, Electron's `isDestroyed()`), and while it is minimized (Windows reports the rectangle of its minimized icon, not its normal one; added in WP-A15) the call does nothing.
- Recording hide and restore through `RecordingVisibilityController` (7.4.6).
- `ShowFromSecondInstance()`: `Show()`, then `Foreground.TryActivate(hwnd)` (2.2), whose `SW_RESTORE` brings a minimized window back to the state it had before, maximized included, as Electron's `restore()` does (corrected in WP-A13: setting `WindowState = Normal` first would drop a maximized state, and `Activate()` is the same `SetForegroundWindow` that `TryActivate` makes). A signal that arrives after the window closed, while the app exits and before exit step 4 disposes the listener, is ignored (added in WP-A13).
- Full screen (View then Toggle Full Screen, F11): store `WindowStyle`, `ResizeMode`, `WindowState` and bounds; set `WindowStyle = None`, `ResizeMode = NoResize`, then bounds = `rcMonitor` of the window's monitor; the menu stays visible (Q-SHELL-11); F11 again restores the stored values. `_fullScreen` feeds `DetailResize` (D6). As built in WP-A15: `MainWindow.ToggleFullScreen` and `IsFullScreen`. A maximized window is first set to `Normal`, so the stored rectangle is its normal one; leaving full screen puts back the frame, the resize mode and that rectangle, then the maximized state.
...
- The window derives from `ShotAIWindow`, which registers it; `OnSourceInitialized` then applies `MakeNonActivatingToolWindow` and adds Platform's `WindowStyles.NoActivateOnClick` as the `HwndSource` hook, since the App has no interop of its own. `Closing` is cancelled unless `ShellShutdown.IsShuttingDown` (7.4.1).
- The pill's colours and its 7 and 5 DIP corners are `toolbar.css`'s own, as `Pill.*` resources of the window rather than theme tokens: the pill looks the same under every brand and appearance, as Electron's toolbar page did. `XamlChromeGuard` exempts the `Pill.` prefix.
- The error's glyph and message carry the whole message as their tooltip, with the 400 ms delay. The row's `LiveSetting` is `Assertive`, and the message's `LiveRegion` text raises `LiveRegionChanged` when it changes.
- The flash and the pulse are `PillAnimations`: keyframe animations started from code on the ring and the dot, with the splines, times and reduced-motion rule above. A new step restarts the flash from its beginning.
- Corrected in WP-B7: the drag measures the cursor's travel from where the button went down (the press's own position, `PointToScreen` of the event's position), not from `GetCursorPos` when the UI thread takes the press. On both runners the press was taken after the cursor had moved on, and the pill trailed the cursor by the difference. Each move still reads `GetCursorPos` (Platform's `MonitorQueries.CursorPosition`). Corrected too: the pill follows every `WM_MOUSEMOVE` it gets while it holds the capture, through its `HwndSource` hook (Platform's `WindowMessages.MouseMove`), not WPF's `MouseMove`, which WPF raises only when the cursor's place within the window changes. The drag keeps the pill under the cursor, so a second step equal to the first puts the cursor on the same place, and on both runners the pill missed it.
- `CapturePillViewModel`: Pause, Resume, Stop and Discard are `AsyncRelayCommand`s and `DismissErrorCommand` is a `RelayCommand`. A Stop or a Discard that fails logs `pill: <action> failed:` and then reads the engine's state into the presenter, because no state may follow to enable the controls again (D4 would leave the pill without controls). The Discard command awaits one turn of the UI thread's queue through `IUiDispatcher.Post`, then asks the `ConfirmDiscard` delegate the window sets, which shows `DiscardConfirmWindow` with `ShowDialog`; the view model knows no window, and without a delegate nothing is discarded.
...
- `AreaSelectionService` has a public constructor over `MonitorQueries.All` and `MonitorQueries.CursorPosition` and an internal one over stand-ins, as `MainWindowSizer` does, and `AddShotAIApp` registers it as the singleton `IAreaSelectionService`. Before anything changes it refuses a call off the UI thread (`InvalidOperationException`) and a requester that has closed, whose handle WPF refuses (`EnsureHandle`).
- A selection that a newer one ended leaves its requester hidden only when the newer selection hid the same window; a requester of its own comes back at once, so no window stays hidden for nobody (D23 as built). A requester closed during the selection is not shown again.
- A monitor list that cannot be read, or an overlay that cannot be made, ends the selection: the overlays already open close, the requester comes back, and the exception reaches the caller.
- The overlay is `AreaOverlayWindow(registration, ui, monitor, finish)`. `finish` is the service's `Finish` for the overlay's generation; `ui` posts the bounds back after a DPI change, since the App may not call the dispatcher itself (RS0030). `OnSourceInitialized` places it in the two steps of 7.7: onto the monitor with `MoveNoActivate`, then the monitor's rectangle with `SetBoundsNoActivate`.
- No overlay needs the `Loaded` guard: the service makes and shows the overlays in one synchronous step, before any input or cancellation can run, and `Finish` closes every overlay of its selection.
- The overlay takes the keyboard focus when it is activated (`Focusable=True`), so Esc reaches whichever overlay is active; `PreviewKeyDown` handles it on every overlay (D12).
- Corrected in WP-B8: a press of any button but the left does not end the selection at the press. The overlay keeps the capture, ignores every later press, and reports null once every button is up again (D25). Closing the overlays at the press hands the button's release to the window beneath them, where a right release can open that window's context menu and a side button's release can go Back or Forward (EDGE-SHELL-52). The result and its log line are unchanged.
- The input handlers call the overlay's `Press`, `Move` and `Release`, which the tests call with chosen points. A left press takes the mouse capture before it records the press point: WPF raises a move at the cursor when the capture changes, which would otherwise end the new drag there.
- Windows keeps a top-level window within the virtual screen's size with its frame (`SM_CXMAXTRACK`), which no monitor exceeds, so an overlay is never cut down and Electron's `enableLargerThanScreen` needs no counterpart.
...
As built in WP-A15 (all but Brand, which joins in WP-A18 with its separator): the menu is XAML in `MainWindow`, whose data context is `AppMenuViewModel`. The view model holds only what is not the window's: `ImportProjectCommand` and `OpenSettingsCommand` (raising `ImportProjectRequested` and `OpenSettingsRequested`), and `ZoomLevel`, `ZoomFactor`, `ActualSizeCommand`, `ZoomInCommand` and `ZoomOutCommand`. The items that act on the window itself are routed commands, `ShellCommands.Exit`, `ToggleFullScreen`, `Minimize`, `Close` and `About`, which the main window's command bindings handle, so the view model never holds the window. Edit's items are `ApplicationCommands`, routed to the focused element (D10). Every item's `InputGestureText` is set, `""` for Delete, so WPF shows no `Del` and localizes nothing; the key bindings are the window's `InputBindings`. The zoom is a `LayoutTransform` on the content below the menu bar, which keeps its size (Q-SHELL-12). The menu keeps WPF's default look, the system's font and colours, as the Win32 menu bar had. The not-while-recording rule of Import Project and Settings (INV-SHELL-18) joins with their handlers, which need the capture service (WP-D15, AC-SHELL-22); Exit's `IsShuttingDown` flag (EDGE-SHELL-51) joins with the pill (WP-B7).
...
As built in WP-A18: View ends with a separator and the Brand submenu (`BrandMenu`), whose `IsEnabled` is `AppMenuViewModel.BrandMenuEnabled` and whose `ItemsSource` is `BrandItems`: one `BrandMenuRowViewModel` per row of `BrandMenuModel.Items` (App default, then each brand in catalog order), made once in the constructor, so the menu is right from its first frame (EDGE-SHELL-11), and never replaced. A row has `BrandId`, `Label`, `IsChecked` and `Command` (the menu's `ChooseBrandCommand`); the container style sets `Header` to `Label`, `IsCheckable` to false, `IsChecked` one way, `InputGestureText` to `""`, `Command` and `CommandParameter` to `BrandId`. `AppMenuViewModel(IShellNavigationState, ISettingsService, IUiDispatcher, ILogger<AppMenuViewModel>)` recomputes on `IShellNavigationState.Changed` (UI thread, at once) and on a settings change of `Brand` (posted, then re-read); it compares the input triple `(ProjectOpen, RawProjectTheme, AppBrand)` with the last one and, only when it differs, logs L3 and updates the rows, each raising only a value that changed (EDGE-SHELL-12). It is disposable and leaves both events on `Dispose`. The L3 line writes `open` as `true` or `false`, as Electron's template string did. Corrected in WP-A18: the command does not apply the operation itself, because the open session is the project view's own (05 INV-REP-31) and the menu, a singleton, holds no reference to it. `ChooseBrandCommand(brand)` reads `OpenProjectPath` at execution (EDGE-SHELL-13); with none it returns; else it raises `ProjectThemeChosen` with a `ProjectThemeChoice(ProjectPath, Brand)`, as File's items raise their requests, and the shell hands it to `ProjectDetailViewModel.SetProjectTheme(path, brand)`, which applies `SetProjectThemeOperation(brand)` to its session if that session's project is still `path`. The value still passes through untouched (INV-SHELL-17, INV-IPC-14), and an id outside the catalog throws `ArgumentException` from the operation's constructor before anything is applied (D-IPC-9). A failed write rolls back through the session, and the menu, the theme and the project view follow `Current` back; the project view shows 05's rollback notice. `NavigationState` raises `Changed` through `EventRaiser`, so a failing subscriber is logged as `event handler failed: Changed` and neither the other subscribers nor the brand change fail (11 EDGE-IPC-6). AC-SHELL-21's "debug build that re-raises the navigation state every second" is any Debug build run with `SHOTAI_DEBUG_NAV_PULSE=1`: `DebugNavigationPulse` then raises `NavigationState.Changed` once a second with nothing changed; a Release build has neither the switch nor the code.

`AboutWindow`: a small WPF dialog (a `ShotAIWindow`) owned by the main window, `ResizeMode="NoResize"`, `ShowInTaskbar="False"`, `WindowStartupLocation="CenterOwner"`, title `About shotAI`, the 64 x 64 icon, message `AboutText.Message(info.Version)` in bold, detail `AboutText.Detail(info.DotNetVersion, info.WebView2Version, info.Arch)` selectable, where `info = IAppInfo.Current` (spec 11 7.3.5; a lock-free snapshot the UI thread may read, T9; the WebView2 probe runs lazily on the first read), one `OK` button (`IsDefault` and `IsCancel`). The App never references WebView2 for this (R-ARCH-12, INV-ARCH-5). Registered like every window (INV-SHELL-1). A Win32 `MessageBox` is not used anywhere in the app, because it cannot be registered before it is shown; the only exception is spec 12's `LegacyInstanceGuard` notice before any window exists, allowlisted until stage S5 (Q-SHELL-19, ARCHITECTURE 14.9). As built in WP-A15: `AboutWindow(WindowRegistration, string message, string detail)`, made and shown with `ShowDialog` by the main window's About binding from `IAppInfo.Current`; `SizeToContent="WidthAndHeight"`; it wears the theme (`Brush.surface`, `Brush.ink`, `Font.stack`, `Fs.body`, the message at `Fs.title`) and the `Button.Primary` style; the one button's `IsCancel` closes the modal dialog on a click, Enter and Esc alike.
...
As built in WP-B7: the controller acts on the windows through the seam `IRecordingWindows` (`PillOffScreen()`, `Apply(ShellAction)`), which `RecordingWindows` implements on the main window and the pill and which startup step 8 attaches; until then a change moves no window. The off-screen check runs only for a start that shows the pill. `OnSessionShown` runs just before `ShowPill`, with the state read then. A step on a window that has closed is skipped, as Electron skips a destroyed one. `DockPill` centres with `JsMath.Round(380 * scale)` whatever width the hidden pill reports, and docks once more on the pill's next `DpiChanged` when the two differ (EDGE-SHELL-48). The controller posts one action per event and coalesces none (spec 11 Q-IPC-20, WP-B9b).

#### 7.4.7 Window registration and popups (`WindowRegistration`, `PopupExclusion`)

- `WindowRegistration.Register(Window w)`: `hwnd = new WindowInteropHelper(w).Handle`; `ownWindows.Register(hwnd)` (spec 02 7.9: applies `ExcludedForNewWindow` immediately); `w.Closed += (_, _) => ownWindows.Unregister(hwnd)`. Called from each window's `OnSourceInitialized`, which WPF raises after the HWND exists and before it is first shown. A base class `ShotAIWindow : Window` does this so no window can forget (`AllWindowsRegisteredTests` enforces it). Added in WP-A13: `ShotAIWindow` takes the `WindowRegistration` in its constructor (a container singleton; the composition root passes it to `new MainWindow(...)`) and registers before `base.OnSourceInitialized` raises `SourceInitialized`; `WindowRegistration.Register(HwndSource)` is the popup form, whose HWND leaves the registry when WPF disposes the source.
- `PopupExclusion.Install()` (corrected in WP-A13, Q-SHELL-3 decided by `AllWindowsRegisteredTests`): on the UI thread, before any window, it installs Platform's `WindowShowHook`, the Q-SHELL-3 catch-all: a thread-local `SetWindowsHookEx(WH_CALLWNDPROC, ..., null, uiThreadId)` whose procedure, for a top-level window (no `WS_CHILD`) receiving `WM_WINDOWPOSCHANGING` with `SWP_SHOWWINDOW` or `WM_SHOWWINDOW` with a nonzero `wParam`, calls `OwnWindowRegistry.Register(hwnd)`, and for `WM_NCDESTROY` calls `Unregister(hwnd)`. Windows calls the procedure synchronously before the window procedure, so every window the UI thread shows (a tooltip's, a context menu's, a `ComboBox`'s templated drop-down, a third-party control's `Popup`, a system dialog's) is excluded before it is visible, and leaves the registry when destroyed. The hook is removed when the container disposes `PopupExclusion`. The first design, class handlers for `ToolTip.OpenedEvent` and `ContextMenu.OpenedEvent` (routed events) and a `ShotAIPopup : Popup` subclass whose `OnOpened` registers (`Popup.Opened` is a CLR event), each finding the popup's `HwndSource` (`PresentationSource.FromVisual(child) as HwndSource`), measured late on both Windows runners: each popup's HWND had display affinity 0 when it became visible (WPF raises `Opened` after it shows the window), and a `ComboBox`'s drop-down, the `Popup` inside the theme's template, was reached by no handler. The class handlers (installed once per process) and `ShotAIPopup` remain as a second layer: each registers its popup through the `ShotAIWindow` of its placement target, a no-op once the hook has registered it, and still registers a popup shown on a thread without the hook, late rather than never; a popup with no `ShotAIWindow` is excluded directly with `CaptureExclusion.Apply`.
- System common dialogs (Q-EDIT-21). `ChooseColor` and the `Microsoft.Win32` `OpenFolderDialog`, `OpenFileDialog` and `SaveFileDialog` create top-level HWNDs on the UI thread that no `ShotAIWindow` sees. Corrected in WP-A13: the show hook of the previous item runs for the life of the app, so it registers each of them before it is first visible and unregisters it when it is destroyed; no per-call hook is installed around `ShowDialog`, and `AllWindowsRegisteredTests.AWin32WindowOfTheUiThreadIsExcludedBeforeItIsShown` pins the mechanism with a plain Win32 window. `ChooseColor`: spec 04's `ShotAI.Platform.Dialogs.Win32ColorDialog` may still open it with `CC_ENABLEHOOK` and register on `WM_INITDIALOG` as a second layer. File dialogs: the fallback, if a dialog ever shows its top-level window from another thread, is to drive `IFileDialog` directly and register the HWND from `IFileDialogEvents` through `IOleWindow.GetWindow` (Q-SHELL-22). `AllWindowsRegisteredTests` has a case for each dialog when the WP that adds it lands.

#### 7.4.8 Single-instance activation (`ActivationListener`)

`Start(sid)`: `HwndSource` with `new HwndSourceParameters(SingleInstanceIdentity.ActivationWindowName(sid)) { WindowClassStyle = 0, WindowStyle = 0, ParentWindow = new IntPtr(-3) /* HWND_MESSAGE */ }`, created on the UI thread; since `HwndSource` does not let the caller name the window class, the second instance searches by window name instead: `FindWindowExW(HWND_MESSAGE, null, null, windowName)` (7.3 `ExistingInstance.Activate` takes the name for the lookup). The hook handles the registered `ActivateMainWindow` message: `mainWindow.ShowFromSecondInstance()` on the UI thread (the hook already runs there). The second instance's `AllowSetForegroundWindow(pid)` lets the first take the foreground (the launching process is the foreground process at that moment). After a personal-copy hand-off (12 7.10.4) the second instance was started by the personal copy, not by the shell; `ProcessStarter` passes the personal copy's foreground right on with `AllowSetForegroundWindow(<started pid>)`, so this still holds. Only same-session, same-user, same-integrity instances can see each other, which matches the lock scope. `Dispose()`: `HwndSource.Dispose()`. Added in WP-A13: the composition root constructs the listener at step 11 with `main.ShowFromSecondInstance` and its logger, keeps it in a field beside the instance lock, and disposes it at exit step 4, so it is not a container singleton; the message id comes from `WindowMessages.Register(SingleInstanceIdentity.ActivationMessageName)`; `Start` twice throws.

#### 7.4.9 Crash logging (`CrashLogging`)

| Source | Handler | Behavior |
|---|---|---|
| UI thread | `Application.DispatcherUnhandledException`, hooked as the UI dispatcher's own `Dispatcher.UnhandledException`, which it is raised from, so the tests raise it without an `Application` (corrected in WP-A12) | log error `unhandled exception on the UI thread:` with the exception; `e.Handled = true` (parity: Electron's main process keeps running after `uncaughtException`); if the main window is visible, show the generic notice `Something went wrong. See the log for details.` through `INoticeService` (Q-SHELL-16 default, ARCHITECTURE 8.3; as built in WP-A16: startup step 8 calls `CrashLogging.AttachNotices(notices, mainWindowVisible)` after it makes the main window, and a handled exception shows the notice only while that window is visible); an exception from `OnStartup` is not handled and terminates the process with its log line |
...
- As built in WP-A16: `ShellView`'s top layer (`OverlayLayer`) spans the header row and the content row, with the same two rows through a shared row size, so `NoticeHost` sits in the content row, 0.6rem below the header, whatever the header's height, and scrolls with no view. The layer has no background: a click that misses its content reaches the view below. The notice text announces itself through `ShotAI.App.Chrome.LiveRegion` (06 7.10).
...
Corrected in WP-B7: WPF implements `ShowInTaskbar=False` by making the window the owned window of a hidden window of its own, so the pill's `GW_OWNER` is not 0 on either runner. That owner never shows, so the pill is never hidden, minimized or closed with a window the user sees: `Owner` stays null (EDGE-SHELL-46), and `WS_EX_TOOLWINDOW` keeps the pill out of Alt+Tab.

#### 7.6.3 Discard confirmation dialog (`DiscardConfirmWindow`)

Title `shotAI`; icon none; message text `ShellStrings.DiscardWholeProject` or `ShellStrings.DiscardSessionSteps` (verbatim, REQUIRED); buttons `Discard` and `Cancel`, `Cancel` is `IsDefault` and `IsCancel` (IMPROVEMENT over Electron's `OK` and `Cancel`: the destructive button says what it does, as on macOS, and Enter does not destroy; Q-SHELL-7). Shown with `ShowDialog()` owned by the pill; `Loaded`: `Activate()`. Closed as cancelled if the session ends while it is open (the pill hides). As built in WP-B7: `Loaded` first moves the dialog into its monitor's work area when `CenterOwner` placed part of it off screen (a pill dragged to an edge), then activates it; `Discard` is `Button.Danger` and `Cancel` is `Button.Base`; `HidePill` closes it as cancelled through the pill's `CancelDiscard`.
...
- Overlays: created per monitor from `MonitorQueries.All()`; each is moved to its `rcMonitor`; when WPF raises `DpiChanged` for the move, the overlay re-applies its bounds (Windows' suggested rectangle scales the size and would no longer match the monitor). Conversion uses the overlay's own DPI (INV-SHELL-15). As built in WP-B8: the move onto the monitor comes first, then the rectangle, and after a later DPI change the rectangle goes back through `IUiDispatcher.Post` once WPF has applied the suggested one. No test changes a DPI: the runners have one monitor at 100%, and ShotAI.App.Tests runs without the app's per-monitor manifest; AC-SHELL-15 and AC-SHELL-16 check it on the 100% plus 150% rig in WP-B9a.
...
| info | `region selected: <w>x<h> @ (<x>,<y>) [physical px]` |
| debug | `region: selection cancelled` (as built in WP-B8, the three `region` lines as written, the rectangle's values written as integers) |
...
| D25 | A press of any button but the left ends the area selection with `null` once every button is up again, not at the press | IMPROVEMENT | EDGE-SHELL-52: the release reaches the overlay, not the window beneath it (added in WP-B8). |

As built in WP-B7: D1 to D5 and D8 as written, and D18's teardown at `SessionEnding` (7.4.1).

As built in WP-B8: D12, D13, D22 and D24 as written; D23 for a newer selection of the same requester, while a requester of its own comes back at once (7.4.4); D25 added.
...
| Class | Cases |
|---|---|
| `WindowLayoutTests` | `OpenAtScaleOneGives1010` (current 720 at x 100 on a 1920 work area at 0: newW 1010, x = max(0, min(round(460 - 505), 910)) = 0); `OpenAtScale125Gives1214`; `OpenAtScale065Gives1010`; `DetailClampedToWorkArea` (1.25 on 1100 gives 1100); `DetailFloorExceedsNarrowWorkArea` (900 gives 1010, x = wa.X; EDGE-SHELL-34); `DetailIsGrowOnly` (current 1300, target 1214: null); `CloseGoesTo720`; `SameWidthIsNoOp`; `MaximizedIsNotResized` (both directions); `FullScreenIsNotResized`; `CenterPreservedAndClamped` (right edge clamp); `RoundsLikeJavaScriptForNegativeHalves` (work area X -1920, width 1920; current X -999, width 721; open at scale 1: centerX = -638.5, -638.5 - 505 = -1143.5, JS round gives -1143, clamp to [-1920, -1010] keeps -1143; C# `Math.Round` would give -1144); `ClampScaleSnapsIntegerPercent` (0.825 gives 0.85, NaN gives 1, 2 gives 1.25); `InitialClampsHeight` (work area 1093 x 574 gives height 574; 1093 x 500 gives 560). Added in WP-A15: `DetailGrowsAWindowNarrowerThanTheTarget`, `TheLeftEdgeClampsToTheWorkArea`, `RoundsFractionsToTheNearestAndHalvesUp` (355.3 is 355, 355.5 is 356), `YAndHeightAreKept`, `InitialIsCentredInTheWorkArea`, `InitialRoundsLikeJavaScript`, `InitialRoundsFractionsToTheNearest`, `ToPixelsScalesEachEdge`, `ToDipRoundsToThousandths`, `PixelsSurviveTheTripThroughDip`, `AScaleThatIsNotPositiveAndFiniteIsRefused` and `TheConstantsAreElectrons` (`src/main/main.ts`); the `detailWindowWidth` group of `src/shared/doc-scale.test.ts` ported as `Geometry/DocScaleDetailWindowWidthTests` |
| `PillDockingTests` | `TopCenterAt100Percent` (work area 0,0,1920,1040, width 380: (770, 8)); `TopCenterAt150PercentPhysical` (work area 0,0,2880,1560, width 570, scale 1.5: (1155, 12)); `OddWidthRoundsLikeJs`; `NegativeOriginMonitor`; `NeedsRedockWhenOffAllMonitors`; `NoRedockWhenPartlyVisible`. As built in WP-B7: 20 cases, also `APillWiderThanTheWorkAreaCentresOverIt`, `TheGapScalesWithTheMonitor` (four scales), `TouchingAnEdgeIsOffTheMonitor`, `NoMonitorMeansRedock`, `AScaleThatIsNotPositiveAndFiniteIsRefused`, `ArgumentsAreChecked`, `TheConstantsAreElectrons` (the size and gap from `main.ts`, the animations from `toolbar.css`) and `TheTooltipDelayIsSpec03s` (section 3's row) |
| `PillPresenterTests` | label group (idle `shotAI`, `Capturing · 3`, `Paused · 3`); controls group (Pause only while recording, Resume only while paused, none while idle); hint group (recording and paused strings); error group (`EmptyMessageUsesFallback`, `WhitespaceMessageUsesFallback`, `MessageKeptUntrimmed`, `ClearedWhenStepLands`, `ClearedWhenSessionEnds`, `ClearedByDismiss`, `NewerErrorReplaces`, `HiddenWhileIdle`, `ClearedAtSessionStart`); flash group (`FlashOnIncreaseWhileActive`, `NoFlashAtSessionStartOnNonEmptyProject`, `NoFlashWhileIdle`, `TokenIncrementsPerStep`); accent group (`AccentRedOnlyWithError`); `DiscardMessageFollowsState`; `ControlsDisabledAfterStopUntilNextState`. As built in WP-B7: 33 cases, the groups above with `TheLabelFollowsEachState` (the status too), `TheDotPulsesOnlyWhileRecording`, `KeptWhilePaused`, `FlashWhilePausedToo`, `EachSessionStartsItsFlashesAgain`, `ASessionShowsEnabled`, `ChangedOnlyWhenTheViewChanges` and `ArgumentsAreChecked` |
| `RecordingVisibilityPlannerTests` | `StartWithPillHidesDocksShows`; `SecondStartDoesNotRedock`; `RedocksWhenOffScreen`; `ScreenshotHidesMainOnly`; `EndHidesPillShowsAndActivatesMain`. As built in WP-B7: as listed, the last for three combinations of its inputs, 7 cases |
| `AreaSelectionMathTests` | `NormalizeAnyDirection` (drag up-left); `MinDragBoundary` (3.99 rejected, 4 accepted, per side); `BadgeThresholds` (39.9 x 22 hidden, 40 x 21.9 hidden, 40 x 22 shown); `ToPhysicalAt100Percent` (sel 10.4, 20.6, 300.5, 200.4 on monitor 0,0: 10, 21, 301, 200); `ToPhysicalAt150Percent` (sel 10, 10, 101, 51 on monitor 1920,0 at 1.5: 1935, 15, 152, 77); `ToPhysicalNegativeOrigin` (monitor -2560, 0 at 1.25); `BadgeEqualsResult`; `BadgeTextFormat` (`1280 × 720px`). As built in WP-B8: 37 cases, also `ToPhysicalRoundsEachValueBeforeScaling`, `ToPhysicalFloorsTheOriginAndCeilsTheSize` (at 125%, where rounding would differ), `ANegativeOffsetRoundsLikeJavaScriptAndFloors` (past the left edge and past the top: -11.5 rounds to -11, then -13.75 floors to -14), `ADragPastTheOverlaysEdgeIsNotClamped` (EDGE-SHELL-26, Q-SHELL-6), `TheBadgeShowsTheCapturedSizeNotElectrons` (100.4 DIP at 150% is 150 px, where Electron's badge said 151), `TheOverlayUnderTheCursorTakesTheFocus` (the edges), `OverlappingMonitorsGiveTheFirst`, `NoMonitorGivesNoOverlay`, `AScaleThatIsNotPositiveAndFiniteIsRefused` and `TheConstantsAreElectrons` (`overlay/App.tsx`, `RegionService.ts`, `overlay.css`); `ToPhysicalNegativeOrigin` covers a monitor above the primary too |
| `BrandMenuModelTests` | truth table rows of 2.8.2 (`NoProjectDisablesSubmenu`, `NoPinTicksAppDefault`, `UnrecognisedPinTicksNothing`, `PinnedDefaultTicksShotAI`, `PinnedLfiTicksLfi`); `EmptyStringThemeTicksAppDefault` (`""` is not unrecognised); `UnknownAppBrandLabelsDefault` (`App default (shotAI)`); `AppBrandLfiLabel` (`App default (LFI)`); `EveryBrandListedInCatalogOrder`; `PrototypeNameIsUnrecognised` (`toString`, #89); as built in WP-A18 also `IdsAreMatchedExactly` (`LFI`, `shotai`), `AtMostOneRowIsTicked` (the whole table) and `ArgumentsAreChecked` |
| `UiZoomTests` | factor at 0, 0.5, -0.5; clamp at 0.25 and 5.0; `ActualSizeResets`. As built in WP-A15: `FactorIsOnePointTwoToTheLevel`, `FactorStopsAtFive`, `FactorStopsAtAQuarter`, `ActualSizeResets`, `EachPressMovesHalfALevel`, `TheEndsAreTheLastHalfStepsInsideTheFactorRange`, `ZoomStopsAtTheEnds`, `PressesPastTheEndAreNotStored` |
| `AboutTextTests` | exact message and detail strings with `\u2014`, `·`, `\n\n` and `\n` placement; `NullWebView2VersionShowsNotInstalled` (the `AppInfo.WebView2Version` that `IWebView2RuntimeInfo` leaves `null` when the runtime is missing, R-ARCH-12); added in WP-A15: `TheTaglineIsElectrons` (`src/main/menu.ts`), `NullsAreRefused` |
| `RuntimeDiagnosticsTests`, `RenderModePolicyTests` | 8.1 |
| `SingleInstanceIdentityTests` | names for a sample SID (use a made-up SID such as `S-1-5-21-1-2-3-1001`, never a real one); `Local\` prefix; stable across calls; added in WP-A13: `EachUserHasItsOwnNames`, `EmptyAndNullAreRefused`, `BackslashIsRefused` (a SID never holds one; it would move the mutex into another namespace) |
| `ShellStringsTests` | every constant equals the 2.11 text; the three em-dash strings contain U+2014 exactly once and no U+2013 (each string arrives with the WP of the window that shows it; WP-A13 has `AppName` and `AppNameMatchesTheElectronTitle`); added in WP-A13: `PixelRectTests` (exclusive edges, touching rectangles do not intersect); added in WP-A15: `MenuLabelsAreTheTableOf211`, `ImportProjectEndsWithOneEllipsisCharacter`, `MenuTsLabelsMatchTheElectronSource`, `AboutStringsMatchTheElectronSource`, `TheTaglineHasOneEmDashAndNoEnDash`; added in WP-B7: `PillStringsAreTheTableOf211` (with the Discard confirmation's buttons), `TheCountIsPlainDigits`, `PillStringsMatchTheElectronSource` (`main.ts` and `App.tsx`) and `ThePillsEmDashStringsHaveOneEmDashAndNoEnDash`; added in WP-B8: `OverlayStringsAreTheTableOf211`, `OverlayStringsMatchTheElectronSource` (`overlay.html` and `overlay/App.tsx`) and `TheOverlayTitleHasOneEmDashAndNoEnDash` |

**ShotAI.Platform.Tests (Windows only, new project):**

| Class | Cases |
|---|---|
| `SingleInstanceTests` | `SecondAcquireFails` (acquire in a child process started from the test, then `TryAcquire` in the test returns null); `ReleasedOnDispose`; `AbandonedCountsAsAcquired` (child exits without releasing); `SecondLaunchSignalsFirst` (child creates the message-only window; `ExistingInstance.Activate` returns true and the child observes the message). As built in WP-A13: the child is Windows PowerShell holding the named mutex; `SecondLaunchSignalsFirst` creates the message-only window in the test and reads the posted message with `PeekMessage` (the cross-process signal is `StartupOrderTests.SecondInstanceCreatesNoWindow` and `SecondLaunchTests` on the real exe); added `HeldUntilDisposed`, `NoRunningWindowReturnsFalse`, `AnOrdinaryWindowWithTheNameIsNotFound`, `RegisteredMessageIsTheSessionsId`. Each test uses a name of its own, never the app's |
| `WindowStylesTests`, `ForegroundTests`, `WindowShowHookTests` | added in WP-A13: the pill's and the overlays' styles and placement on a `STATIC` window; a minimized window restored; the hook reports a top-level window before it is visible, on every show, not a child, only on its thread, each destroy, and survives a throwing callback; added in WP-B7: `WindowStylesTests.NoActivateOnClickAnswersOnlyTheMouseActivation` |
| `MonitorQueriesTests` | `AllMatchesEnumDisplayMonitors`; `WorkAreaInsideBounds`; `ScaleFromEffectiveDpi`. Added in WP-A15: `ExactlyOnePrimaryAtTheOrigin`, `ForPointFindsEachMonitor`, `AnOffScreenPointGetsTheNearestMonitor`, `ForWindowFindsTheWindowsMonitor` (its scale is `GetDpiForWindow`'s), `CursorPositionIsOnAMonitor`; and `Export/WebView2RuntimeInfoTests` (`ReturnsTheInstalledVersion`, `RepeatedReadsAgree`) |
...
| Class | Cases |
|---|---|
| `AllWindowsRegisteredTests` | `EveryShownHwndIsRegistered` (main, pill, overlays, Discard dialog, About, a tooltip, a context menu, a combo-box drop-down, the `ChooseColor` dialog and the folder, open and save dialogs, Q-EDIT-21); `NoWin32MessageBoxInSource` (source scan of `ShotAI.App` for `MessageBox.Show` and `System.Windows.Forms`, allowlisting only spec 12's `LegacyInstanceGuard.cs` until stage S5, Q-SHELL-19 default and ARCHITECTURE 14.9). As built in WP-A13: a show probe (a test `WH_CALLWNDPROC` hook) records each window's display affinity at the `WM_WINDOWPOSCHANGED` that made it visible, and every case asserts `WDA_EXCLUDEFROMCAPTURE` there, so a registration after the show fails however soon; one case each for the main window, a tooltip, a context menu, a `ComboBox` drop-down, a `ShotAIPopup`, a plain `Popup` and a Win32 window of the UI thread, the sweep, `EveryWindowDerivesFromShotAIWindow`, and the scan's own failing cases; the pill, overlays, dialogs and system dialogs join with their WPs. Added in WP-B7: `PillIsExcludedBeforeItIsShown` and `DiscardConfirmationIsExcludedBeforeItIsShown`; added in WP-B8: `OverlaysAreExcludedBeforeTheyAreShown` |
| `StartupOrderTests` | `WindowsExcludedBeforeSettingApplied`; `SecondInstanceCreatesNoWindow` (as built in WP-A13: the test holds the user's lock and listens as the running instance; the real exe exits 0, signals it, logs the lock line once and neither the runtime nor the first-render line, and leaves `settings.json` and its temp folder untouched) |
| `SecondLaunchTests` | added in WP-A13, AC-SHELL-4 automated: `SurfacesTheMinimizedFirstInstanceAndExits` (two real instances; the second exits 0, the first's minimized window is restored, the lock line is logged once; the foreground stays with the manual check) |
| `ActivationListenerTests` | added in WP-A13: a signal from another thread runs the callback on the UI thread and logs its line; a disposed listener, or another user's, is not found; `Start` twice throws |
| `LifecycleTests` | `ClosingMainWindowShutsDown` (as built in WP-A13: the real exe, once its first render is logged, exits 0 on `WM_CLOSE` to its main window, the exit line last); `ExitCallsCaptureTeardown`; `SessionEndingCallsTeardown`; `PillCloseCancelledWhileRunning`; `MainWindowCloseClosesPillDespiteVeto` (EDGE-SHELL-51); `ExitOrderMatchesSpec11` (fake services record: Stopping cancelled, Teardown, flush, listener and lock disposed, container disposed, exit line). As built in WP-B7: `ExitCallsCaptureTeardown`, `SessionEndingCallsTeardown`, `PillCloseCancelledWhileRunning` and `MainWindowCloseClosesPillDespiteVeto` on the STA harness, and `ExitOrderMatchesSpec11` records the teardown second |
| `MainWindowTests` | `InitialSizeAndMinimums`; `TitleIsShotAI`; `SetDetailViewUsesPhysicalMath` (fake monitor at 150%); `ShowFromSecondInstanceRestoresMinimized` (with `ShowFromSecondInstanceKeepsMaximized`, `ShowFromSecondInstanceShowsAHiddenWindow` and `ShowFromSecondInstanceAfterCloseDoesNothing`, added in WP-A13); `InitialPlacementOnSecondaryMonitorAtOtherDpi` (EDGE-SHELL-48: final physical size equals `WindowLayout.Initial` times the target scale; as built in WP-A15 it skips, saying why, without a second monitor at another scale, as on the runners, so `InitialPlacementMovesOntoTheMonitorThenSetsItsRectangle` checks the two steps against a stood-in 150% monitor); added in WP-A15: `SetDetailViewLeavesAMaximizedWindow`, `SetDetailViewLeavesAMinimizedWindow`, `SetDetailViewWithoutAWindowDoesNothing`, `SetDetailViewLeavesAFullScreenWindow`, `FullScreenCoversTheMonitorAndRestores`, `FullScreenFromMaximizedRestoresMaximized`; and `AboutWindowTests` (`TitleTextsIconAndButton`, `ADialogOfTheMainWindow`, `TheMainWindowWearsTheAppIcon`) |
| `CapturePillWindowTests` | `HasToolWindowAndNoActivateStyles`; `ShowDoesNotActivate` (foreground HWND unchanged on the first show AND on a show after `Hide()`, Q-SHELL-21); `HasNoOwner` (EDGE-SHELL-46); `ClickDoesNotActivate` (synthesized click on Pause via `SendInput`, foreground unchanged, command invoked); `DragMovesWithoutActivating`; `DocksOnceThenKeepsPosition`; `DiscardCancelDoesNotCallService`; `DiscardConfirmCallsServiceOnce`; `ErrorTooltipShowsWhileInactive` (Q-SHELL-3). As built in WP-B7: 15 cases, also `AButtonDoesNotStartADrag`, `AnEndedSessionCancelsTheConfirmation`, `RendersTheSessionState`, `AStepFlashesTheRing`, `NoControlTakesTheFocus` and `ControlsCarryTheirTooltipsAndNames`; `HasNoOwner` as corrected in 7.6.2. The class runs alone in App.Tests, which holds a mutex of the session for its whole run against the Platform tests' input hook collection (`RealInputLock`, the same class in both test projects): `dotnet test` runs the two projects at once, and in WP-B7's runs their real input collided and the Platform tests' clicks closed `OverflowMenuTests`' popups. Its pill sits just above the taskbar, below the Platform tests' topmost windows; each test puts the cursor back where it was, and the drag waits for the pill to take each step. Each real-input step first checks that the pill is the window at its point: the windows-11-arm runner can have the shell's Start menu open, above every topmost window, and the step closes it with Escape first (`ShellOverlay`), as the Platform tests' click target does |
| `CapturePillViewModelTests` | added in WP-B7: `PauseAndResumeRunOffTheUiThread`, `StopDisablesTheControlsUntilTheNextState`, `AFailedActionIsLoggedAndIgnored`, `DiscardNeedsAConfirmation`, `TheConfirmationComesAfterTheClick`, `ChangesAreRaisedWithTheCommands`, `ArgumentsAreChecked` |
| `RecordingVisibilityTests` | added in WP-B7, on real windows: `RecordingChangedHidesAndShows` (INV-SHELL-7, INV-SHELL-9), `AScreenshotHidesTheMainWindowOnly`, `AMinimizedMainWindowComesBackRestored`, `ClosedWindowsAreSkipped` |
| `AreaOverlayTests` | `OneOverlayPerMonitorWithExactBounds`; `TransparentAreaReceivesMouse`; `OverlaySeededFromSetting`; `EscCancels`; `RightButtonCancels`; `SmallDragCancels`; `DragReturnsPhysicalRect`. As built in WP-B8: 17 cases, the seven with `OverlaySeededFromSetting` in three parts (remote visibility on relaxes a new overlay on the pool, possibly before it first shows; a grab keeps it excluded at its show and until the grab ends; off keeps it excluded), `EscCancels` on either overlay and during a drag, and `RightButtonCancels` at the release (D25); also `IsATransparentTopmostToolWindow`, `AnyButtonButTheLeftCancels` (3), `ALeftPressAfterAnotherButtonStartsNoDrag`, `AReleaseWithoutAPressCancels`, `ASecondLeftPressRestartsTheDrag`, `TheHintShowsUntilTheDragStarts`, `TheDragDimsOutsideAndShowsItsSize` and `EachOverlayDrawsOnlyItsOwnDrag`. The overlays stand in for monitors: `dotnet test` runs ShotAI.Platform.Tests at the same time, whose topmost windows fill the runners' screens down to 640 pixels and are probed by screen point, so no overlay covers a real monitor, and none is larger than the runners' 1024 by 768 screens, beyond which Windows cuts a window down. The real input (`TransparentAreaReceivesMouse`, `SmallDragCancels`, `DragReturnsPhysicalRect`) goes to an overlay on a 480 by 72 band above the primary monitor's taskbar, where the pill's tests click; the rest goes to overlays off every screen, through the overlay's own input methods. The class runs alone, in the real input collection with the pill's. Overlays on real monitors, and a DPI change, are AC-SHELL-15 and AC-SHELL-16 (WP-B9a) |
| `AreaSelectionServiceTests` | `NewSelectionResolvesPreviousNull`; `StaleOverlayCannotResolve`; `OverlayClosedResolvesNull`; `CancellationResolvesNull`; `RestoresMainWindowOnCancelAndError`; `EmptyMonitorListResolvesNull` (EDGE-SHELL-50); `SecondSelectionKeepsRequesterHidden` (EDGE-SHELL-47); `PreCancelledTokenDoesNotHide`; `CancellationRegistrationDisposed` (a token cancelled after the selection ended changes nothing). As built in WP-B8: 15 cases, over overlays off every screen: as listed (`OverlayClosedResolvesNull` closes one with `WM_CLOSE`, as Alt+F4 does; `CancellationResolvesNull` cancels on the pool; `RestoresMainWindowOnCancelAndError` also fails the monitor read and the second overlay; `CancellationRegistrationDisposed` counts the posts), and `AnotherRequestersSelectionBringsTheFirstBack`, `ARequesterClosedMeanwhileStaysClosed`, `AClosedRequesterIsRefused`, `OnlyTheUiThreadSelects`, `TheLinesAreElectrons` and `ArgumentsAreChecked` |
| `AppMenuViewModelTests` | `ItemsAndGestures` (the 7.4.5 table); `BrandChoiceCallsStoreForOpenProject`; `BrandChoiceIgnoredWithNoProject`; `BrandItemsNotifyOnlyOnChange`; `ImportAndSettingsIgnoredWhileRecording`; `CheckedIsNotToggledByWpf` (clicking a ticked row leaves the checked state to the model). As built in WP-A15: `ItemsAndGestures` (every item as header, gesture text and command, and every key binding), `HeadersAreTheShellStrings`, `ZoomStepsHalfALevelAndBack`, `ZoomScalesTheContentNotTheMenu`, `ImportAndSettingsRaiseTheirRequests`, `MinimizeMinimizes`, `CloseClosesTheMainWindow`, `ToggleFullScreenIsTheCommand`, `HelpAboutShowsTheAppInfo` (the modal dialog with `AboutText` of `IAppInfo.Current`, closed by its OK button); the Brand cases join in WP-A18 and `ImportAndSettingsIgnoredWhileRecording` in WP-D15; and `AllWindowsRegisteredTests.AboutIsExcludedBeforeItIsShown`. As built in WP-A18: the four Brand cases, and `BrandChoiceReadsTheOpenProjectWhenRun` (EDGE-SHELL-13), `BrandItemsFollowTheAppBrand`, `BrandStateIsLoggedOnChange` (L3), `EveryRowRunsTheMenusCommand`, `DisposeStopsFollowing`, `BrandSubmenuFollowsTheOpenProject` (AC-SHELL-20's automated half) and `ArgumentsAreChecked`; `ItemsAndGestures` ends View with the separator and `Brand`; `BrandItemsNotifyOnlyOnChange` re-raises the navigation state with nothing changed (AC-SHELL-21's automated half); `CheckedIsNotToggledByWpf` clicks rows generated from the window's own container style through UI Automation |
| `ShellEventOrderingTests` | `RecordingChangedBeforeStateChanged`; `NoSynchronousDispatcherInvoke` (source scan of `ShotAI.App` for `Dispatcher.Invoke(`). As built in WP-B7: `RecordingChangedBeforeStateChanged`, `AnEndRightAfterAStartEndsWithTheMainWindowShown`, `TheStateIsReadWhenThePostRuns`, `AScreenshotShowsNoPill`, `TheOffScreenCheckIsForAShowOnly`, `ADisposedControllerMovesNothing`, `StartIsOnceAndWaitsForTheWindows` and `ArgumentsAreChecked`, over a fake engine and recorded window actions; `NoSynchronousDispatcherInvoke` is `NoSyncWaitTests`' `Dispatcher.Invoke` rule, which already scans the App |
...
**AC-SHELL-29.** Code review: `ShotAI.Core/Shell` references no `System.Windows`, `Windows.Win32` or `Microsoft.Win32` namespace (the Core project builds for `net10.0`). As built in WP-B8: checked by `CoreReferencesTests.TheShellFolderNamesNoWindowsNamespace` as well.
...
**Q-SHELL-2. Pill corners.** Electron's pill is a plain rectangle; macOS rounds 10 pt. Recommended default: rectangle (parity); optionally `DWMWCP_ROUND` on Windows 11 later, never `AllowsTransparency` for the pill. Decided in WP-B7: the rectangle.

**Q-SHELL-3. Tooltips and popups in a non-activating window.** Whether WPF opens `ToolTip`s for a window that is never active, and whether a popup HWND can be visible for one frame before `Opened` registers it, are both unverified. Recommended default: build `CapturePillWindowTests.ErrorTooltipShowsWhileInactive` and `AllWindowsRegisteredTests` first; if tooltips do not open, replace them on the pill with a `ShotAIPopup` opened on `MouseEnter` after 400 ms; if the popup HWND is visible before registration, set the popup's affinity from a `HwndSource` hook on `WM_CREATE` (`HwndSource.AddHook` via `PresentationSource.AddSourceChangedHandler`), or hide the tooltip during grabs. A catch-all candidate that also covers templated popups (the `ComboBox` drop-down, 7.4.7): a thread-local `SetWindowsHookEx(WH_CALLWNDPROC, ..., null, uiThreadId)` that, for any top-level window of the UI thread receiving `WM_WINDOWPOSCHANGING` with `SWP_SHOWWINDOW` (or `WM_SHOWWINDOW` with `wParam = TRUE`), registers the HWND before it becomes visible; the hook runs synchronously before the window procedure. Its reliability for WPF's layered popup windows is UNVERIFIED; `AllWindowsRegisteredTests` decides. Decided in WP-A13 for the second half: on both Windows runners a tooltip's, a context menu's and a `ShotAIPopup`'s HWND had display affinity 0 at the `WM_WINDOWPOSCHANGED` that made it visible when it was registered from `Opened`, and a `ComboBox`'s drop-down was not registered at all, so the catch-all is adopted (7.4.7), and with it every popup of the UI thread was excluded before it was visible. The first half, whether WPF opens tooltips for the never-active pill, stays with `CapturePillWindowTests.ErrorTooltipShowsWhileInactive` (WP-B7). Decided in WP-B7 for the first half: on both runners WPF opens the error's tooltip on the never-active pill, so no `ShotAIPopup` replaces the pill's tooltips.

**Q-SHELL-4. `WS_EX_NOACTIVATE` and dragging.** The outline-drag and activation behavior of the system move loop for no-activate windows varies by Windows version. Recommended default: the manual drag in 7.4.3, which sidesteps the question; confirm live dragging with AC-SHELL-8. Decided in WP-B7: the manual drag, which on both runners moves the pill by the cursor's travel and never activates it (`DragMovesWithoutActivating`), measured from the press's own position and following every `WM_MOUSEMOVE` (7.4.3 as built); AC-SHELL-8 stays with WP-B9a.

**Q-SHELL-5. Chromium's exact `DIPToScreenRect` rounding.** The spec assumes origin floored and size ceiled after Electron's own `Math.round` of the DIP rect. Recommended default: implement as written; on a 125% and a 150% monitor compare the Electron log line `region selected:` with the native one for the same drag endpoints and adjust `ToPhysical` if they differ. As built in WP-B8: as written, in double precision. Chromium's `gfx` geometry is single precision (UNVERIFIED here); at 125%, 150% and every other multiple of 25% both products are exact, so only a custom scaling could tell them apart, and M-B's 125% and 150% cannot. The comparison stays with M-B.

**Q-SHELL-6. Clamp cross-monitor drags?** Electron and macOS allow a drag to leave the starting overlay; the capture then clips. Recommended default: parity (no clamp); revisit if users report confusing partial rectangles. Decided in WP-B8: parity; a drag past the overlay keeps its coordinates (`ADragPastTheOverlaysEdgeIsNotClamped`).

**Q-SHELL-7. Discard dialog buttons.** Parity would be `OK` and `Cancel` with OK as default. Recommended default: `Discard` and `Cancel`, Cancel default (D8). Decided in WP-B7: the default.
...
**Q-SHELL-11. Full-screen behavior details.** Electron's F11 on Windows may hide the menu bar and cover the taskbar; unverified. Recommended default: cover the monitor (taskbar included), keep the menu visible, restore exactly on the second F11. Decided in WP-A15: the default. The window covers its monitor's `rcMonitor`, taskbar included, without its frame, and keeps the menu; the second toggle restores the frame and the rectangle, and a window maximized before is maximized again (`MainWindowTests.FullScreenCoversTheMonitorAndRestores`, `FullScreenFromMaximizedRestoresMaximized`).

**Q-SHELL-12. UI zoom persistence and scope.** Chromium may persist the zoom level per origin across launches; unverified for `file:` pages. Recommended default: do not persist; zoom applies to the main window's content only (not the pill, overlays or dialogs). If the pilot misses persistence, add a `uiZoomLevel` setting (spec 10). Decided in WP-A15: the default. The level lives in `AppMenuViewModel` for the run, and the `LayoutTransform` scales the main window's content only, not its menu bar (`AppMenuViewModelTests.ZoomScalesTheContentNotTheMenu`).
...
**Q-SHELL-14. Initial monitor.** Electron's default centering monitor is not verified (it may be the primary display rather than the cursor's). Recommended default: the cursor's monitor (WPF `CenterScreen` semantics, documented by Microsoft), since that is where the user just launched the app. Decided in WP-A15: the cursor's monitor, and the primary monitor when Windows gives no cursor position.

**Q-SHELL-15. Remember window size and position?** Neither Electron nor native persists it; macOS persists it and needed workarounds (#56). Recommended default: do not persist. Decided in WP-A15: not persisted.
...
**Risk R2 (medium).** WPF focus or activation logic re-activating the pill through an unexpected path (a tooltip, a `Focus()` call in a control template, a future style change). Mitigated by `Focusable=False`, `MA_NOACTIVATE` and the foreground assertions in `CapturePillWindowTests`. Outcome in WP-B7: on both runners no show, click, drag or tooltip activated the pill.

**Risk R3 (medium).** Mixed-DPI positioning errors (overlay not exactly on its monitor, pill docked on the wrong monitor, resize off by one DPI factor). Mitigated by doing monitor-targeted positioning in physical px (7.7) and AC-SHELL-15 and AC-SHELL-16 on a 100% plus 150% rig. Outcome in WP-B8: the overlays land exactly on stood-in monitors on both runners; the runners have one monitor at 100%, so the mixed-DPI half stays with AC-SHELL-15 and AC-SHELL-16.
