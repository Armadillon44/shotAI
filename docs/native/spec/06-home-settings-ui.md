...
- Two `DispatcherTimer`s at `Background` priority, the 20 s tick and the 1 minute regroup (`AutoRefreshPolicy.RegroupInterval`, D-HOME-25), both started by `OnEnter` and stopped by `OnLeave`. Until WP-A19 the tick passes `false` for renaming, selection and busy.
...
- Moved: `OpenFailedShowsNotice` to WP-A17; the selection, rename, busy and bulk cases of `HomeViewModelTests` to WP-A19; `RecordingHidesViews` and `MenuRequestsIgnoredWhileRecording` to WP-B9; `ReplayTourClosesSettingsAndProject` to the tour (WP-B10); "Project and Settings start at 0" to WP-A17 and WP-B10; "update shown once per launch" to WP-E1.
...
**Q-HOME-3. Keep the Home list state across navigation?** Electron resets tab, sort, search and selection on every return (EDGE-HOME-3), which is surprising when returning to a search. Recommended default: parity for 2.0.0; revisit after the pilot. Either way, an open rename is committed (not dropped) when leaving Home. Decided in WP-A16: parity, `OnEnter` resets tab, sort and search on every entry (the rename commit joins with the rename in WP-A19).
