...
`ICaptureService` is exactly 02 7.2. This spec adds only the subscriber rules of 7.7 and fixes that `CaptureScreenshotAsync` throws a `ShotAIException` (02's `CaptureException`) with the message `A screenshot needs an explicit target (screen, window, or area).` for a null or `Auto` target (EDGE-IPC-16), and that `StartAsync` clamps a negative `InsertAt` to 0 (EDGE-IPC-17).
...
09's `IShellReveal.RevealInExplorer(path)` and `OpenFolder(dir)` are `RevealInExplorerAsync` and `OpenFolderAsync`; 06's `reveal.RevealInExplorer(path)` for a project row is `RevealProjectAsync(path)` (P8, with the known-project gate) (section 10).
...
| `Composition.ViewModelDependencyTests` | reflection over view model constructors: parameters are catalog interfaces (7.3 and ARCHITECTURE 4.4, including `ICaptureTargetSelection`, R-ARCH-26), 06's chrome services, the factories of ARCHITECTURE 4.1 C6, `ILogger<T>`, value types or other view models; never `ProjectStore`, `EntraSession`, `IApiKeyStore`, `MsalGateway`, `CaptureEngine`, `SettingsService` (concrete) or any Platform type (INV-ARCH-3) |
...
| 07 SOP generation | `IClaudeService` (`EstimateAsync`, `GenerateAsync`, `Cancel`), `ApplySopPlanOperation`, `RevertSopOperation`, `SopProgress`, `SopEstimate`, `IProjectSettle` | REQUEST: `claude:test-connection` maps to 08's `IAuthService.TestConnectionAsync`; `IClaudeService.TestConnectionAsync` is removed and 07's models-retrieve leg (leg 3) sits behind an internal seam (R-ARCH-3); 07's `IAuthService` sketch (7.11) is superseded by 08's, and `VerifyFederationAsync` and `IApiKeyStore` are internal to Core (R-ARCH-2); `SopPanelViewModel` depends on `IProjectSession`, `IClaudeService`, `IAuthService`, `ISettingsService`, `IStepFlattener`, `IConfirmService` only (R-ARCH-16); Anthropic clients come only from 08's `IAnthropicClientFactory` (R-ARCH-15); egress waits with `IProjectSettle.WhenSettledAsync` (R-ARCH-6) |
...
| 09 Export | `IExportService` (7.13), `ExportResult`, `PackageResult`, `ExportProgress`, `ExportFormat`, `WebView2PdfRenderer` hardening | REQUESTS: `IProjectStore` is renamed `IProjectService` (R-ARCH-4); 09's requested `IProjectService.WhenIdleAsync(path)` is `IProjectSettle.WhenSettledAsync(path)` over the session's `WhenIdleAsync` (R-ARCH-6); `IRenderGate` injected and `RenderGateException` caught, never `RenderRefusedException` (R-ARCH-8); `IShellReveal.RevealInExplorerAsync` and `OpenFolderAsync` (async names); export reads `ISettingsService.Current.Brand` at call time; `ExportException` derives from `ShotAIException`; the WebView2 host stays the only web engine (INV-IPC-20), with its user data folder under `IAppPaths.LocalDataDirectory` (`%LOCALAPPDATA%\LFI\shotAI\WebView2`, R-ARCH-13) |
