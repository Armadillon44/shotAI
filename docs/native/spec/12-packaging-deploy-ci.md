...
| Runtime check (new) | WiX NetFx `DotNetCompatibilityCheck`, `RuntimeType="desktop"`, `Version="10.0.0"`, `RollForward="Minor"`, property `SHOTAI_DESKTOPRUNTIME` (NetCoreCheck exit code, `0` means found) | | first per-user installs only: the extension schedules it in every transaction, so `Package.wxs` overrides that (INV-PKG-37) | 7.4.4 |
...
**INV-PKG-36. Every managed deployment is per-machine: the Intune Win32 app's install command passes `ALLUSERS=1`, its install behavior is System, and there is exactly one shotAI Intune app per architecture (no User-context app that installs shotAI; the removal app of 7.13.5 only uninstalls).** Why: without the flag, a System-context install of this package is a per-user install for the SYSTEM account, into that account's own profile, where no signed-in user sees it (EDGE-PKG-61); and Intune fixes a dual-mode Win32 app's context for every assignment of that app ("the admin must choose if the app will install as a User Mode or Machine Mode app for all assignments associated with that instance. The deployment context can't be changed per assignment", Microsoft Learn, Windows app deployment), so a second context would mean a second app entry per architecture to keep in step. Company Portal "Available" installs also run as SYSTEM, so standard users on managed PCs still install without admin rights. New 2026-09-23. Citation: 7.5.2. Test: `WorkflowContractTests.SmokePerMachineLegPassesAllUsers1` and `.IntuneNotesInstallCommandPassesAllUsers1`; AC-PKG-25.

**INV-PKG-37. A first per-user install is refused, with an exact message and before anything is written, when (a) a per-machine copy of another build is installed (`HKLM\SOFTWARE\LFI\shotAI` value `InstallFolder` exists) or (b) the .NET 10 Desktop Runtime for the package's architecture is missing (`SHOTAI_DESKTOPRUNTIME` is not `0`); a per-machine install is refused by neither, and no per-machine install, repair or removal runs the runtime check.** Why: (a) the two scopes can coexist and are upgraded separately (EDGE-PKG-62), so a personal copy beside IT's copy would be a second shotAI that IT never updates; (b) a per-user install succeeds without the runtime and then cannot start, and installing the runtime needs an administrator (7.5.1), so the message tells the user what to ask IT for. A per-machine install stays unblocked on the runtime so that an Intune dependency retry cannot wedge it (EDGE-PKG-42), and a personal copy never blocks it, because IT must always be able to deploy (INV-PKG-38 covers that order). The runtime check runs `NetCoreCheck.exe` from the temporary folder and fails the transaction when that cannot run (`Return="check"`), and the WiX extension schedules it in every transaction, so `Package.wxs` overrides its scheduling with `SHOTAI_PERUSER AND NOT Installed` (7.4.4); otherwise application control that blocks the temporary folder would also stop IT's installs and every removal (EDGE-PKG-66). (a) names another build because a per-user run of the MSI whose own ProductCode is installed per-machine is maintenance of that copy, which every `Installed OR` term lets through (EDGE-PKG-67). New 2026-09-23. Citation: 7.4.4. Test: `PackageSourceTests.PerUserLaunchConditionsExact` (both conditions and messages exact, the `SHOTAI_PERUSER` rule exact, the `DotNetCompatibilityCheck` attributes exact, the scheduling override exact in both sequences); AC-PKG-35 (the guard leg with the probe build, and the verbose logs showing the per-machine install and uninstall skip the runtime check), AC-PKG-37.
...
**EDGE-PKG-62. The two scopes coexist, and each upgrades only itself.** Windows Installer permits "one instance of a product code to be installed per context", "only one instance ... in the machine context and only one instance ... in each user context" (Microsoft Learn, "Installing Multiple Instances of Products and Patches"), and "Applications that have been installed per-user therefore receive all updates or repairs on a per-user basis and applications installed per-machine receive updates or repairs on a per-machine basis" (`MSIINSTALLPERUSER`). A major upgrade stays in its scope: "The Windows Installer will not install major upgrades across installation context" (Microsoft Learn, Major Upgrades); the smoke's scope leg measures it (AC-PKG-35). A user who installed a personal copy and later receives IT's per-machine copy then has two Start menu entries named `shotAI`, two Installed apps entries, and a personal copy that IT's updates never reach. Required: INV-PKG-37 (a) stops a new personal copy beside a per-machine one; INV-PKG-38 makes an existing personal copy open the per-machine one; 7.13.5 lets IT remove personal copies. Both copies use the same settings, logs, projects and single-instance mutex, so none of this can lose data or run two instances for one user. New.
...
**EDGE-PKG-66. Application control can block a personal copy.** AppLocker's default executable rules allow `BUILTIN\Administrators` every path and `Everyone` only `%windir%\*` and `%programfiles%\*` (Microsoft Learn, "Executable rules in AppLocker"), so `%LOCALAPPDATA%\Programs\shotAI\shotAI.exe` is blocked for a standard user; the per-user runtime check also runs `NetCoreCheck.exe` from the user's temporary folder (WiX `netfxca.cpp`), which the same rules block, and a failed check fails the install. App Control for Business with Intune as the managed installer trusts files that Intune installed; a user's own install carries no managed-installer tag (Microsoft Learn, "Automatically allow apps deployed by a managed installer"). IT can also forbid every per-user MSI on a device with the Windows Installer machine policy `DisableUserInstalls` = 1, after which "An attempt to perform a per-user installation causes the installer to display an error message and stops the installation" (Microsoft Learn, Machine Policies); with the policy set, "Calls to the Windows Installer application programming interface or system ignore per-user applications" (Microsoft Learn, DisableUserInstalls), so personal copies installed earlier can no longer be found or removed through Windows Installer. Required: the Intune notes state these facts, including that IT removes personal copies (7.13.5) before setting `DisableUserInstalls`; where application control is enforced, personal copies need a publisher rule for the signing identity (INV-PKG-7, Q-PKG-2), or those PCs stay per-machine only; Q-PKG-34 records which applies at LFI. New.

**EDGE-PKG-67. The same build installed in the other scope turns an install into maintenance of that copy.** `Installed` "is set only if the product is installed per-machine or for the current user" (Microsoft Learn, Installed property), and without `DisableUserInstalls` "the installer searches the registry for applications in the following order: managed applications registered as per-user, unmanaged applications registered as per-user, and finally applications registered as per-machine" (Microsoft Learn, DisableUserInstalls). So while a build's ProductCode is installed per-machine, a per-user run of that same MSI is a maintenance run of IT's copy: every `Installed OR` term passes, nothing is refused, and `msiexec /qn` exits 0. The reverse holds too: an account that has a personal copy of a build and runs the same build with `ALLUSERS=1` only maintains its personal copy. Intune is not affected (SYSTEM has no personal copy, INV-PKG-36), and every build has its own ProductCode (7.4.2), so the case needs the identical MSI in both scopes: the smoke, or one person running one file twice. Required: the smoke's guard and scope legs install the probe build, a second build of the same sources (7.11.2, 7.11.4); INV-PKG-37 (a) and 7.4.5 rule 2 say which builds they cover. New.
...
<Wix xmlns="http://wixtoolset.org/schemas/v4/wxs"
  ...
  <Package Name="shotAI" Manufacturer="LFI" Version="$(ProductVersion)"
    ...
    <!-- The extension schedules the check in every transaction; run it for a first per-user install only (7.4.4). -->
    <InstallExecuteSequence>
      <Custom Action="override Wix4NetFxDotNetCompatibilityCheck_$(NetFxSuffix)" After="SetSHOTAI_PERUSER"
              Condition="SHOTAI_PERUSER AND NOT Installed" />
    </InstallExecuteSequence>
    <InstallUISequence>
      <Custom Action="override Wix4NetFxDotNetCompatibilityCheck_$(NetFxSuffix)" After="SetSHOTAI_PERUSER"
              Condition="SHOTAI_PERUSER AND NOT Installed" />
...
| No `ProductCode` attribute on `Package`: WiX then generates one for every build (`Compiler_Package.cs` defaults it to `*`), so each release is a major upgrade and the smoke's probe build differs from the MSI it tests | REQUIRED (INV-PKG-12, EDGE-PKG-67) |
...
| `SetProperty` `SHOTAI_PERUSER` with exactly the condition above, `Before="LaunchConditions"`, `Sequence="both"`; the two per-user `Launch` elements with exactly the conditions and messages above; `netfx:DotNetCompatibilityCheck` with `Property="SHOTAI_DESKTOPRUNTIME"`, `RuntimeType="desktop"`, `Platform="$(RuntimePlatform)"`, `Version="10.0.0"`, `RollForward="Minor"`; in both `InstallExecuteSequence` and `InstallUISequence`, `Custom Action="override Wix4NetFxDotNetCompatibilityCheck_$(NetFxSuffix)"` with `After="SetSHOTAI_PERUSER"` and `Condition="SHOTAI_PERUSER AND NOT Installed"` | IMPROVEMENT (INV-PKG-37) |
...
1. **A per-machine copy is present.** `SHOTAI_MACHINECOPY` is the `HKLM` (64-bit view) `InstallFolder` value, found by `AppSearch`. It covers other builds only: while this build's own ProductCode is installed per-machine, a per-user run is maintenance of that copy, and `Installed OR` lets it through (EDGE-PKG-67). Message (user-visible, exact): `shotAI is already installed for all users of this PC, so you don't need your own copy. IT or an administrator keeps it up to date.`
2. **The Desktop Runtime is missing.** WiX's `DotNetCompatibilityCheck` is an immediate custom action (`Return="check"`) that the extension schedules before `LaunchConditions` in both sequences, with no condition, as a `virtual` action (`NetFxExtension_Platform.wxi`); it extracts `NetCoreCheck.exe` for the package platform to a temporary folder, runs it with `-n <runtime type> -v 10.0.0 -r Minor`, stores the exit code in `SHOTAI_DESKTOPRUNTIME` and deletes the file (`netfxca.cpp`). Run in every transaction, it would fail per-machine installs, repairs and removals wherever `NetCoreCheck.exe` cannot run (EDGE-PKG-66), so `Package.wxs` overrides its scheduling in both sequences: after `SetSHOTAI_PERUSER`, with `Condition="SHOTAI_PERUSER AND NOT Installed"` (7.4.2). WiX numbers an action scheduled after one that is scheduled before `LaunchConditions` into the same gap, so the check still runs before the launch conditions (`SequenceActionsCommand.cs`). The action id carries the package platform's suffix, `Wix4NetFxDotNetCompatibilityCheck_X64` or `_A64` (the extension references it for the build platform, `NetFxCompiler.cs`). That WiX accepts this `override` is UNVERIFIED until the first `package` build; the smoke's verbose logs show which transactions ran the check (7.11.4). `0` means a compatible runtime is installed. A missing property is an empty string, which never equals `0` (the "Conditional Statement Syntax" rule above), so a check that did not run blocks a per-user install (fail closed), and a check that fails to run fails the install through `Return="check"`. Message (user-visible, exact): `shotAI needs the .NET 10 Desktop Runtime, and installing it needs an administrator. Ask IT to install it, then run this installer again.`

A double-click install shows a failed condition's message in a message box; with `/qn` it goes to the log and `msiexec` exits 1603 (AC-PKG-35). Both conditions start with `Installed OR`, so repair and removal of an existing copy always work, and neither runs the runtime check. A per-machine install passes both and never runs the check, because `SHOTAI_PERUSER` is not set (EDGE-PKG-42 still applies to it).
...
2. **The scopes can coexist on one PC** (EDGE-PKG-62). A per-user install refuses when a per-machine copy of another build exists (INV-PKG-37); the identical MSI run for the other scope only maintains the copy it installed there (EDGE-PKG-67), which never happens through Intune. A per-machine install never refuses, because IT must be able to deploy over whatever users installed; an existing personal copy then hands off to it at launch (INV-PKG-38, 7.10.4), and IT removes personal copies with 7.13.5.
...
{ /* new ProcessStartInfo(exePath) { UseShellExecute = false, WorkingDirectory = the exe's folder },
     each argument added to ArgumentList (no string concatenation); then AllowSetForegroundWindow(process.Id),
     result ignored (see below); the returned Process disposed */ }
...
There is no dialog: apart from a second process start, the hand-off is invisible. The personal copy was started by the foreground process (the Start menu or Explorer), which lets it set the foreground window; the copy it starts was not, so `ProcessStarter` passes that right on with `AllowSetForegroundWindow(<started pid>)` (Microsoft Learn, SetForegroundWindow and AllowSetForegroundWindow; the function is already in `NativeMethods.txt` for 03 7.3). Without it the per-machine window could open behind the current one, or only flash on the taskbar (AC-PKG-38 checks it). It runs before the mutex because a process holding the mutex would make the started copy find it held, activate this exiting process and quit, so nothing would open. When the per-machine copy is already running, the started copy finds the mutex held and activates that window, the normal second-launch path (03 7.4.8). IMPROVEMENT (INV-PKG-38, D-PKG-13).
...
jobs:
  ...
  package:
    ...
    steps:
      ...
      - build it again into artifacts/msi-probe/win-${{ matrix.arch }}: the probe build, the same sources and properties with its own ProductCode (7.4.2), used only by the smoke and never uploaded (EDGE-PKG-67)
      - smoke (PowerShell, 7.11.4: per-machine leg, guard leg, per-user leg, scope leg)
...
| `sign` | windows-latest | `contents: read`, `id-token: write` | `release` (required reviewers) | `azure/login` OIDC; sign pass 1 on both payloads (and the internal payloads if present); verify; build MSIs with the WiX project, plus the unsigned probe builds for the smoke (7.11.2), kept only in the workflow artifact `msi-probe`; sign pass 2; verify (7.6); install smoke on x64 (7.11.4); compute `SHA256SUMS.txt`; zip PDBs into `shotAI-<v>-symbols.zip` |
| `smoke-arm64` | windows-11-arm | `contents: read` | | download the signed ARM64 MSI and the ARM64 probe build from `msi-probe`, smoke (7.11.4) |
...
function Arp($hive) {
  ...
  if (-not (Test-Path $key)) { return @() }        # a fresh profile can have no HKCU Uninstall key
  @(Get-ChildItem $key | Get-ItemProperty | Where-Object DisplayName -eq 'shotAI')
}
function InLog($log, $text) { Select-String -Path "$env:RUNNER_TEMP\$log" -SimpleMatch -Quiet -Pattern $text }
...
$msi = <path>
$probe = <path>        # the probe build (7.11.2): the same sources with its own ProductCode (EDGE-PKG-67)
$ca = 'Wix4NetFxDotNetCompatibilityCheck_' + $(if ('<arch>' -eq 'arm64') { 'A64' } else { 'X64' })   # 7.4.4
...
Assert (InLog 'machine.log' "Skipping action: $ca (condition is false)") 'the per-machine install ran the runtime check'
...
# Leg 2: a per-user install of another build beside the per-machine copy is refused (INV-PKG-37 a).
# It installs $probe: $msi itself would only run maintenance on the per-machine copy (EDGE-PKG-67).
Assert ((Msi "/i `"$probe`" /qn /norestart /l*v `"$env:RUNNER_TEMP\guard.log`"") -eq 1603) 'per-user install beside a per-machine copy was not refused'
Assert (InLog 'guard.log' "shotAI is already installed for all users of this PC, so you don't need your own copy. IT or an administrator keeps it up to date.") 'guard message missing from the log'
Assert (-not (Test-Path $userDir)) 'the refused per-user install left a folder'
Assert (@(Arp 'HKCU').Count -eq 0) 'the refused per-user install left an ARP entry'

Assert ((Msi "/x `"$msi`" /qn /norestart /l*v `"$env:RUNNER_TEMP\machine-x.log`"") -eq 0) 'per-machine uninstall exit'
Assert (InLog 'machine-x.log' "Skipping action: $ca (condition is false)") 'the per-machine uninstall ran the runtime check'
...
Assert (InLog 'user.log' "Doing action: $ca") 'the per-user install skipped the runtime check'
...
Assert (-not (Test-Path 'HKCU:\SOFTWARE\LFI\shotAI')) 'HKCU marker left behind'

# Leg 4: the scopes coexist; a per-machine install never removes a personal copy (EDGE-PKG-62, 7.4.5 rule 2)
Assert ((Msi "/i `"$probe`" /qn /norestart") -eq 0) 'per-user probe install exit'
Assert ((Msi "/i `"$msi`" ALLUSERS=1 /qn /norestart") -eq 0) 'per-machine install beside a personal copy exit'
Assert (@(Arp 'HKCU').Count -eq 1 -and @(Arp 'HKLM').Count -eq 1) 'expected one personal and one per-machine ARP entry'
Assert ((Msi "/x `"$msi`" /qn /norestart") -eq 0) 'per-machine uninstall exit (scope leg)'
Assert ((Msi "/x `"$probe`" /qn /norestart") -eq 0) 'per-user probe uninstall exit'
Assert (-not (Test-Path $machineDir) -and -not (Test-Path $userDir)) 'the scope leg left an install folder'
...
The runtime installer is downloaded from the official .NET release metadata with its published SHA-512 checked before it runs (the same rule as INV-PKG-19 for any fetched binary). The runner account is an administrator, so leg 3 proves the per-user layout, not the absence of a UAC prompt for a standard user (AC-PKG-36 does that by hand). Where Windows Installer registers a per-user product's Installed apps entry is UNVERIFIED (`HKCU` Uninstall is assumed above); if the first run shows otherwise, the legs assert through the Windows Installer API (`WindowsInstaller.Installer` `ProductsEx` with the per-user context) instead, and the check is never dropped. `Skipping action: <action> (condition is false)` and `Doing action: <action>` are Windows Installer's verbose-log wording, UNVERIFIED for this exact form until the first run; if the log words it differently, the checks match the action name with the log's own wording, and they are never dropped.

The release smoke also runs the upgrade checks, once per scope: per-machine (`ALLUSERS=1` on every command), then per-user (no scope property). Each pass starts with no shotAI installed, installs the previous `v2.*` release MSI of the same architecture (downloaded with `gh release download`), then the new one; exactly one `shotAI` Installed apps entry remains in that scope with the new DisplayVersion; it then attempts the old MSI again and expects exit code 1603 with the downgrade message in the log; last, it uninstalls the new copy (exit 0) and checks that the install folder is gone, because a per-machine copy left in place would make INV-PKG-37 (a) refuse the per-user pass. With no earlier `v2.*` release (the first one), the upgrade checks are skipped with a notice in the job log. The scope check of EDGE-PKG-62 is leg 4, in every run.
...
8. The Intune notes shipped to IT include the prerequisites of 7.5.1, EDGE-PKG-49, EDGE-PKG-51, the `ALLUSERS=1` rule (INV-PKG-36, EDGE-PKG-61), application control and `DisableUserInstalls`, with personal copies removed (7.13.5) before the policy is set (EDGE-PKG-66), and the personal-copy removal (7.13.5).
...
```powershell
# remove-shotai-personal-copy.ps1 (user context). Run it before DisableUserInstalls is set (EDGE-PKG-66).
$ErrorActionPreference = 'Stop'
$upgradeCode = '{PINNED-UPPERCASE-GUID}'   # the Package.wxs UpgradeCode (INV-PKG-12); public, not a secret
$installer = New-Object -ComObject WindowsInstaller.Installer
$failures = @(); $restart = $false
foreach ($code in @($installer.RelatedProducts($upgradeCode))) {
  if ($installer.ProductInfo($code, 'AssignmentType') -ne '0') { continue }   # 0 = per-user, 1 = per-machine
  $exit = (Start-Process msiexec.exe -ArgumentList "/x $code /qn /norestart" -Wait -PassThru).ExitCode
  if ($exit -eq 3010) { $restart = $true }                                    # removed; files go at restart
  elseif ($exit -notin 0, 1605) { $failures += "msiexec /x $code exited $exit" }   # 1605: already gone
}
if (-not $restart -and (Test-Path "$env:LOCALAPPDATA\Programs\shotAI\shotAI.exe")) { $failures += 'the personal copy is still installed' }
if ($failures.Count -gt 0) { $failures | ForEach-Object { Write-Output "remove-shotai-personal-copy: $_" }; exit 1 }
exit 0
```

`RelatedProducts` enumerates "the set of all products installed or advertised for the current user and machine" with that UpgradeCode (Microsoft Learn, `Installer.RelatedProducts`), and `AssignmentType` "Equals 0 (zero) if the product is advertised or installed per-user" (`MsiGetProductInfo`, `INSTALLPROPERTY_ASSIGNMENTTYPE`), so the script never touches the per-machine copy. That PowerShell accepts both parameterized COM properties in this call form is UNVERIFIED (AC-PKG-38 runs the script). Exit code 1, with the reasons on standard output, reports a failed removal to Intune; `3010` counts as removed, because the remaining files go at the next restart. Run it before `DisableUserInstalls` is set on a device: with the policy set, "Calls to the Windows Installer application programming interface or system ignore per-user applications" (Microsoft Learn, DisableUserInstalls), so `RelatedProducts` returns no personal copy and none can be removed through Windows Installer until the policy is lifted (EDGE-PKG-66). The Win32-app form's detection rule: file `%LOCALAPPDATA%\Programs\shotAI\shotAI.exe` exists. Removing a personal copy never touches user data (INV-PKG-2). `PackageSourceTests.UpgradeCodePinned` also asserts that the script's `$upgradeCode` equals the pinned UpgradeCode.
...
| Class | Cases |
|---|---|
| `Startup.DllSearchTests` | `CurrentDirectoryNotSearched`: after `SetDefaultDllDirectories`, `LoadLibraryEx("shotai_test_probe.dll")` with the DLL only in the current directory fails with `ERROR_MOD_NOT_FOUND`. Added in WP-A12: the probe is a 4-byte `MZ` file, which the default search finds in the current folder before the call and fails to load (so the test proves the folder was searched), and which its full path still reaches after it; a second `Apply` succeeds too |
...
| Class | Cases |
|---|---|
| `Startup.PersonalCopyGuardTests` | with fakes: `PerUser` plus a `MachineCopyExe` starts that exe with the process arguments unchanged and returns true; `PerMachine`, `Unpackaged`, or a null `MachineCopyExe` start nothing and return false; a self-test mode starts nothing, logs the exact line and returns false; a throwing starter logs the exact warning and returns false; the call order (step 1b, before the lock of step 2) is 03 7.4.1's and is checked by AC-PKG-38, not by a fake |
...
**AC-PKG-35.** The `package` job on x64 and ARM64: the MSI build's ICE validation shows ICE105 ran with no error (EDGE-PKG-64), and the smoke passes all four legs of 7.11.4: the per-machine leg with `ALLUSERS=1`, whose install and uninstall logs show the runtime check skipped; the refused per-user install of the probe build beside it (exit 1603, the exact INV-PKG-37 (a) message in the log, no folder or Installed apps entry left); the per-user leg (the runtime check run, files under `%LOCALAPPDATA%\Programs\shotAI`, one Installed apps entry for the user and none for the machine, `HKCU` `InstallFolder`, the user's Start menu shortcut, `--selftest` PASS, a clean uninstall that also removes the `HKCU` values); and the scope leg (a per-machine install beside the probe's personal copy leaves both Installed apps entries, EDGE-PKG-62).
