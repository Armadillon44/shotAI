...
**INV-PKG-2. The installer never creates, modifies or deletes per-user data: `%APPDATA%\shotAI\` (settings, logs, Electron files), the projects folder, and `%LOCALAPPDATA%` data folders survive install, upgrade, repair and uninstall.** Why: the Electron and native builds share them (`src/main/settings.ts:125-131`); uninstall must never cost a user their projects. Citation: 2.6, 7.10. Test: `PackageSourceTests.NoUserProfileLocations` (no `AppDataFolder`, `LocalAppDataFolder`, `PersonalFolder`, `RemoveFolderEx` or `util:RemoveFolderEx` in `Package.wxs`), AC-PKG-9.
...
**INV-PKG-24. The native app never reads, writes or deletes Electron-owned files (`secrets.json`, `entra-cache.bin`, `tessdata\`, the Chromium profile entries in `%APPDATA%\shotAI`) or anything under the Squirrel root `%LocalAppData%\shotai\`, and keeps none of its own data under that root.** Why: rollback needs Electron's files intact (08 EDGE-AUTH-33); Squirrel's uninstall deletes its whole root, which is the same folder as `%LOCALAPPDATA%\shotAI` on a case-insensitive file system (EDGE-PKG-22). Citation: 2.6, 7.10. Test: `AppPathsTests.NoPathUnderSquirrelRoot` (every `IAppPaths` local path is outside `%LOCALAPPDATA%\shotai`, compared `OrdinalIgnoreCase` after `Path.GetFullPath`); AC-PKG-24.
...
**EDGE-PKG-22. `%LOCALAPPDATA%\shotAI` is the Squirrel install root.** Squirrel installs to `%LocalAppData%\shotai`, and Squirrel's uninstall removes that whole folder. Specs 08 (MSAL cache `%LOCALAPPDATA%\shotAI\entra\`) and 09 (WebView2 user data `%LOCALAPPDATA%\shotAI\WebView2`) currently place native data there, so removing Electron at cutover would silently delete the native sign-in cache and WebView2 profile, and a Squirrel repair could too. Required: native local data under a folder that is not the Squirrel root (Q-PKG-4, recommended `%LOCALAPPDATA%\LFI\shotAI\`); INV-PKG-24. New.
...
**EDGE-PKG-28. WebView2's default user data folder sits next to the exe**, which is read-only under `Program Files`, so environment creation fails in a per-machine install. Required: explicit user data folder (09 EDGE-EXP-40), placed per Q-PKG-4. 09.

**EDGE-PKG-29. An upgrade while shotAI is running finds `shotAI.exe` in use.** With `/qn`, Windows Installer's Restart Manager asks applications to close; a refusal leaves files for replacement at reboot (3010). Required: the app handles `WM_QUERYENDSESSION` and `WM_ENDSESSION` by flushing its write queue and exiting (03 session end), and Intune "Device restart behavior" is "Determine behavior based on return codes". New; verify (AC-PKG-8).
...
| `Fonts\Archivo.ttf`, `Fonts\OFL.txt` (and `Fonts\static\*.ttf` plus `OFL.txt` if 06 ships static faces as loose files) | `src/renderer/fonts/` until cutover, then moved under `dotnet/` | 10 INV-INFRA-31 |
...
Inherited `Program Files` ACL; the app writes only to `IAppPaths` locations (10) and never to `AppContext.BaseDirectory` (INV-PKG-15). `FontPackaging` and `AppPaths.BrandFontPath()` read from `AppContext.BaseDirectory\Fonts\` (03, 09).
...
| MSAL cache | recommended `%LOCALAPPDATA%\LFI\shotAI\entra\msal-cache.bin` (08 currently `%LOCALAPPDATA%\shotAI\entra\`, Q-PKG-4) | no | 08 | IMPROVEMENT |
| WebView2 user data | recommended `%LOCALAPPDATA%\LFI\shotAI\WebView2` (09 currently `%LOCALAPPDATA%\shotAI\WebView2`, Q-PKG-4) | no | 09 | IMPROVEMENT |
...
| `test` | ubuntu-latest and windows-latest (x64), windows-11-arm | `contents: read` | | same as `dotnet.yml` linux and windows jobs, locked restore, audit as error |
...
7. For `2.0.0`: the README deployment and licence sections and the wiki `Installation` page describe the MSI (2.4.8); `docs/native/PLAN.md` references are resolved (Q-PKG-31).
...
| `Shell.AppPathsTests` (shared with 10) | `NoPathUnderSquirrelRoot` (INV-PKG-24) |
...
**AC-PKG-9.** After uninstall, `%APPDATA%\shotAI\settings.json`, `%APPDATA%\shotAI\logs\`, the projects folder and the native local data folder are byte-identical to before (hash comparison).
...
**AC-PKG-13.** For the first native prerelease: the GitHub release shows Pre-release and not Latest; `gh release view --json isPrerelease,isLatest` returns `true,false`; an Electron 1.3.0 client's manual update check reports up to date; `GET /repos/Armadillon44/shotAI/releases/latest` still returns a `v1.3.x` tag.
...
**AC-PKG-24.** Manual, before the pilot: on a PC with Electron 1.3.0 and native installed and signed in, running the 7.13.3 script removes `%LOCALAPPDATA%\shotai`, the Electron shortcuts and the HKCU ARP key; `%APPDATA%\shotAI\settings.json`, logs, projects and the native MSAL cache and WebView2 folders are intact; native still shows the user as signed in.
...
| 08 Auth | `ShotAIFederationFile` property, the five required keys, `BakedFederationParser` and `FederationConfigValidator` (reused by `ShotAI.Release federation-check`), `msalruntime` per RID, MSAL cache path | release secret wiring and preflight (7.11.3); public build is bring-your-own-key (INV-PKG-14); installer never writes the policy key (INV-PKG-3); path change request (Q-PKG-4) |
| 09 Exports | `shotai_avif.dll` ABI, WebView2 prerequisite, WebView2 user data folder, fonts | the pinned AVIF build (7.7), payload layout (7.2.3), WebView2 deployment (7.5.1), answer to Q-EXP-20 (no deletion on uninstall), path change request (Q-PKG-4) |
| 10 Infra | `ShotAI.GenBrand --check`, `--selftest` exit codes, `IAppPaths`, settings unknown-key preservation, prerelease-aware `IsNewer`, log sink sharing, the bootstrap logger used by `LegacyInstanceGuard` | the self-test behavior of the guard (EDGE-PKG-50: stderr line, exit code 2, no dialog), CI steps (7.11.2), release flags (INV-PKG-9), answer to Q-INFRA-8 (MSI), notices file, `IncludeSourceRevisionInInformationalVersion=false` |
| 11 Services | DI composition root | nothing at startup: `ProcessSnapshot` (Platform) and `LegacyInstanceGuard` (App) are constructed directly in 03 step 2a, before the container exists (7.10.3); `IProcessSnapshot` may also be registered later for diagnostics |
...
**Q-PKG-4. Native local data collides with the Squirrel root.** Recommended default: move native local data to `%LOCALAPPDATA%\LFI\shotAI\` (`entra\msal-cache.bin` for 08, `WebView2\` for 09), and add `IAppPaths.LocalDataDirectory` (10) returning it; update 08 and 09 in their next revision. Alternative: keep `%LOCALAPPDATA%\shotAI\` and require Electron removal before first native launch, which rules out the pilot's coexistence.
...
**Q-PKG-14. NuGet audit severity in PR CI.** Recommended default: audit errors fail the release workflow always; in PR CI set `<WarningsNotAsErrors>NU1901;NU1902</WarningsNotAsErrors>` (low and moderate) and keep high and critical as errors.
...
**Q-PKG-31. `docs/native/PLAN.md` does not exist.** `dotnet/README.md:8`, `dotnet/Directory.Build.props:22-23` and `.github/workflows/dotnet.yml:1` link it. Recommended default: the ordered implementation plan is written there (it is the natural home of the PR sequence that this spec's 7.11 to 7.13 assume); until then the links are dead and a reader lands on nothing.
