...
As built in WP-B9a: `AdoptAsync(dir, manifest)` is the three `Adopt` rows: into the open session through `ApplyDurable` when `dir` is its project (`IsOpen(dir)`: full paths, compared ignoring case), else a new open generation, the open session closed as Back closes it but without navigating, and a new session over `OpenedProject(dir, manifest)` made and shown as a successful open shows it. `ReloadAsync()`, which the shell calls when a session ends with a project shown, is 2.1's reload after a recording (`App.tsx:291-298`) without `Loading\u2026`: the store reads the open project again and its manifest is adopted into the session (EDGE-REP-43). A failed read closes the project as Back does: silently when the project is gone, as after a Discard of a new one (the Debug line), and with `OpenFailed` otherwise (the Warning line); a read overtaken by another open, a Back or another session changes nothing. `ProjectDetailViewModel` takes `ICaptureTargetSelection` (R-ARCH-26). The command bar has an actions row, 0.55rem under the head row, a `WrapPanel` whose other actions join with their packages; Resume capturing is its first (`Button.Small`, its tooltip shown while disabled too). `ResumeCaptureCommand` runs while a project is open and raises `ResumeCaptureRequested` with `BuildTarget()` read at the click, and the shell records into the open project with it, leaving the session as it is (06 7.7). The disable while a text step editor is open, with `ReportStrings.ResumeCapturingBlockedTitle` as its tooltip (INV-REP-24), joins with the editor (WP-C2).
...
As built in WP-A17: the frame is `ReportFrame`, a `Decorator` whose width is `ReportLayout.FrameWidth(s, viewport)` (a NaN viewport, which WPF would treat as Auto, offers the whole frame) and whose child is arranged centred inside the padding. The numbers of this section are `ReportLayout`'s constants; `FigureChrome` is 142, so a full-width figure is `HtmlImageMax(s)`. The command bar is its head row: Back, the title on one line with an ellipsis and the whole title as its tooltip, and the count. Its actions join with their packages (the SOP panel WP-D8, the size control WP-C3, Resume WP-B9a, Export WP-D10). The cards are `ReportList` with `StepCard.Row`; the insert zones, the overview's Add, Edit and Remove, the grip and the number editor join with WP-C2 and WP-C3. The empty state's hint is `ReportStrings.EmptyHint` in ink-2 with a 24.8 DIP line height, at most 468 DIP wide. The notices are 03's `NoticeHost`, over the report's row.
...
As built in WP-B9a: `Report/ProjectDetailStateTests` has `AdoptIntoOpenSessionIsDurable` (no new session, no `Loading\u2026`, the report reconciled; its inline editors join with WP-C2) and `ResumeUsesCaptureTargetSelection` (the target read at the click), with `AdoptWithNothingOpenMakesASession`, `AdoptOfAnotherProjectReplacesTheOpenOne`, `ReloadReadsTheDiskAgain`, `ReloadOfAGoneProjectClosesSilently`, `ReloadFailureRaisesOpenFailed` and `AnOvertakenReloadChangesNothing`; `ReportStringsTests` has Resume capturing's strings; 06's `Shell/ShellViewModelTests.ResumeRecordsIntoTheOpenProject` records into the open project through the shell.
