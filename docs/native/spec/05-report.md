...
- `round(x)` means JavaScript `Math.round`, ported as `JsMath.Round` from spec 01 (`var f = Math.Floor(x); return (x - f >= 0.5) ? f + 1 : f;`). Never C# `Math.Round`, which rounds half to even. `floor` is `Math.Floor`. `min`, `max` are `Math.Min`, `Math.Max` on doubles.
...
- Spec numbers: 01 model and store, 02 capture engine, 03 windows and shell, 04 editor and redaction, 05 this spec, 06 home list and settings UI, 07 SOP generation, 09 exports, 10 settings, logging and infrastructure, 11 service boundary, 12 packaging and CI. The brand and theme owner is numbered 08 by spec 03 and folded into 10 by spec 01; this document names it "the brand spec" instead of guessing. If PLAN.md numbers them differently, the names win.
...
| The in-app confirm and alert dialogs as used by the report | `src/renderer/useConfirm.tsx:1-91` |

**Does not own**

| Concern | Owner |
|---|---|
| The `project.json` schema, codec, `ProjectStore`, the serialized write queue, `ProjectSession` mechanics, path confinement | 01 |
...
| The Home list, project rename, the Home capture mode picker used by "Resume capturing" | 06 |
...
| Settings, logging sinks, the brand palette tokens (colors, radii, fonts) used by every rule quoted here | 10 and the brand spec |

Specs this one touches: 01, 02, 03, 04, 06, 07, 09, 10, 11, 12, and the brand spec.
...
| `projectTheme`, `projectPinUnrecognised` | `pinnedBrand(manifest.theme)`, `pinIsUnrecognised(manifest.theme)` (brand spec) | same | `:120-121,163-164,201-202` |
...
**INV-REP-18 [SECURITY]. Report images are read only through `ProjectStore.ResolveImage` (confined to the project folder, extension `.png`, `.jpg` or `.jpeg`); a manifest path that resolves outside the project or to another type is never read and the figure shows the missing-image state.** Why: manifest paths are untrusted (a synced or imported project); Electron enforced it with the `shot://` handler (01 2.9.10). Citation: `store.ts:209-213`, `src/main/main.ts:52-86`. Test: `App.Tests Report/ReportImageLoaderTests.RefusesPathsOutsideTheProject` (`..\..\secret.png`, an absolute path, `shots/x.exe`).
...
**EDGE-REP-43. Reload after a recording.** When a recording on the open project ends, App reopens the project, which shows `Loading\u2026` and remounts the report. Required natively: the capture flow's result is adopted into the existing session as an `External` change (7.3, 7.6); IMPROVEMENT: no `Loading\u2026` flash when the session is still open. Source: `App.tsx:291-298`.
...
| Optimistic zoom and pan | `pendingZoom` and `liveOffset` in the figure, cleared when the persisted framing round-trips (`macOS:shotAI/ReportView.swift:1127-1177,1229-1262`); zoom-in disabled at the max (`:1218`) | macOS is optimistic for zoom and pan only; every other edit waits for the store (`AppModel.swift:1185-1350`) | Windows generalizes this to every edit through `ProjectSession` (7.5), so no per-control pending state is needed |
...
| `ShotAI.Core.Report` | ShotAI.Core | `ReportPresentation`, `StepContext`, `StepNumberEntry`, `ReorderPlanner`, `MergePlanner`, `MergePlan`, `ReportEditState`, `ManifestChangeKind`, `DocScaleEditor`, `CaptureTargetPicker`, `PopoverPlacement`, `CardListDiff`, `WindowLine`, `ReportStrings`, `ReportEditPolicy` |
| `ShotAI.Core.Report.Operations` | ShotAI.Core | `ReportOperation` and the eleven concrete operations of 7.4, `TextStepFactory` |
| `ShotAI.Platform.Imaging` | ShotAI.Platform | `ReportImageDecoder` (WIC COM through CsWin32, sharing 02's `WicImageCodec` factory and its orientation and color helpers; scaled and oriented decode), `WicImageSizeProbe` (implements `IImageSizeProbe`), `CursorPosition` (CsWin32 `GetCursorPos`) |
| `ShotAI.App.Report` | ShotAI.App | Views: `ProjectDetailView`, `CommandBar`, `DocScaleControl`, `ExportButton`, `PackageExportDialog`, `ReportView`, `StepCardTemplates` (resource dictionary with the four card templates and `StepCardTemplateSelector`), `StepRail`, `ReportFigure`, `InsertZone`, `InsertMenu`, `InlineEditBox`, `TextStepEditor`, `OverflowMenuButton`, `CaptureInsertDialog`, `ConfirmDialog`, `NoticeStack` (03's control, hosted). View models: `ProjectDetailViewModel`, `ReportViewModel`, `StepCardViewModel`, `IntroViewModel`, `DocScaleViewModel`, `ExportViewModel`, `CaptureInsertViewModel`, `NoticeStackViewModel`. Services: `ReportImageLoader`, `DragReorderController`, `DragAutoScroller`, `MergeCoordinator`, `ReportRegistration` (DI) |
...
One `ProjectSession` (01 7.10) per open project, owned by `ProjectDetailViewModel`:

| State | Event | Action | Next |
|---|---|---|---|
| Closed | `OpenAsync(path)` (from 06 or the capture flow) | `IsLoading = true`; `opened = await store.OpenProjectAsync(path)`; `session = new ProjectSession(store, opened.Dir, opened.Manifest, uiContext)`; subscribe `Changed` and `PersistFailed`; build the report view model | Loaded |
...
| Closed | `Adopt(dir, manifest)` (the capture flow's `applyOpened`) | as a successful open without the store call | Loaded |
| Loaded | `Back` | cancel the view's `CancellationTokenSource` (image loads, merge flatten, export flatten); unsubscribe; `await session.DisposeAsync()` in the background (it drains pending writes, 01); navigate Home; ask the shell `SetDetailView(false, 1)` (03) | Closed |
| Loaded | session `Changed` | `ReportViewModel.Sync(session.Current, e.Kind, e.AffectedStepIds)` (7.8) | Loaded |

Every handler checks `sender == currentSession` and ignores events from a disposed or previous session (INV-REP-31). Opening B while A's session still drains is allowed; A's writes complete on the shared queue. `OpenAsync` increments an open generation before awaiting and, when the await returns, discards the result if a newer `OpenAsync` or a `Back` happened meanwhile (disposing the session it created, never showing it) (EDGE-REP-44, IMPROVEMENT). Navigating to Settings (06) is NOT `Back`: the session, `ReportEditState` and the open editors' drafts stay alive; only the view-level token's export flatten is canceled (EDGE-REP-41). Each open starts the report `ScrollViewer` at offset 0 (EDGE-REP-42).

`ProjectDetailViewModel` exposes, for the shell (03) and the brand spec: `bool ProjectOpen`, `string? OpenProjectPath`, `string? RawProjectTheme` (from `Current.Theme`), `double CommittedScale` (`DocScale.Clamp(Current.DisplayScale)`), raising `Changed` (03's `IShellNavigationState`). The View, Brand menu (03) edits through `session.Apply(new SetProjectThemeOperation(brand))` (owned by the brand spec; it follows 7.5's rules).

### 7.4 Operations (Core)

Every report edit is a `ReportOperation : ProjectOperation` (01 7.10): `Apply(ProjectManifest m)` is pure, deterministic, performs the edit in place and returns `Changed` or `Unchanged`; ids and all inputs are fixed at construction. The same instance runs twice: once on a clone of `Current` (the optimistic step) and once inside `store.MutateAsync` on the fresh disk read. A step lookup always takes the FIRST step whose `Id` equals the id (`StringComparison.Ordinal`); a missing step throws `StepNotFoundException($"step {id} not found")` (01's message).
...
public abstract class ReportOperation : ProjectOperation
{
    ...
    public abstract IReadOnlyList<string>? AffectedStepIds { get; }
}
```
...
`TextStepFactory.Create(id, callout)` returns a `ProjectStep` over a new `JsonObject` with keys in exactly this order: `id`, `order: 0`, `kind: "text"`, `screenshot: ""`, `trigger: "hotkey"`, `click: null`, `monitor: null`, `window: null`, `element: {available: false, name: null, controlType: null, bounds: null}`, `caption: ""`, `heading: ""`, `body: ""`, `callout` (only when non-null), `crop: null`, `annotations: []` (`project-store.ts:946-962`). 01's `AddTextStepAsync` must call the same factory so the optimistic and store paths cannot drift (section 10).

`AffectedStepIds`: R1, R2, R4, R5, R8 return `[stepId]`; R9 returns null (numbering of later steps changes); R3, R6, P1 return null; R10 and P4 return an empty list (no card changes; the overview or the layout does).

### 7.5 Optimistic editing policy (the required improvement)

Rule: every report edit that writes only `project.json` goes through `session.Apply(op)`: the screen changes immediately, the write is queued FIFO behind any earlier write, clicks are never dropped, and a failure rolls back and shows the rollback notice. Edits that write a file besides `project.json` go through `session.ApplyDurable(...)` and show their result only after the store call returns. The editor save path is the named exception of the fixed decision and is durable.
...
| P5, P6 | export, package export | durable: `IStepFlattener.EnsureFlattenedAsync` (04, the session overload), which persists each re-bake through `ApplyDurable`, then the export (09) | the export button's busy label and progress | the export's own result (09) | `Export failed: {message}` (parity) |
...
| P8 | View, Brand (03) | `Apply(SetProjectThemeOperation)` (brand spec) | the brand change | | rollback; notice |

Required session behaviors this policy depends on (requests to 01, section 10):

1. `Apply` whose optimistic `op.Apply` throws `StepNotFoundException` queues nothing and returns a faulted task; the report treats it as "the target is gone" and shows nothing.
2. `ApplyDurable` completion sets `Current` to the durable result WITH every optimistic operation issued after the durable call started re-applied on top, so an optimistic edit made during a merge or import is not lost from the screen.
3. `Changed` carries `ManifestChangeKind` and the operation's `AffectedStepIds` (7.6).
4. A persisted echo that is value-equal to `Current` (apart from `updatedAt`) raises `Changed` with `Kind = Persisted` and no visible change.
5. `PersistFailed` carries the failed `ProjectOperation`, so the report can recover a text draft (R4, R5, R8, R10).

**Rollback notice** (IMPROVEMENT, answers 01 Q-MODEL-12): `Your last change couldn't be saved and was undone. ` followed by the exception message. One slot (`SaveError`): a new failure replaces the previous text; dismissed by the user only. Logged at Warning with the operation type and the exception (no step text in the log).
...
public enum ManifestChangeKind { Local, Persisted, RolledBack, Durable, External }
...
| `Durable` | merge, import, screenshot, export flatten, editor save completed | yes | yes (parity) |
| `External` | open, adopt after capture, SOP apply and revert (07), a disk state adopted with changes this session did not make | yes | yes (parity) |
...
{
    public DocScaleEditor(Func<double> committed, Action<double> commit);   // commit = v => session.Apply(new SetDisplayScaleOperation(v))
...
All popovers and modals are drawn in 03's in-window overlay layer, not as separate `Popup` or `Window` HWNDs (IMPROVEMENT reasoning: parity with the DOM popovers, the same placement arithmetic as Electron, and no extra top-level windows to register for capture exclusion or to steal focus).
...
- Confirm and alert (`ConfirmDialog`): modal in the overlay layer with the 2.15 layout; the confirm button is focused on open (Enter confirms, parity); Escape cancels; Tab cycles inside (`KeyboardNavigation.TabNavigation = Cycle`); the backdrop does NOT close a confirm (parity: the confirm overlay has no click handler).
...
- Only one confirm or alert is shown at a time; a second request while one is open waits for the first to close (the Electron hook silently replaced it, 2.15).
...
5. Any exception except cancellation: alert `Could not merge these steps: {message}`. Finally `EditState.EndMerge()`.

Capture insert: `CaptureInsertViewModel(variant, atIndex)` wraps Core `CaptureTargetPicker` (the 2.19 logic with injected `Func<CancellationToken, Task<CaptureTargets>> listTargets` (02 `ListTargetsAsync`) and `Func<CancellationToken, Task<Rect?>> selectArea` (03 `IAreaSelectionService.SelectAreaAsync(window, ct)`)). Confirm: `screenshot` runs P3; `capture` checks `EditState.TextDraftOpen` and, when set, shows the info notice `Finish editing the text step before capturing.` without the import prefix (IMPROVEMENT, EDGE-REP-23, Q-REP-5), else raises `CaptureInsertRequested(atIndex, target)` for the App's capture coordinator (06, 02: `StartAsync(path, {Target, InsertAt})`). Resume capturing raises `ResumeCaptureRequested` (disabled while `TextDraftOpen`).

### 7.15 Export wiring (App `ExportViewModel`)

`ExportAsync(format)` and `ExportPackageAsync()` follow 2.4 exactly with: a per-run `CancellationTokenSource` linked to the view's token (canceled on Back, EDGE-REP-35); `await stepFlattener.EnsureFlattenedAsync(path, dir, session.Current.Steps, session, ct)` (04's session overload, which persists each re-bake through `session.ApplyDurable`; the four-argument overload writes around the session and is for Home exports only, 04 7.7, Q-EDIT-20); then `IExportService.ExportAsync(path, format, progress, ct)` or `ExportPackageAsync(path, includeOriginals, ct)` (09); `progress` is an `IProgress<ExportProgress>` created on the UI thread. `OperationCanceledException` after a cancel returns silently; any other exception sets `ExportError` (`Export failed: {message}`). The button's enabled state and labels follow 2.4 with `IsImporting` from P2 and `TextDraftOpen`.

### 7.16 Notices and errors

`NoticeStackViewModel` holds four slots, each one message: `ImportError` (`Import failed: `), `ExportError` (`Export failed: `), `SaveError` (the rollback notice, 7.5), `Info` (no prefix; the capture guard). Rendered by 03's shared notice control pinned to the top of the report viewport under the command bar (EDGE-REP-31). No auto-dismiss (parity). The Electron `Error: ` slot for a failed open moves to Home as `OpenFailed` (EDGE-REP-39). Merge failures use the alert (parity). The editor's notices are 04's.

### 7.17 Threading, cancellation, disposal

- UI thread: all view models, `ReportEditState`, `DocScaleEditor`, `session.Apply` (the optimistic step, a manifest clone plus `op.Apply`) and `Changed` handling.
- Store queue thread (01): `op.Apply` on the disk read and the write.
...
- Disposal: the view unsubscribes from the session, stops `DragAutoScroller`, cancels the token, then `DisposeAsync` the session without blocking the UI.
...
Keyboard: every rule of 2.21, implemented by `InlineEditBox` (single-line: `Enter` commits, `Escape` cancels, focus loss commits, `GotKeyboardFocus` selects all; multi-line: `AcceptsReturn = true`, `Ctrl+Enter` commits, `Escape` cancels, focus loss commits; a `done` flag makes the first exit win; `Abandon()` used by reconciliation sets `done` without committing) and `TextStepEditor` (no shortcuts, Save and Cancel only). Focus on open is set with `Dispatcher.BeginInvoke(DispatcherPriority.Input, ...)` so the element is in the tree.

### 7.19 Composition and packages

`ReportRegistration.AddReport(IServiceCollection)`: singletons `ReportImageLoader`, `ReportImageDecoder`, `IImageSizeProbe`; transients `ProjectDetailViewModel`, `ExportViewModel`, `CaptureInsertViewModel` factory; per-open factories for `ReportViewModel` and `DocScaleEditor`. Consumes `ProjectStore` (01), `IStepFlattener` (04), `ICaptureService` (02), `IAreaSelectionService` and `IMainWindowLayout` (03), `IExportService` (09), `ISopSettings` (07, for `sopEnabled`), `ILogger<T>` (10, category `report`).

NuGet (add to `dotnet/Directory.Packages.props`): `CommunityToolkit.Mvvm`, `Microsoft.Extensions.DependencyInjection`, `Microsoft.Extensions.Logging.Abstractions` (if not already added by another spec). No WinRT API is used by this subsystem. CsWin32 `NativeMethods.txt`: `GetCursorPos`; the WIC interfaces (`IWICBitmapScaler`, `IWICBitmapFlipRotator`, `IWICFormatConverter`, `IWICColorTransform`, `IWICMetadataQueryReader`) are already requested by 02 and 04.
...
| D-REP-22 | Escape closes the package dialog; every modal traps Tab; one confirm or alert at a time | IMPROVEMENT | EDGE-REP-46, 2.15 |
...
| `Report/ExportFlowTests` (fakes for `IStepFlattener` and `IExportService`) | `FlattenFailureStopsExport` (the export service is never called; `Export failed: Step 2 ("x") couldn't be prepared: ...`); `CancelIsSilent`; `ProgressLabel` (`Exporting HTML\u2026 3/10`, and without progress `Exporting PDF\u2026`); `PackageDialogOpensRedacted` (checked, closed, reopened: unchecked); button enabled rules (no shots, exporting, packaging, text draft, importing) |
...
| `Report/DialogKeyboardTests` | `PackageDialogEscapeCloses`; `CaptureDialogEscapeClosesWithDropdownOpen`; `ConfirmEnterConfirmsEscapeCancels`; `TabStaysInsideEachDialog`; `SecondConfirmWaits` (D-REP-22) |
...
|---|---|---|
| 01 Model and store | `ProjectStore` (`OpenProjectAsync`, `MutateAsync`, `ImportStepAsync`, `MergeStepsAsync`, `ResolveImage`), `ProjectSession` (`Apply`, `ApplyDurable`, `Current`, `Changed`, `PersistFailed`, `DisposeAsync`), `ProjectOperation`, `MutateResult`, `ProjectManifest` and `ProjectStep` views, `StepList.Renumber`, `CalloutKinds`, `CalloutGlyphs`, `JsMath`, `JsString`, `JsJson`, `StepNotFoundException` | the optimistic policy (7.5, answering 01 7.10's "decided by 05"); the rollback notice text (Q-MODEL-12); `DocScale.Clamp` (shared); `TextStepFactory` (01's `AddTextStepAsync` must use it); requests: (a) `Changed` carries `ManifestChangeKind` and `AffectedStepIds`, (b) `ApplyDurable` re-applies optimistic ops issued during it, (c) a throwing optimistic `Apply` queues nothing, (d) `PersistFailed` carries the failed operation; image-loading rule EDGE-MODEL-39 implemented in 7.11 |
| 02 Capture | `ICaptureService.ListTargetsAsync`, `CaptureScreenshotAsync(path, target, atIndex)`, `StartAsync(path, {Target, InsertAt})` via the App coordinator; `CaptureTarget`, `CaptureTargets`, `WindowInfo`, `MonitorInfo` | `CaptureInsertRequested(atIndex, target)`, `ResumeCaptureRequested`; `Adopt(dir, manifest)` after a recording |
| 03 Windows and shell | `IMainWindowLayout.SetDetailView(open, scale)`; `IAreaSelectionService.SelectAreaAsync(window, ct)`; the overlay layer; the shared notice control; `IShellNavigationState` consumer (the menu) | `IShellNavigationState` (`ProjectOpen`, `OpenProjectPath`, `RawProjectTheme`, `Changed`); the committed scale; the stepper hold (Q-SHELL-13) |
| 04 Editor | `EditorOverlayView` (Edit entry point `OpenEditor(step)`), `IStepFlattener.FlattenStepAsync` and `EnsureFlattenedAsync`, `StepPatch`, `StepPatchApplier`, `AnnotationStyle.MarkerColorFor`, `AnnotationFactory.Marker`, `FlattenMarker`, `CssColor.TryParse`, the orientation-respecting decode rule | the merge flow and its captions; the editor save adoption (durable) |
| 06 Home and settings UI | navigation into and out of the detail view; the Home capture target for Resume; `OpenFailed` display | `OpenAsync(path)`, `Back`, `OpenFailed(message)`; the Home list refresh after SOP changes is 06's (App effect `App.tsx:330-338`) |
| 07 SOP | `SopPanel` (hosted in the command bar), `ISopSettings.Enabled`; SOP apply and revert raise `External` changes through the session | the command bar slot; `session` access for the panel |
| 09 Exports | `IExportService.ExportAsync(path, format, progress, ct)`, `ExportPackageAsync(path, includeOriginals, ct)`, `ExportProgress`, the export crop of `reportZoom`/`reportPan` (`zoomCropRect`) | `DocScale.Widths` (the shared derivation 09's CSS and geometry use); the export menu, package dialog and flatten-first rule |
| 10 Settings and infra | `ILogger` category `report`; brand tokens (colors, radii, fonts) through the brand spec | log lines: Warning on persist failure (operation type, exception), Debug on image decode failure |
| Brand spec | theme tokens; `SetProjectThemeOperation` | `RawProjectTheme` from the session |
| 11 Service boundary | nothing | the deletion of the report's IPC channels (`projects:update-step` for report fields, `projects:reorder-steps`, `projects:delete-step`, `projects:merge-steps`, `projects:add-text-step`, `projects:set-intro`, `projects:set-display-scale`, `projects:import-step`, `projects:export`, `projects:export-package`, the `projects:export-progress` event, `capture:screenshot`, `capture:list-targets`, `view:set-detail`; `src/shared/ipc.ts:187-262`), replaced by direct calls |
...
**Q-REP-3.** Title editing and the macOS meta line (`Created`, `Updated`, numbered-step count) in the detail header. Recommended default: not in 2.0.0 (no new features); rename stays on Home (06).
...
**Q-REP-7.** Show an existing overview in the empty state (EDGE-REP-19). Recommended default: parity (hidden) for 2.0.0; low risk to change later.
...
**Q-REP-17.** Pixel equality with Chromium's text layout is not a goal: card heights will differ slightly because WPF and Chromium lay out text differently. Recommended default: accept; widths (the invariant) are exact, heights are not.
