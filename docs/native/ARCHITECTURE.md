...
| `ITriggerSource`, `IMonitorCapture`, `IWindowInfoProvider`, `IElementLocator`, `IWindowProtection`, `IOwnWindows`, `IImageCodec` | `Win32TriggerSource`, `GdiMonitorCapture`, `Win32WindowInfoProvider`, `UiaElementLocator`, `DisplayAffinityProtection`, `OwnWindowRegistry`, `WicImageCodec` (as built in WP-B6, the window and element rules are Core's: `WindowDescriber` over `IWindowFacts`, `ElementQueryPool` over `IElementReader`) | 02 7.2 |
...
| Core | `ShotAI.Core.Geometry` | 05 (01 uses `DocScale.Clamp`; 06 adds `FlexWrap`) | `DocScale`, `DocWidths`, `ReportGeometry`, `FlexWrap` (06's list head, WP-A16) |
...
| Core | `ShotAI.Core.Report`, `.Report.Operations` | 05 | `ReportEditState`, `DocScaleEditor`, `CardListDiff`, `PopoverPlacement`, `IImageSizeProbe` (the seam of 2.3, 05 7.1), the report operations; as of WP-A17 also `ReportPresentation`, `StepContext`, `WindowLine`, `ReportStrings`, `OpenFailure` and `ReportImageFile` |
| Core | `ShotAI.Core.Home`, `.Tour`, `.SettingsUi`, `.Theme` | 06 | `HomeListPipeline`, `DateGroups`, `AutoRefreshPolicy`, `ListSync`, `HomeText`, `HomeSelection`, `TourLayout`, `SettingsText`, `ThemeTokenSet`, `ISystemAppearance` (in `.Theme`, the seam of 2.3, 06 7.1) |
...
| Platform | `ShotAI.Platform.Shell` | 03, 10, 11 | `SingleInstanceLock`, `ExistingInstance`, `WindowMessages`, `WindowStyles`, `Foreground`, `MonitorQueries`, `ProcessMachine`, `ShellReveal`, `ShellUrlLauncher`, `StaThread`, `ConsoleAttach` |
| Platform | `ShotAI.Platform.Imaging`, `.Ocr`, `.Dialogs` | 02, 04, 05 | `WicFactory` (the one MTA `IWICImagingFactory`, 02 7.1), `ReportImageDecoder` and `WicImageSizeProbe` (in `.Imaging`, 05 7.1), and, internal, `WicDecoding` (the one decode path, R-ARCH-21) and `WicScope` (as of WP-A17), `WindowsOcrEngine` (`.Ocr`), `Win32ColorDialog` (`.Dialogs`, 04 7.1) |
...
| App | `ShotAI.App.Shell`, `.Capture`, `.Startup` | 03, 02, 12 (06 for the shell view) | windows (`ShotAIWindow` and its subclasses, `MainWindow` and `AboutWindow` among them), `ShotAIPopup`, `WindowRegistration`, `PopupExclusion`, `ActivationListener`, `MainWindowSizer`, `ShellCommands`, `NavigationState`, `ShellView`, `ShellViewModel`, `ScrollMemory` (06 7.7, WP-A16), `AreaSelectionService`, `RecordingVisibilityController`, `AppMenuViewModel`, `LegacyInstanceGuard`, `PersonalCopyGuard` |
...
| App | `ShotAI.App.Home`, `.Settings`, `.Tour`, `.Chrome` | 06 | views and view models (`HomeView`, `HomeViewModel`, `ProjectRowViewModel` as of WP-A16), `NoticeCenter`, `NoticeHost`, `LiveRegion`, `FlexWrapPanel`, `ConfirmService`, `ThemeManager`, `Themes/*.xaml` |
...
| 10 | same | `CaptureShield.ApplyRemoteVisibility(settings.Current.RemoteVisible)`, the only startup application of the setting, called synchronously on the UI thread: the one reasoned exception to DL1 (6.4). As built in WP-B5: `App.ApplyRemoteVisibility`; it stays synchronous, since the affinity call is not serviced by the UI thread (Q-ARCH-6) | protected first, relaxed after | INV-SHELL-2, 02 7.8 rule 2 |
...
| `ICaptureService` | `CaptureEngine` (also `IDisposable`; registered in WP-B6, with `ICaptureClock` as `TimeProviderCaptureClock`) | singleton | Core, with Platform seams | 02, 11 |
| `CaptureShield`, `IScreenCapture` (`ShieldedScreenCapture`) | same (registered in WP-B5; the shield subscribes to `IWindowProtection.WindowAdded` for 02 7.8 rule 1) | singleton | Core | 02 |
...
| `IAreaSelectionService`, `IMainWindowLayout`, `AppMenuViewModel`, `RecordingVisibilityController`, `CapturePillViewModel`, `PopupExclusion`, `WindowRegistration` (the `OwnWindowRegistry` wiring) | 03 types (as of WP-A15: `IMainWindowLayout` forwarded to the `MainWindowSizer` singleton, which startup step 8 passes to the main window, and `AppMenuViewModel`; as of WP-B7: `RecordingVisibilityController` and `CapturePillViewModel`, with `ShellShutdown`, the App's shutting-down flag of 03 EDGE-SHELL-51; as of WP-B8: `IAreaSelectionService`, the `AreaSelectionService` singleton) | singleton | App | 03 |
| `OwnWindowRegistry` | same, registered as itself (one of the two public Platform registrations, with 05's `ReportImageDecoder` as of WP-A17, 02 7.1; corrected in WP-A13: the App wires it through `WindowRegistration`, and Platform, which owns the type, registers it) | singleton | Platform | 02, 03 |
...
The view models see the application through the interfaces of 11 7.3, with the canonical names after 15.3; this table and 11 7.3 list the same interfaces. Nothing else is reachable from a view model except what INV-ARCH-3 names (the C6 factories, `IUiDispatcher`, the registered `TimeProvider`, `ILogger<T>`, and the editor's named Core dependencies).
...
| Capture dispatcher `shotAI.CaptureDispatcher` | `Win32TriggerSource.Attach`, as Core's `TriggerDispatcher` (corrected in WP-B4: it drains the trigger source's ring into the engine's callbacks, so it is the source's) | MTA | mousedown decisions, the synchronous click-time menu grab, starting element queries, enqueuing jobs | wait on the capture worker or the UI thread | engine events | 02 7.3 |
...
| UIA workers `shotAI.Uia.0`, `.1` | `UiaElementLocator.WarmUp` (as built in WP-B6: Core's `ElementQueryPool`, background threads) | MTA | UI Automation COM calls, 500 ms connection and transaction timeouts, 600 ms overall cap | touch WPF | awaited task | 02 7.6 |
...
| Categories | the type's namespace maps by the longest prefix that is a whole namespace to a label: `projects` (Store, Settings, Archive), `capture`, `claude` (Sop, Auth), `ocr`, `svc` (Threading, Links, and the boundary `call:` lines), `main` (everything else), and the empty banner label; no label longer than `projects` | 10 7.5.3, 11 L1 |
...
|---|---|---|---|---|---|
| PB-1 | Time inside `LowLevelMouseProc` per event | zero managed allocations; p99 under 50 microseconds; never near `LowLevelHooksTimeout` (at most 1 s, silent removal) | INV-CAP-17, feasibility "Hook timing"; p99 proposed | `MouseHookTests.CallbackIsAllocationFree` (02 8.4); a Platform perf test storing `QueryPerformanceCounter` stamps at entry and exit into a preallocated array over 1000 `SendInput` clicks (as built in WP-B4: `HookLatencyTests.HookP99IsUnder50Microseconds`, explicit, so CI does not run it); in the field, zero watchdog reinstall lines during the pilot | allocation test in Windows CI; latency manual |
| PB-2 | Mousedown to dispatcher pickup | p95 under 5 ms | proposed | hook stamp in the ring versus the dispatcher's dequeue stamp, Debug log (as built in WP-B4: `input: mousedown picked up <n> us after the hook`) | manual |
...
| `ShotAI.App.Tests` | Windows | view models against faked catalog interfaces, window registration and styles, the pill's non-activation, overlays, menu, startup and exit order, composition container, dispatcher contract, rasterizer and painter parity, report and Home UI behavior, end-to-end redaction | an STA harness (a small in-repo helper that runs each test on a dedicated STA thread with a `Dispatcher` and pumps until the test's task completes; Q-HOME-15). As built in WP-A16 it runs one test body at a time: WPF's resource package is shared by the process and not safe to read from two UI threads at once, and parallel test classes loading the same XAML failed in `PackagePart.CleanUpRequestedStreamsList`. The app has one UI thread; whether its second STA thread, `StaRenderThread` (04), can read a pack part (a font) while the UI thread reads the same one is checked where it lands |
...
| `Composition.ContainerTests`, `ViewModelDependencyTests`, `CommandConventionsTests`, `SubscriberDisposalTests` | 4.1 and 5.1 rules, INV-ARCH-3 (the allowed set includes the C6 factories, `ICaptureTargetSelection`, `IUiDispatcher`, `TimeProvider` and `EditorViewModel`'s named Core dependencies) | 11 |
...
`ShotAI.App.Tests` creates real windows on the STA harness: the pill shows, hides and shows without activating (`CapturePillWindowTests.ShowDoesNotActivate`, Q-SHELL-21), one overlay per monitor with exact bounds, Esc and right-button cancel, the startup order excludes windows before the setting applies, the exit order matches 4.5, the editor's pointer mapping at several zooms, painter parity between preview and bake. These run on the Windows CI job; cases that need a specific monitor layout (mixed DPI) or a live screen share are manual. As built in WP-B7: App.Tests holds a mutex of the session for its whole run, and the Platform tests' input hook collection holds it for its own, because `dotnet test` runs the test projects at once: the two sides' `SendInput` clicks collided, and the Platform tests' clicks closed App popups under their tests. The App tests that send real input (the pill's clicks, drags and tooltip) also run alone within App.Tests. A runner's open Start menu or Search, which sits above every topmost window, is closed with Escape before real input in both test projects (`ShellOverlay`).
...
| Q-ARCH-6 | Display affinity from worker threads (Q-CAP-15, Q-CAP-22) | direct calls from the grab thread; marshal to the UI thread | decided by the phase B probe before the pill and overlay are built; if marshaling is needed, a bounded wait that skips the grab on timeout (fail closed, DL2), and the startup application of 4.2 step 10 moves to `Task.Run` (DL1). Decided in WP-B5: direct calls from the grab thread; on both Windows runners the call returns without the owning thread and is in effect for the next read, for normal and layered WPF windows |
| Q-ARCH-7 | Watch the open project's folder for external changes? | `FileSystemWatcher`; none | none in 2.0.0 (parity); every write re-reads the disk; kept in WP-A17: the report shows what its session holds |
