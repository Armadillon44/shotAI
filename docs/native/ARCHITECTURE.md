...
- Where two specs disagree, 15.3 records which one wins and why (`R-ARCH-n`). A spec that is edited later follows the resolution; until then an implementer follows this document.
...
| G5 | **Operational gains that justify the rewrite** | Microsoft patches the runtime (framework-dependent, INV-PKG-5); per-machine signed MSI for Intune (INV-PKG-1, INV-PKG-7); native ARM64 (D-PKG-2); fewer moving parts (no IPC, preload, sandbox, `shot://`, asar). | 12 |
...
| INV-ARCH-3 | View models depend only on catalog interfaces (11 7.3), 06's chrome services, `ILogger<T>`, value types and other view models; never on `ProjectStore`, `EntraSession`, `IApiKeyStore`, `MsalGateway`, `CaptureEngine`, `SettingsService` (concrete) or any Platform type. | secrets never reach the UI (INV-IPC-1); fakes for tests | `Composition.ViewModelDependencyTests` (11 8.2) |
| INV-ARCH-4 | A Platform type that implements a Core seam is `internal sealed` and registered only as its Core interface. Platform helpers the App calls directly (03's `SingleInstanceLock`, `ExistingInstance`, `WindowStyles`, `MonitorQueries`, `Foreground`, the registration surface of `OwnWindowRegistry`, 12's `DllSearchHardening` and `ProcessSnapshot`, the existing `CaptureExclusion`, the `AddShotAIPlatform` extension) are public and have no Core counterpart. | the shield funnel depends on `GdiMonitorCapture` being reachable only through `ShieldedScreenCapture` (02 7.7) | `CaptureFunnelSourceTests` (02 8.4); reflection test in `Composition.ContainerTests` |
...
| Core | `ShotAI.Core.Store` | 01, 11 | `ProjectStore`, `IProjectService`, `IProjectSession`, `IProjectSessionFactory`, `IProjectSettle`, `ProjectOperation`, `SerialWriteQueue`, `AtomicFile`, `PathConfine`, `ReparseSafeDelete`, `ArchiveEngine`, `ImportLimits` |
...
| Core | `ShotAI.Core.Report`, `.Report.Operations` | 05 | `ReportEditState`, `DocScaleEditor`, `CardListDiff`, `PopoverPlacement`, the report operations |
| Core | `ShotAI.Core.Home`, `.Tour`, `.SettingsUi`, `.Theme` | 06 | `HomeListPipeline`, `DateGroups`, `HomeSelection`, `TourLayout`, `SettingsText`, `ThemeTokenSet` |
...
| Platform | `ShotAI.Platform.Capture` | 02 | hook, capture, UIA, affinity, own-window registry |
...
| Platform | `ShotAI.Platform.Imaging`, `.Ocr`, `.Dialogs` | 04, 05 | codec extensions, `WindowsOcrEngine`, `Win32ColorDialog`, `ReportImageDecoder` |
...
| Platform | `ShotAI.Platform.Theme`, `.Processes`, `.Composition` | 06, 12, 11 | `SystemAppearanceMonitor`, `ProcessSnapshot`, `PlatformServiceCollectionExtensions` |
...
| App | `ShotAI.App.Shell`, `.Capture`, `.Startup` | 03, 02, 12 | windows, `AreaSelectionService`, `RecordingVisibilityController`, `AppMenuViewModel`, `PopupExclusion`, `LegacyInstanceGuard` |
| App | `ShotAI.App.Home`, `.Settings`, `.Tour`, `.Chrome` | 06 | view models, `NoticeCenter`, `ConfirmService`, `ThemeManager`, `Themes/*.xaml` |
| App | `ShotAI.App.Report`, `.Editor`, `.Sop`, `.Export`, `.Auth` | 05, 04, 07, 09, 08 | views, view models, `ReportImageLoader`, `StaRenderThread`, `ExportService`, `EmbeddedBakedFederationSource` |
| App | `ShotAI.App.Threading`, `.Services`, `.Composition` | 11 | `WpfUiDispatcher`, `AppLifetime`, `ShutdownFlush`, `IFileDialogs`, `IAppInfo`, `AppServiceCollectionExtensions` |
...
| V5 | `NuGetAudit=true`, `NuGetAuditMode=all`; high and critical advisories fail every build, low and moderate fail the release workflow only. | Q-PKG-14 |
...
| 6 | same | build the container (4.3) | | 11 7.10 |
...
| 10 | same | `CaptureShield.ApplyRemoteVisibility(settings.Current.RemoteVisible)`, the only startup application of the setting | protected first, relaxed after | INV-SHELL-2 |
...
| `ISettingsService`, `IProjectStoreSettings`, `ICaptureSettings` | the `SettingsService` loaded at step 5b (instance plus forwarding factories; `Dispose` idempotent) | singleton | App (instance), Core (forwarders) | 10 |
...
| `IUiDispatcher` | `WpfUiDispatcher(Dispatcher.CurrentDispatcher)` | singleton instance | App | 11 |
...
The view models see the application through the interfaces of 11 7.3, with the canonical names after 15.3. Nothing else is reachable from a view model (INV-ARCH-3).
...
| # | Rule | Why | IDs |
|---|---|---|---|
| DL1 | No UI-thread code path takes `CaptureShield`'s lock. Window registration first applies `WDA_EXCLUDEFROMCAPTURE` without the lock, then posts the reconcile to the pool; the Settings toggle calls `ApplyRemoteVisibility` through `Task.Run`. | a cross-thread `SetWindowDisplayAffinity` may be serviced by the owning UI thread while the UI thread waits on the lock | 02 7.8, INV-CAP-29, Q-CAP-22 |
...
| WIC (`IWICImagingFactory`) | MTA factory, per-image objects per job | all in-box WIC codecs support MTA since Windows 7 (02 7.12) |
...
| S7 | `IProjectSettle.WhenSettledAsync(path)` finds the registered session for `Path.GetFullPath(path)` (compared `OrdinalIgnoreCase`, 02 D9) and awaits its `WhenIdleAsync`; with no open session it completes at once. 09's requested `IProjectService.WhenIdleAsync(path)` is this member (R-ARCH-6). | 07 7.13, 09 Q-EXP-11 |
...
| S9 | `DisposeAsync` unregisters the session from `IProjectSettle`, stops raising events, and lets queued writes drain on the shared queue; it never cancels them. | 01 7.12, 05 7.3 |
...
| capture steps during a recording | the engine calls the store directly on the shared queue; the project view is not shown while recording (INV-HOME-18), and after the session 05 `Adopt`s the result as `External` | when the recording ends | `CaptureFailed` on the pill | 02 7.12, 05 7.3 |
...
| remote visibility | the settings write above; `RemoteVisibilityApplier` applies the shield on every change, rollbacks included | immediately | the shield follows the rolled-back value | INV-IPC-13, D-IPC-8 |
...
|---|---|
| 01 | `ProjectNotKnownException`, `StepNotFoundException`, a merge-into-itself exception (replacing 01's `InvalidOperationException`, 11 X2), `UnsupportedImageException`, `ImportRejectedException`, `ArchiveException`, `ManifestCorruptException`; `ImportLimits` messages |
| 02 | capture errors, including the explicit-target message `A screenshot needs an explicit target (screen, window, or area).` and `TriggerException` text |
...
| 07 | `SopException`, `SopDisabledException`, `NoScreenshotsException`, `NothingToRevertException`; `SopNoLandingException` is internal to the apply flow |
...
| S7 | **Remote visibility matches the switch.** A change applies to open windows at once and on rollback; startup is protected first, then relaxed. | 11 `RemoteVisibilityApplier`; 03 startup step 10 | settings `Changed` subscription | INV-IPC-13, INV-HOME-41, INV-SHELL-2 | `Settings.RemoteVisibilityApplierTests`, AC-IPC-12 |
...
| Install | `%ProgramFiles%\shotAI\` (payload of 12 7.2.3), never written by the app | no | MSI | IMPROVEMENT (INV-PKG-15) |
...
| `Composition.ContainerTests`, `ViewModelDependencyTests`, `CommandConventionsTests`, `SubscriberDisposalTests` | 4.1 and 5.1 rules, INV-ARCH-3 | 11 |
...
| `JsMathTests.NoMathRoundInCapture` and the Core-wide ban of `Math.Round` (14.5) | JS rounding only | 02, this document |
...
Per-machine MSI per architecture (WiX MSBuild SDK), deployed by Intune as a Win32 app with the .NET 10 Desktop Runtime as a dependency and requirement rules routing x64 and ARM64 devices (INV-PKG-1, 12 7.5). MSIX was rejected because it virtualizes new `AppData` files, which would split `settings.json` and the logs from the Electron build during the pilot (12 7.1, EDGE-PKG-21). The installer never launches the app, never writes the policy key, adds a Start menu shortcut (`shotAI Preview` for prereleases), no desktop shortcut by default, and keeps all per-user data on uninstall (INV-PKG-3, INV-PKG-4, 12 7.4).
...
| F-20 | A per-machine MSI or MSIX, spec 12 decides | device-context Intune deployment, signing | 12 7.1 chose the MSI (I-2) |
...
| I-2 | The installer is a per-machine MSI per architecture, deployed as an Intune Win32 app; no MSIX. | MSIX virtualizes new `AppData` files, splitting `settings.json` and logs from Electron; the per-folder opt-out needs build 20348 | 12 7.1, INV-PKG-1, closes Q-INFRA-8 |
...
A spec edited later adopts these; until then implementers follow this table.

| # | Conflict | Resolution | Specs to align |
|---|---|---|---|
| R-ARCH-1 | Two homes for `JsMath` (02 lists `ShotAI.Core.Capture`, 01 `ShotAI.Core.Json`), and loose "floor(x + 0.5)" wording in 02's and 03's notation | one copy in `ShotAI.Core.Json` with the exact definition of 01 7.2.3 and INV-CAP-18; the notation wording is descriptive only | 02 7.1, 03 notation |
...
| R-ARCH-3 | Owner of the connection test | `IAuthService.TestConnectionAsync` is the one public entry point; 07's models-retrieve leg sits behind an internal seam; `IClaudeService.TestConnectionAsync` is removed | 07 (Q-IPC-2) |
| R-ARCH-4 | `IProjectStore` (09), concrete `ProjectStore` consumers (05 7.19, 07 `SopRequestAssembler`) | every consumer outside the store depends on `IProjectService` | 05, 07, 09 (Q-IPC-3) |
| R-ARCH-5 | Session construction and durable signature | sessions come from `IProjectSessionFactory.Create` on the UI thread (not `new ProjectSession`, 05 7.3); `ApplyDurable` takes `Func<IProjectService, ...>`; 04's `IStepFlattener` session overload takes `IProjectSession` | 01, 04, 05 |
| R-ARCH-6 | Three "wait for pending writes" APIs (07 `IProjectSettle.WhenSettledAsync`, 09 `IProjectService.WhenIdleAsync(path)`, 11 `IProjectSession.WhenIdleAsync`) | `IProjectSession.WhenIdleAsync` is the primitive; `IProjectSettle.WhenSettledAsync(path)` finds the open session and awaits it, or completes at once; 09 calls `IProjectSettle` | 01, 09 (Q-EXP-11) |
| R-ARCH-7 | `IStepFlattener` namespace (04 `ShotAI.Core.Rendering`, 11 `ShotAI.Core.Editor`) | `ShotAI.Core.Rendering`, 04 owns it | 11 (Q-EDIT-20) |
| R-ARCH-8 | `RenderGateException` (04) versus `RenderRefusedException` (09) | `RenderGateException`, deriving from `ShotAIException`, with a `(string, Exception)` constructor (07 D-SOP-22); 09 injects `IRenderGate`, 07 calls the static `RenderGate` with its probe | 07, 09 (Q-EDIT-20) |
| R-ARCH-9 | Export entry names (05 `ExportAsync`, 06 and 09 `ExportWithSaveDialogAsync`) | 09 7.13's seven members | 05 (Q-EXP-18) |
| R-ARCH-10 | Exit flush and container disposal (01 7.12: `Task.Run(...).Wait(6 s)` and `await provider.DisposeAsync()`; 11 7.10: one 5 s flush of both queues and `provider.Dispose()`) | 11 7.10: `ShutdownFlush.Run(5 s)` over the project and settings queues, then `provider.Dispose()`; every disposable singleton (including `ProjectStore` and `CaptureEngine`) also implements `IDisposable` | 01, 02 |
| R-ARCH-11 | Capture event marshaling (03 7.5 says `InvokeAsync`, 03 7.4.6 and 11 say `Post`) | `IUiDispatcher.Post` only | 03 7.5 |
| R-ARCH-12 | WebView2 version for About (03 7.11 references WebView2 from the App) | `IAppInfo.Current.WebView2Version` through Platform's `IWebView2RuntimeInfo`; the App has no WebView2 reference (INV-ARCH-5) | 03 |
| R-ARCH-13 | Native local data under `%LOCALAPPDATA%\shotAI\` (08 MSAL cache, 09 WebView2 folder), which is the Squirrel install root `%LocalAppData%\shotai\` under case-insensitive paths, so removing Electron at cutover would delete it (EDGE-PKG-22) | `IAppPaths.LocalDataDirectory` = `%LOCALAPPDATA%\LFI\shotAI\`; MSAL cache at `entra\msal-cache.bin`, WebView2 data at `WebView2\` (12 Q-PKG-4 default adopted) | 08 7.6, 7.16; 09 7.7; 10 7.4.4 |
| R-ARCH-14 | Spec numbering ("08 Brand and theme" in 06, 07, 09) and 09's brand names (`Brands.PinnedBrand`, `Brands.Coerce`, `BrandId`) | brand, palette, narrowing, settings, logging and links are 10; auth is 08; the brand functions are 10's `BrandPalette.PinnedBrand`, `CoerceBrand`, `IsBrandId`, `PinIsUnrecognised` with string brand ids | 06, 07, 09 (Q-AUTH-1, Q-INFRA-10) |
| R-ARCH-15 | Two Anthropic client factories and handler chains (07 `ClaudeClientFactory` with `EgressPinHandler` and `ResponseHeaderCaptureHandler` on the shared handler; 08 `AnthropicClientFactory` with a per-client `AnthropicHostGuardHandler`) | 08's `IAnthropicClientFactory` is the only factory (07's `IClaudeClientFactory`, `ClaudeClientFactory` and `ISopCredentialSource` are superseded). The shared `SocketsHttpHandler` carries NO Anthropic-specific handler (it also serves MSAL, the update check and GitHub). Per client, `ClientOptions.Handlers` holds a fresh host guard enforcing 07's stricter rule (`https`, host `api.anthropic.com`, port 443) and 07's header capture; the exchange client gets its own guard over the shared handler. That `ClientOptions.Handlers` observe every retry attempt's response (the 429 classifier needs the last one) is UNVERIFIED and pinned by a 07 test | 07 7.6, 08 7.9, 7.14 |
| R-ARCH-16 | `SopPanelViewModel` dependencies (07 7.10 lists `ProjectSession`, `IApiKeyStore`, `IConfirmHost`) | `IProjectSession`, `IClaudeService`, `IAuthService`, `ISettingsService`, `IStepFlattener`, `IConfirmService` (06's name) | 07 |
| R-ARCH-17 | 09 names `IPathConfine` and `IAtomicFile` | 01's static `PathConfine` with an `IPathProbe` argument, and the concrete `AtomicFile` (tests use temp folders, a fake classifier and `FakeTimeProvider`) | 09 |
| R-ARCH-18 | 04's `StaRenderThread` and inline text box use `Dispatcher` APIs that 11 bans in the App | allowlisted per file: `StaRenderThread.cs` (its own dispatcher) and one App helper `UiDeferral` for focus and layout deferrals at a named priority; service-event marshaling stays `Post` only | 04 7.10.5, 7.10.6 |
| R-ARCH-19 | Popovers as overlay-layer elements (05 7.13) versus `ShotAIPopup` (06 7.8) | the overlay layer by default; `ShotAIPopup` where WPF needs a popup HWND (tooltips, context menus, `ComboBox` drop-downs, 06's `OverflowMenu`); every HWND registered | none (both allowed) |
| R-ARCH-20 | `ManifestChangeKind` in 05's `ShotAI.Core.Report` while the session raises it; `AffectedStepIds` only on `ReportOperation` | `ManifestChangeKind` and the event args live in `ShotAI.Core.Store`; `ProjectOperation` gets a virtual `AffectedStepIds` returning null | 01, 05 |
| R-ARCH-21 | 04 7.13 and 05 7.11 decode with `IWICImagingFactory::CreateDecoderFromStream`, which picks a codec by sniffing and can invoke any installed third-party codec | after the magic-byte check, create the decoder explicitly with `CreateDecoder(GUID_ContainerFormatPng or GUID_ContainerFormatJpeg, ...)` and `Initialize` it on the stream (Q-IPC-16 default); undecodable input fails closed | 01, 04, 05, 09 |
| R-ARCH-22 | Monitor id types (02 `uint`, 06 `CaptureReadiness` `int?`) | `uint` (`(uint)HMONITOR`, 02 Q-CAP-11); the manifest's JSON number converts at the codec edge; never compared across launches | 06 |
| R-ARCH-23 | The session contract is spread over requests in 05, 07, 09 and 11 | the consolidated contract of 7.4 (S1 to S10), tested by `Store/ProjectSessionTests` (AC-ARCH-4) | 01 7.10 |
| R-ARCH-24 | Startup auto-archive refresh (01 7.14 calls `homeViewModel.RequestRefresh()`) | `IProjectService.ProjectsChanged`, raised by `AutoArchiveStaleAsync` when at least one project moved (Q-IPC-6) | 01 |
| R-ARCH-25 | `IExternalLinks.OpenAsync` throwing (11: only if the launcher throws; 06: never) | 11's contract; 06's callers wrap the call and show nothing | 06 (Q-INFRA-22) |
| R-ARCH-26 | `ICaptureTargetSelection` exists only in 06 | accepted: an App interface in `ShotAI.App.Home`, implemented by `CaptureModePickerViewModel`, consumed by 05 for Resume | 05, 11 |
...
| Q-ARCH-4 | `ANTHROPIC_CUSTOM_HEADERS`: mutate the environment or filter in a handler (Q-AUTH-17) | remove at startup; handler-only filter | remove at startup (08 default), with the host guard kept |
| Q-ARCH-5 | Ban `Math.Round` in Core | ban with `JsMath.cs` allowlisted; source-scan per subsystem only (02 `NoMathRoundInCapture`) | ban Core-wide: a banker's rounding slip breaks cross-platform values silently (05 risk on `clampScale`) |
| Q-ARCH-6 | Display affinity from worker threads (Q-CAP-15, Q-CAP-22) | direct calls from the grab thread; marshal to the UI thread | decided by the phase B probe before the pill and overlay are built; if marshaling is needed, a bounded wait that skips the grab on timeout (fail closed, DL2) |
...
| Q-PKG-31 | `docs/native/PLAN.md` is missing | write it; drop the links | closed: `PLAN.md` exists |
...
| Q-IPC-16 | Image decoding in the privileged process | accept with explicit decoders; an out-of-process decoder | accept with explicit decoders (R-ARCH-21) |
| Q-IPC-21 | `VSTHRD200` against `Apply` and `ApplyDurable` | keep names and suppress; rename | keep and suppress |
| Q-INFRA-1, Q-INFRA-2, Q-INFRA-3, Q-INFRA-12 | Settings file changes (preserve unknown enum strings and nested `sop` keys, default a non-absolute `projectsDir`, back up a corrupt file) | adopt; parity | adopt, with 01's agreement on `projectsDir` |
| Q-INFRA-5 | Update notice on managed devices | keep the notice; `updateCheckEnabled: false` via Intune | keep for the pilot; decide with IT |
| Q-HOME-15 | STA test harness | in-repo helper; a package | in-repo helper |
...
| D-ARCH-2 | Native per-machine-local data lives under `%LOCALAPPDATA%\LFI\shotAI\` | IMPROVEMENT | the obvious `%LOCALAPPDATA%\shotAI\` is the Squirrel install root and would be deleted with Electron (R-ARCH-13) |
