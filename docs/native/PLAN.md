...
| One test class | `dotnet test --project tests/ShotAI.Core.Tests/ShotAI.Core.Tests.csproj -c Release --filter-class "ShotAI.Core.Tests.Conformance.ConformanceTests"` (xunit.v3 filter option; checked in WP-A3: the .NET 10 SDK takes it without a `--` separator) | Linux, Windows |
| Conformance report (the open cases' output) | the same command with `--output Detailed`: `dotnet test` prints a passing test's output only then (01 7.11) | Linux, Windows |
...
| Deliverables | `src/ShotAI.Core/Model/`: `ProjectManifest`, `ProjectStep` (wraps the raw `JsonObject`; typed lenient views whose setters keep key position), `SopIntro`, `SopBackup`, `StepClick`, `CapturedWindow`, `CapturedMonitor`, `StepElement`, `CaptureTarget`, `CaptureTargetWindow` (added in WP-A3: the `window` of a capture target, the type 02's `IWindowInfoProvider.Resolve` takes), `Rect`, `Point`, `CalloutKinds`, `CalloutGlyphs`, `StepNumbering`, `StepList`, `StepGeometry` (`ParseRect`, `ParsePoint`; not `Geometry`, which would clash with the namespace `ShotAI.Core.Geometry` and fail with CS0234, 01 7.1), `ProjectTitles`; `src/ShotAI.Core/Json/JsValue.cs` (added in WP-A3: the internal JavaScript `typeof` and truthiness the codec, the views and the writer share); `src/ShotAI.Core/Geometry/DocScale.cs` with `Clamp` only; `src/ShotAI.Core/Codec/ManifestCodec.cs` (exactly `FileName`, `Decode`, `NormalizeSteps`, `CoerceIntro`, `CoerceSopBackup`, `Encode`, `Serialize` and `Read`, the authoritative member list of 01 7.4; `Encode` returns the `JsonObject` that `JSON.stringify` sees, and there is no `ToJsonNode`), `ManifestKeys.cs` (`All`, the 15 names); `ManifestCorruptException` (namespace `ShotAI.Core.Store`, 01 7.13, deriving from `ShotAIException`, ARCHITECTURE 8.1), which `Decode` throws on a null root and `Read` throws around a parse error; the harness: `Round_trips_through_the_codec` unskipped with the algorithm of `src/main/conformance.test.ts:70-139`, `ConformanceCase.Load` through `JsJson.Parse`, the `ExpectPath.Canonical` fix for `-0`, open cases reported as the test's output, which a `dotnet.yml` Linux step, `Conformance report (open cases)`, prints by re-running `ConformanceTests` with `--output Detailed` (changed in WP-A3: under Microsoft.Testing.Platform neither `xunit.runner.json`'s `diagnosticMessages` nor `TestContext.Current.AddWarning` reaches the `dotnet test` console, so there is no `xunit.runner.json`; 01 7.11 `Decided in WP-A3`); `tests/ShotAI.Core.Tests/Golden/macos-fixture/b7e2c4d1-9f3a-4e8b-a2c5-6d1f8e9a0b3c/` (the whole `macOS:Fixtures/b7e2c4d1-9f3a-4e8b-a2c5-6d1f8e9a0b3c/` folder, byte-identical: `project.json`, `shots/step-0001.png` to `step-0003.png`, `export/.render/9a1b3c5d-7e2f-4b8a-9c1d-2e4f6a8b0c3e.png`, about 790 KB) with a README naming `f445bca` (Q-MODEL-14); `Golden/codec/` inputs and Electron outputs; `.gitattributes`: `dotnet/tests/ShotAI.Core.Tests/Golden/**/*.json text eol=lf` and the same for `*.css`, `*.html`, `*.md` and `*.txt`, plus `dotnet/tests/ShotAI.Core.Tests/Golden/**/*.<ext> binary` for each of `png`, `jpg`, `jpeg`, `avif`, `pdf`, `docx`, `pptx` and `zip` (an explicit `text` would turn off binary detection and rewrite bytes inside those files); Electron test-only generator `src/main/codec-golden.test.ts` (2.7) |
| Tests | Port `src/shared/project.test.ts` (`Model/StepGeometryTests`, `Model/CalloutKindTests`), `src/main/conformance.test.ts` (`Conformance/ConformanceTests`, `ExpectPathTests`), `src/main/normalize-steps.test.ts` (`Codec/NormalizeStepsTests`), `src/main/unknown-callout.test.ts` (`Model/CalloutKindTests`, `Model/StepNumberingTests`), `src/main/section-callout.test.ts` (`Codec/SectionCalloutTests`), and the codec cases of `src/main/manifest-extras.test.ts` (`Codec/ManifestExtrasTests`; its store cases land in WP-A6). New: `Codec/DecodeTableTests` (including `NullRootThrowsManifestCorrupt`), `RootShapeTests`, `SopBackupTests`, `StepPassThroughTests`, `KeyOrderTests`, `ElectronGoldenTests`, `RegexAnchorTests` (its import-whitelist case moved to WP-A7 in WP-A3, with the regex), `Model/JsonObjectPositionTests`; added in WP-A3 for the types above: `Model/ProjectStepTests`, `Model/ModelJsonTests`, `Model/ProjectTitlesTests`, `Geometry/DocScaleClampTests` |
...
| Depends on | WP-A1 |
| Size | M |
| Risks and de-risking | Referencing an executable from the Microsoft.Testing.Platform test executable may not work (10 verifier note): fall back to a small `net10.0` library `ShotAI.GenBrand.Core` referenced by both, and record it in 10 7.3. Outcome in WP-A4: the reference works on both runners, so there is no library (10 7.3). CRLF checkouts: run the check on the Windows job with `core.autocrlf=true`. AC-INFRA-3's macOS stamp is compared by hand against `macOS:Scripts/gen-brand.swift` output |
...
| Risks and de-risking | Restricted DLL search against WPF's native loads (Q-PKG-30): measure the render tier and the loads with Process Monitor on both reference machines in this WP and add `AddDllDirectory` for the WindowsDesktop folder if needed; the formal AC-PKG-31 check runs on the installed MSI in WP-E3. The exit flush blocks the UI thread (DL4): `ExitFlushTests` blocks the UI thread during the flush and still completes. Outcome in WP-A12: `ExitFlushTests.NothingInTheFlushNeedsTheUiThread` blocks the UI thread in the flush while a session made on it has a write queued, and the flush returns within the bound; the render tier and the loads are step 6 of #134's manual script and not yet measured, since CI has no GPU, so Q-PKG-30 stays open; 104 mutations of the runtime line, the render switch, the startup-mode parser, the store self-test with its factory and store, and `AppVersion` were each caught by a failing test, none by a hang. The first pass found five that no test caught (the parser and the self-test accepting a null argument, the clean-up bound timed on the system clock instead of the given one, a trailing separator kept in the folder name, and the settings service left out of the self-test's disposal); `NullsAreRefused`, `CleanUpIsBounded`, `TrailingSeparatorIsNotPartOfTheName` and `TheCleanUpDisposesTheStoreAndTheSettings` now catch them. Five did not build as first written and ran as variants that do. The App and Platform code runs only on Windows, so its tests were not mutation-checked |
...
| Deliverables | App `ShotAIWindow` (registers and applies `WDA_EXCLUDEFROMCAPTURE` in `OnSourceInitialized`, before the first show), `ShotAIPopup`, `PopupExclusion` (class handlers; the `WH_CALLWNDPROC` catch-all of Q-SHELL-3 if the tests show a late registration), `WindowRegistration`, `ActivationListener`; `MainWindow` derives from `ShotAIWindow`, and in the same change the scaffold's `src/ShotAI.App/MainWindow.xaml` and `MainWindow.xaml.cs` (namespace `ShotAI.App`, `dotnet/src/ShotAI.App/MainWindow.xaml:1`, `dotnet/src/ShotAI.App/MainWindow.xaml.cs:5`) move to `src/ShotAI.App/Shell/` with namespace `ShotAI.App.Shell` and `x:Class="ShotAI.App.Shell.MainWindow"` (03 7.1); Platform `ShotAI.Platform.Shell`: `SingleInstanceLock` (`Local\shotAI.SingleInstance.<SID>`), `ExistingInstance`, `WindowStyles`, `Foreground`, `ProcessMachine` (landed in WP-A12 with the runtime line), and the registration surface of `OwnWindowRegistry` (its query side is completed in WP-B5); Platform `ShotAI.Platform.Imaging.WicFactory` (the shared MTA `IWICImagingFactory`, here because both WP-A17 and WP-B5 depend on this WP and run in parallel lanes); Core `SingleInstanceIdentity`, `ShellStrings` (first constants); startup steps 2, 7, 8 and 11. Added in WP-A13: the tests showed a late registration, so `PopupExclusion` installs Platform's `WindowShowHook` (the catch-all), with the class handlers and `ShotAIPopup` kept as a second layer; Platform `WindowMessages` (the App's way to the activation message id); Core `PixelRect`, ahead of the area selection, because `WindowStyles` takes and returns it; `WicFactory` is internal, since its users are all Platform types |
| Tests | No Electron file. New: Core `Shell/SingleInstanceIdentityTests` (made-up SID `S-1-5-21-1-2-3-1001`), `ShellStringsTests`; Platform `SingleInstanceTests`; App `AllWindowsRegisteredTests` (main window, a tooltip, a context menu, a `ComboBox` drop-down, and `NoWin32MessageBoxInSource` with the WP-E5 allowlist), `StartupOrderTests.SecondInstanceCreatesNoWindow`, `LifecycleTests.ClosingMainWindowShutsDown`; added in WP-A13: Core `Shell/PixelRectTests`; Platform `Shell/WindowStylesTests`, `ForegroundTests`, `WindowShowHookTests`, `Capture/OwnWindowRegistryTests`, `Imaging/WicFactoryTests`; App `Shell/ActivationListenerTests`, `MainWindowTests` (the four `ShowFromSecondInstance` cases and `TitleIsShotAI`), `SecondLaunchTests` (AC-SHELL-4 on two real instances), the `ShotAIPopup`, plain `Popup` and Win32 window cases of `AllWindowsRegisteredTests`, the exit order's listener and lock, and `Support/ShowProbe`, which records a window's display affinity at the moment it becomes visible |
| Acceptance criteria | AC-SHELL-4 |
| Depends on | WP-A12 |
| Size | M |
| Risks and de-risking | A popup HWND visible for one frame before registration (03 risk R1, Q-SHELL-3): `AllWindowsRegisteredTests` is written before any popup exists and decides between the class handler and the catch-all hook. Outcome in WP-A13: on both Windows runners the class handlers and `ShotAIPopup` registered each popup after it was visible (display affinity 0 at the `WM_WINDOWPOSCHANGED` that showed it), and a `ComboBox`'s drop-down not at all; with the `WH_CALLWNDPROC` catch-all every popup, the drop-down and a plain Win32 window of the UI thread were excluded before they were visible, so the hook is adopted and also covers the system dialogs of WP-B10, WP-C9 and WP-D10. 18 mutations of `SingleInstanceIdentity`, `ShellStrings` and `PixelRect` were each caught by a failing test, none by a hang; the Platform and App code runs only on Windows and was not mutation-checked |
...
| Deliverables | Core `ShotAI.Core.Theme`: `AppearanceResolver` (`Appearance` itself is declared by WP-A4, 10 7.2), `ActiveBrandResolver`, `Rgb`, `ColorMix`, `ChromeTokens`, `ThemeTokenSet`, `ThemeTokenKeys`; Platform `ShotAI.Platform.Theme.SystemAppearanceMonitor : ISystemAppearance`; App `ShotAI.App.Chrome`: `ThemeManager` (`ApplyInitial` before `MainWindow.Show`, one merged dictionary swapped per change, the high-contrast mapping behind a switch until design sign-off), `ThemeResources`, `CapsuleCornerConverter`, `UpperCaseConverter`, `Themes/Controls.xaml`, `Themes/FixedColors.xaml`; fonts: `dotnet/assets/fonts/static/*.ttf` (upstream Archivo static instances, Regular 400 to ExtraBold 800 plus the wdth 62 SemiBold and Bold) with `SOURCES.md` hashes and `OFL.txt`; the variable `src/renderer/fonts/Archivo.ttf` and its `OFL.txt` linked into the App output `Fonts\` until WP-E9 moves them under `dotnet/assets/fonts/`; `dotnet.yml` path filter gains `assets/**` and `src/renderer/fonts/**`. Added in WP-A14: the package `Microsoft.Win32.SystemEvents` in Platform (the desktop runtime carries `SystemEvents`, and Platform has no WPF); App `Shell/NavigationState` (`ProjectViewVisible`, `ProjectPinnedBrand`, `Changed`, `SetProjectView`), ahead of WP-A15, because `ThemeManager` takes it; App `Chrome/BundledFonts` (the font folders and the width families); the theme key `Font.label-stack` (the upstream `wdth` 62 files are the family `Archivo ExtraCondensed`); `App.ShowThemed`, steps 8 and 9's apply-then-show; the styles `FocusVisual`, `Badge.Base` and `HomeTabCount`; `.gitattributes` keeps `*.ttf` binary and the `OFL.txt` copies LF; `SOURCES.md` records upstream commit `555fa4a`, since the repository has no release tags |
| Tests | Port `src/renderer/project/app-chrome-tokens.test.ts` as `SourceGuards/XamlChromeGuardTests` (Core.Tests, Linux source scan), the CSS intents of `src/shared/theme-palette.test.ts` as `SourceGuards/XamlResourceKeyGuardTests`, and the theme-manager cases of `src/renderer/project/theme-wiring.test.ts` as App `Chrome/ThemeManagerTests` (the rest of that file lands in WP-A18). New: Core `Theme/AppearanceResolverTests`, `ActiveBrandResolverTests`, `ColorMixTests`, `ThemeTokenSetTests`; App `Chrome/ThemeResourcesTests`, `Shell/ShellStartupTests.ThemeAppliedBeforeMainWindowShown`, `Fonts/FontPackagingTests`, `Fonts/ArchivoRenderingTests`; Platform `Theme/SystemAppearanceMonitorTests`; added in WP-A14: Core `Theme/RgbTests`, `ThemeTokenKeysTests`, `ChromeTokensTests` (the static tokens against `project.css`); App `Chrome/ControlStylesTests` (every style under every theme, and the rendering that settles the capsule corner), `Shell/ShellStartupProcessTests` (the real exe's theme line before its runtime line); the guards' AC-HOME-3 mutations run on in-memory copies of the sources |
| Acceptance criteria | AC-HOME-3, AC-HOME-4, AC-HOME-21, AC-INFRA-30 |
| Depends on | WP-A4, WP-A12 |
| Size | M |
| Risks and de-risking | WPF does not drive the variable font's axes (Q-HOME-2): the static instances and `ArchivoRenderingTests.NormalIsNotSemiBold` decide; if no upstream release matches `fontRevision` 2.001, instance with fontTools in a documented script and say so in `SOURCES.md` (Q-INFRA-19). A single `StaticResource` silently stops following the brand: the source guards run on Linux. Outcome in WP-A14: upstream has no release tags, but its commit `555fa4a` built the bundled variable file byte for byte, so its unmodified statics ship. On both Windows runners each weight 400 to 800 resolved its own static file (`FontWeight.Normal` is Regular), the `wdth` 62 files formed their own family, `Archivo ExtraCondensed`, which `Font.label-stack` names first, and a label set at under 80% of the normal width; WPF also listed the variable file's 9 named weight instances, none condensed (Q-INFRA-9). A `CornerRadius` of 999 drew an ellipse, so the chip corner is `CapsuleCornerConverter`'s. The Core code runs on Linux and was mutation-checked; the Platform and App code runs only on Windows and was not |
...
| Deliverables | Core `WindowLayout` (`Initial`, `DetailResize`), `UiZoom`, `AboutText`, menu strings in `ShellStrings`; App `MainWindow` (720 x 740 DIP and the minimums), `MainWindowSizer : IMainWindowLayout` (physical px with `SetWindowPos` after `SourceInitialized`), `AppMenuViewModel` (File, Edit, View, Window, Help per 7.4.5, gestures including D21), `AboutWindow`, `ShellViewModel` skeleton (`CurrentView`), `NavigationState` skeleton (WP-A14 landed `ProjectViewVisible`, `ProjectPinnedBrand`, `Changed` and `SetProjectView` for the theme manager; this WP sets them from `CurrentView` and adds the rest of 06 7.7), `IAppInfo` and `AppInfoProvider`; Core `ShotAI.Core.Diagnostics.IWebView2RuntimeInfo` (`string? GetVersion()`, 11 7.2); Platform `MonitorQueries`, `MonitorDescriptorEx`, and `ShotAI.Platform.Export.WebView2RuntimeInfo : IWebView2RuntimeInfo` (adds `Microsoft.Web.WebView2` to Platform only). As built in WP-A15: `ShellViewModel` moves to WP-A16, whose `ShellView` hosts give `CurrentView` something to switch between (with the one placeholder view it would never change); `NavigationState` gains 03's `IShellNavigationState` (`ProjectOpen`, `OpenProjectPath`, `RawProjectTheme`) and one setter, `Set`, in place of `SetProjectView`. Added: Core `ShellConstants` (the main window's four), `DipRect`, `WindowLayout.ToDip` and `ToPixels`, and `DocScale.DetailWindowWidth` with the constants it needs, ahead of WP-A17, because `DetailResize` calls it; App `ShellCommands` (the window's own menu items as routed commands), `AppInfo` in a file of its own, the app icon as a WPF resource; `dotnet/Directory.Build.targets`, which drops WebView2's WPF and WinForms control assemblies from every project; package `Microsoft.Web.WebView2` 1.0.4191.47 |
| Tests | No Electron file. New: Core `Shell/WindowLayoutTests`, `UiZoomTests`, `AboutTextTests`; Platform `MonitorQueriesTests` (`ProcessMachineTests` landed in WP-A12); App `MainWindowTests`, `AppMenuViewModelTests.ItemsAndGestures`. As built in WP-A15, also: the `detailWindowWidth` group of `src/shared/doc-scale.test.ts` as Core `Geometry/DocScaleDetailWindowWidthTests`; the menu and About cases of Core `Shell/ShellStringsTests`; Platform `Export/WebView2RuntimeInfoTests`; App `Shell/AboutWindowTests`, `Services/AppInfoProviderTests`, `Shell/NavigationStateTests` (the basics; WP-A18 adds the Brand cases), `AllWindowsRegisteredTests.AboutIsExcludedBeforeItIsShown`, and `Architecture/SingleWebViewTests` (ahead of WP-D12, with the package) |
| Acceptance criteria | AC-SHELL-5, AC-SHELL-23, AC-SHELL-24 |
| Depends on | WP-A13, WP-A14 |
| Size | M |
| Risks and de-risking | Whether Electron's widths include the invisible borders (Q-SHELL-20): keep outer widths and measure both builds with `DWMWA_EXTENDED_FRAME_BOUNDS` in M-A. Initial placement on a secondary monitor at another DPI (EDGE-SHELL-48): `MainWindowTests.InitialPlacementOnSecondaryMonitorAtOtherDpi`. Outcome in WP-A15: the constants stay outer widths, and the comparison with the Electron window is in #137's manual script (Q-SHELL-20). The runners have one monitor, so `InitialPlacementOnSecondaryMonitorAtOtherDpi` skips there, saying why, and `InitialPlacementMovesOntoTheMonitorThenSetsItsRectangle` checks the two steps against a stood-in 150% monitor; the real mixed-DPI check is in the manual script. WebView2's package targets reference its WPF and WinForms control assemblies in every project that gets it, which in Platform conflicted with the framework's `WindowsBase` (MSB3277); `Directory.Build.targets` drops both, and on both runners the probe read the installed runtime's version through Platform alone. Of 57 mutations of the Core window rules, `DetailWindowWidth`, the zoom, the About text, the constants and the strings, 56 were each caught by a failing test, none by a hang, and one is equivalent: a +Infinity work area gives the wanted width either way (`Math.Min(want, +Infinity)` is `want`), so the `IsFinite` there only mirrors Electron's check. Planning them found four missing cases, now tested: the rounding of a positive fraction and of a positive half of the new x (`RoundsFractionsToTheNearestAndHalvesUp`) and of the first placement's x (`InitialRoundsFractionsToTheNearest`), and in `ToPixels` a nonzero top and a width that rounds differently on its own. The first CI run failed one new test, whose expected zoom factor was wrong. The App and Platform code runs only on Windows and was not mutation-checked |
...
| Deliverables | Core `ShotAI.Core.Home`: `HomeTab`, `HomeSortKey`, `DateBucket`, `DateGroups`, `DateGroup<T>`, `HomeListQuery`, `HomeListGroup`, `HomeListView`, `HomeListPipeline`, `AutoRefreshPolicy`, `HomeText` (list strings); App `ShotAI.App.Home`: `HomeView`, `HomeViewModel` (tabs, search, sort, refresh on the 20 s tick, activation and `ProjectsChanged`, newest result wins), `ProjectRowViewModel`, `GroupHeaderItem`; App `ShotAI.App.Shell.ShellView` (Home, Project, Settings hosts), `ShellViewModel` (`CurrentView`, moved from WP-A15, which had no second view for it to switch to; `NavigationState` follows it, 06 7.7), `ScrollMemory`; App `ShotAI.App.Chrome`: `INoticeService`, `NoticeCenter`, `NoticeHost` (03's notice control). As built in WP-A16: Core also has `ListSync` (`ListEdit<T>`, `ListEditKind`: the keyed diff of 06 7.6) and `Geometry.FlexWrap` (`FlexItem`, `FlexSlot`: the list head's CSS flex-wrap row), and gains 01's `IsoTime.FromLocalTime` (the local midnights) and `ProjectSearch.ByTitle(CompareInfo)` and 11's `UserMessage.IsUnexpected`; App also has `Chrome.NoticeKind`, `NoticeViewModel`, `LiveRegion` (the live-region announcement) and `FlexWrapPanel`, and `Shell.ShellViewKind`. `MainWindow` shows `ShellView`: the header (the logo, the name, the `⚙ Settings` button), the Home, project and Settings hosts, and the overlay layer with the notices. Startup step 8 makes the shell, has `NavigationState.Follow` it, attaches the notices to `CrashLogging` (Q-SHELL-16) and starts it after the window is shown. `HomeViewModel` hands over the chosen project as `OpenRequested` and the Import button as `ImportRequested`, which WP-A17 and WP-D15 connect; the shell's Settings transitions wait for the Settings view (WP-B10), and until then the header's button runs the menu's `OpenSettingsCommand`. The view model guard allows `TimeProvider` (INV-ARCH-3) |
| Tests | Port `src/renderer/project/date-groups.test.ts` (`Home/DateGroupsTests`). New: Core `Home/DateGroupsDstTests`, `HomeListPipelineTests`, `AutoRefreshPolicyTests`, `HomeTextTests` (list literals); App `Home/HomeViewModelTests` (`OnEnterResetsListControls`, `RefreshTriggers`, `RefreshCoalesces`, `OlderResultIgnored`, `BackgroundRefreshKeepsError`, `BackgroundRefreshFailureIsSilent`, `OpenFailedShowsNotice`), `Chrome/NoticeCenterTests`, `Shell/ShellScrollTests` (Home part). As built in WP-A16: also Core `Home/ListSyncTests` and `Geometry/FlexWrapTests`; App `Home/HomeViewTests`, `Chrome/FlexWrapPanelTests`, `Shell/ShellViewModelTests`, `NavigationStateTests.FollowsTheShell` and `.ProjectViewNotVisibleInSettings`, `CrashLoggingTests.GenericNoticeAfterStartupWhileVisible`, and `HomeViewModelTests.UserRefreshFailureShowsNotice` and `.SettingsFromHomeStopsTickAndRefreshesOnReturn`; `OpenFailedShowsNotice` moves to WP-A17 with the open, and `NoticeCenterTests`' update case to WP-E1 |
| Acceptance criteria | AC-HOME-1, AC-HOME-5, AC-HOME-6, AC-HOME-12, AC-HOME-35, AC-SHELL-33 |
| Depends on | WP-A15, WP-A8 |
| Size | M |
| Risks and de-risking | Local midnights in DST gaps (EDGE-HOME-2): custom time zones in `DateGroupsDstTests`. 500 rows with a shadow each (Q-HOME-14): measure against AC-HOME-35 and replace the shadow if frames exceed 16 ms. Outcome in WP-A16: the DST cases pass with custom zones on Linux and on both runners, and the rows keep their shadows, with AC-HOME-35 in #138's manual script. The first Windows run failed five new App tests, none in the app: parallel test classes loading the same XAML raced in WPF's resource package (`PackagePart.CleanUpRequestedStreamsList`), so the STA harness runs one body at a time (the App tests went from 22 s to 24 s on x64 and from 28 s to 44 s on arm64); `Binding.TargetUpdated` fired before a new notice's text arrived, so the announcement is the `LiveRegion` attached property; `TextBlock.Text` does not follow XAML inlines until the first measure, so the tests read text through the automation peer, which does (the app's accessible names were right); the notice's entry animation moved the card the position test measured; and a dispose test let the posted refresh run first. Of 82 mutations of the Core list rules (the date spans, the pipeline, the refresh policy, the strings, the flex row, the keyed diff, `FromLocalTime`, `IsUnexpected`, the name collation), the first run caught 60. Twelve were the script's own mistakes (a constant `if (false)`, which the build refuses, and patterns whose escapes were decoded before matching), and two were equivalent: a line of more than one item never overflows, so the proportional shrink only ever ran for one item, and an item already at its maximum is clamped by the loop anyway. Both were removed from `FlexWrap`. The other eight were missing cases, now tested: the zone's date for the week start (`TodayIsTheZonesDate`), the gap in line filling (`TheGapCountsWhenFilling`) and a negative gap, a comparer that calls different instances equal and an undefined edit in `ListSync`, and a culture whose name order differs from invariant (`TheNameSortIsTheCulturesOwn`). The rerun caught all 81, none by a hang. The App and Platform code runs only on Windows and was not mutation-checked |
...
| Deliverables | Core `ShotAI.Core.Geometry`: `DocScale` (detents, `IsLegal`, `DetentIndex`, `DetentAt`, `Widths`, `DetailWindowWidth`; `DetailWindowWidth` and its constants landed in WP-A15 with the window sizing, and the rest of `doc-scale.test.ts` ports here), `DocWidths`, `ReportGeometry` (`Fit`, `WrapContentSlack`), `ReportFit`, `ImageSize`, `ReportLayout`; Core `ShotAI.Core.Report`: `ReportPresentation`, `StepContext`, `WindowLine`, `CardListDiff`, `ReportStrings` (display strings); Platform `ShotAI.Platform.Imaging` (uses WP-A13's `WicFactory`): `ReportImageDecoder` (magic bytes first, explicit PNG and JPEG decoders, EXIF orientation, decode at display size, `FileShare.Delete`, R-ARCH-21), `WicImageSizeProbe : IImageSizeProbe`; App `ShotAI.App.Report`: `ProjectDetailView`, `ProjectDetailViewModel` (`OpenAsync` with an open generation, `Back`, `OpenFailed` shown on Home with the Q-MODEL-11 wording), `ReportView`, `StepCardTemplates` (four variants and the selector), `StepRail`, `ReportFigure` (fit, stored zoom and pan framing, marker overlay; no editing), `ReportImageLoader` (cache key `(flattened, renderRev)`), `NoticeStackViewModel` (display), the command bar skeleton (Back, title, step count), `IMainWindowLayout.SetDetailView` on open and Back. Added by WP-A16: the open's wiring in the shell, `HomeViewModel.OpenRequested(path)` to `ProjectDetailViewModel.OpenAsync`, and the project view calling `ShellViewModel.ShowProject(path, rawTheme)` on a successful open and `CloseProject()` on Back (WP-A16 left both unconnected). As built in WP-A17: Core also has `CardContext`, `ReportImageKey`, `ReportFraming`, `MarkerStyle`, `ReportMarker` and `MarkerFraction` (beside `StepContext` and `ReportPresentation`), `OpenFailure` (the gone rule, which also looks through the exceptions a failure wraps, since the store wraps a missing `project.json` in `ManifestCorruptException`) and `ReportImageFile` (the read that shares the file for writing and deleting); ahead of WP-C8 and WP-C7, 04's `CssColor`, `Rgba`, `AnnotationStyle` (`Accent`, `RightClickColor`, `MarkerColorFor`) and `ScreenshotLoadException`. Platform also has `DecodedImage` and, internal, `WicDecoding` (the one decode path: magic bytes, the built-in decoder with a vendor check, orientation, an embedded profile to sRGB, premultiplied BGRA, Fant, a buffered flip-rotator) and `WicScope`, and the WIC names in `NativeMethods.txt`. App also has `ReportFrame`, `ReportList` (List and ListItem peers), `StepCardKind`, `ReportNoticeSlot`, `ReportImage`, `ReportViewModelFactory` and `FixedColors.MarkerHalo`; `ReportView` is part of `ProjectDetailView`, and a `DataTrigger` on the card's kind picks its template (`StepCard.Row`), because WPF runs a selector once per container. `OpenAsync` returns whether its project is the one shown; `OpenFailed` carries the exception, which Home shows through `INoticeService.ShowError`. The image key is `ReportImageKey(path, renderRev)`, the render's revision for a render and 0 for a raw screenshot, and the bitmap is made and frozen on the UI thread from the pixels the pool decodes. `NoticeStackViewModel` has its four slots and no producer yet |
| Tests | Port `src/renderer/project/report-geometry.test.ts` (`Geometry/ReportGeometryTests`), `src/shared/doc-scale.test.ts` (`Geometry/DocScaleTests`) and `src/shared/report-matches-export.test.ts` (`Geometry/ReportMatchesExportTests`). New: Core `Geometry/ReportLayoutTests`, `Report/ReportPresentationTests`, `Report/CardListDiffTests`; App `Report/ReportLayoutTests`, `ReportImageLoaderTests` (`DoesNotHoldTheFile`, `RefusesPathsOutsideTheProject`, `JpegExifOrientationIsApplied`, `DecodeSizeTracksZoom`), `Report/ProjectDetailStateTests` (open cases); moved from WP-A16: `Home/HomeViewModelTests.OpenFailedShowsNotice`, and `Shell/ShellScrollTests` (the project starts at the top). As built in WP-A17: Core also `Report/StepContextTests`, `OpenFailureTests`, `ReportImageFileTests`, `ReportStringsTests` (display strings), `Editor/CssColorTests` and `AnnotationStyleTests`; `CaptionEditTouchesOneCard` is App `Report/ReportViewModelTests.CaptionEditTouchesOneCard`, since it needs the card view models; `ProjectDetailStateTests` is App's, with 13 open cases; App also `Report/ReportImageDecoderTests`, `ImageSizeProbeTests` (App, not Platform: the fixtures need WPF's encoders), `ReportViewModelTests`, `StepCardViewModelTests`, `NoticeStackViewModelTests`, `ReportAutomationTests` (the list and row names), the figure's cases in `ReportImageLoaderTests` (`ReloadsOnRenderRevOnly` ahead of WP-C10, `ANewKeyKeepsTheOldImageUntilTheNewOneLoads`, `AMissingImageShowsItsPath`, `LoadsNearTheViewportOnly`, `TheRingSitsOnTheClick`, `PanOffsetFollowsTheStoredPan`), `ReportLayoutTests.NarrowWindowShrinksTheColumn` ahead of WP-C3, `Shell/ShellScrollTests.TheProjectStartsAtTheTop`, `Shell/ShellViewModelTests.HomesOpenShowsTheProjectAndBackReturns` and `.AFailedOpenStaysOnHome`; Platform `Imaging/WicDecodingTests` |
| Acceptance criteria | AC-MODEL-25, AC-REP-1, AC-REP-3, AC-REP-5, AC-REP-28, AC-REP-32, AC-REP-33 |
| Depends on | WP-A16, WP-A9 |
| Size | M |
| Risks and de-risking | The native report is 50 DIP narrower than Electron's shipped report (Q-REP-2, EDGE-REP-1): decide at M-A whether the optional Electron CSS parity fix ships before the pilot (2.7). JS rounding in `DocScale` (05 risk): only `JsMath.Round`. WIC projection names are unverified: CsWin32 reports unknown names at build time. Image decoding now runs in the privileged process (Q-IPC-16): only explicit decoders after the magic-byte check. As built in WP-A17: CsWin32 projects every WIC name the decoder uses by its SDK name |
...
| Deliverables | Core `BrandMenuModel`, `BrandMenuItem`, `BrandMenuInput`; Core `ShotAI.Core.Report.Operations.SetProjectThemeOperation` (the value passes through untouched, `null` clears); App `NavigationState` complete (`ProjectOpen`, `OpenProjectPath`, `RawProjectTheme`, `ProjectViewVisible`, `Changed`; the properties and 03's `IShellNavigationState` landed in WP-A15, and this WP feeds them from the open project), the Brand submenu and `ChooseBrandCommand` in `AppMenuViewModel`; `ThemeManager` follows `ActiveBrandResolver`. As built in WP-A18: Core `ShellStrings.Brand` and `BrandAppDefault`, and `ReportStrings.RolledBack`; `ReportOperation` landed here, ahead of WP-C1, as `SetProjectThemeOperation`'s base, and `ProjectStore.SetProjectThemeAsync` applies the same operation, so the optimistic and the store paths share one rule. App: `NavigationState` takes an `ILogger` and raises `Changed` through `EventRaiser`, so a subscriber that throws is logged and never fails the edit; `ShellViewModel` follows the project view's `RawProjectTheme` while the project it shows is the one open; `AppMenuViewModel(IShellNavigationState, ISettingsService, IUiDispatcher, ILogger)` makes the rows once as `BrandMenuRowViewModel`s, has `BrandMenuEnabled`, `BrandItems`, `ChooseBrandCommand` and 11's L3 line, and is disposable. The menu holds no session: `ChooseBrandCommand` reads the open path and raises `ProjectThemeChosen` (a `ProjectThemeChoice`), which the shell hands to `ProjectDetailViewModel.SetProjectTheme(path, brand)`, which applies the operation to its session while that project is still the one open. Ahead of WP-C2, `ProjectDetailViewModel` subscribes to `PersistFailed` and shows the rollback notice in the Save slot, whose prefix `NoticeStackViewModel` now adds. `ThemeManager` needed no change: it has followed `ActiveBrandResolver` through `NavigationState` since WP-A14 and now sees the pin change. In a Debug build only, `DebugNavigationPulse` re-raises the navigation state every second when `SHOTAI_DEBUG_NAV_PULSE=1`, for AC-SHELL-21's manual check |
| Tests | Port `src/renderer/project/theme-wiring.test.ts` (the portable cases per 06 8.3 and 11 8.1; push, rebuild, arm-before-build and coercion cases are ELECTRON-ONLY): Core `Shell/BrandMenuModelTests`; App `Shell/NavigationStateTests`, `ServiceBoundary/BrandChoicePassThroughTests`, `AppMenuViewModelTests` (`BrandChoiceCallsStoreForOpenProject`, `BrandChoiceIgnoredWithNoProject`, `BrandItemsNotifyOnlyOnChange`, `CheckedIsNotToggledByWpf`). As built in WP-A18: Core also `Report/Operations/SetProjectThemeOperationTests`, `BrandMenuModelTests.IdsAreMatchedExactly` and `.AtMostOneRowIsTicked`, the Brand rows of `Shell/ShellStringsTests` and the rollback prefix in `Report/ReportStringsTests`; App `NavigationStateTests` has 06 8.3's six cases and `FollowsTheOpenProjectsPin`, `BrandChoicePassThroughTests` adds `TheTickedRowWritesNothing` and `AppDefaultClearsAnUnrecognisedPin`, `AppMenuViewModelTests` adds `BrandChoiceReadsTheOpenProjectWhenRun`, `BrandItemsFollowTheAppBrand`, `BrandStateIsLoggedOnChange`, `EveryRowRunsTheMenusCommand`, `DisposeStopsFollowing` and `BrandSubmenuFollowsTheOpenProject`, and Brand joins `ItemsAndGestures`; App `Report/ProjectThemeEditTests` (the repaint, Settings over the project, an unrecognised pin, the rollback notice, the generic sentence, a stale choice) |
...
| Deliverables | Core `ShotAI.Core.Capture`: `CaptureConstants`, `CaptureGeometry`, `AutoClassifier`, `ClickCaptions`, `DownscalePolicy`, `ShotNaming`, `UiaControlTypes`, `ElementMapping`, `CaptureShield` (reference counted, restores the setting, exception-safe, `ApplyRemoteVisibility`, `ExcludedForNewWindow`), `ShieldedScreenCapture : IScreenCapture`; the seam interfaces `ITriggerSource`, `IMonitorCapture`, `IScreenCapture`, `IWindowInfoProvider`, `IElementLocator`, `IOwnWindows`, `IWindowProtection`, `IImageCodec`, `ICaptureClock` (`ICaptureSettings`, also a 02 7.2 seam, is declared by WP-A10 and consumed here by `CaptureShield(IWindowProtection, ICaptureSettings)`); the records of 7.2 (`CaptureState`, `CaptureStartOptions`, `DiscardResult`, `CaptureTargets`, `MouseDown`, `PixelFrame`, `MonitorDescriptor`, `ForegroundInfo`, `ListedWindow`, the event args) and `ICaptureService`. As built in WP-B1: `ICaptureService` moves to WP-B2 with `CaptureEngine`, because a catalog interface must resolve as soon as it exists (`ContainerTests.EveryCatalogInterfaceResolves`); `WindowInfo` and `MonitorInfo` are declared here, in `ShotAI.Core.Capture`, with `CaptureTargets` (02 7.1); the clock's delay is `DelayAsync` (VSTHRD200); the internal `AsciiCase` folds ASCII only, for the JavaScript `/i` patterns |
| Tests | Port `src/main/capture-geometry.test.ts` (`Capture/CaptureGeometryTests`, `AutoClassifierTests`), `src/main/capture-shield.test.ts` (`Capture/CaptureShieldTests`, `CaptureFunnelSourceTests`) and `src/main/click-caption.test.ts` (`Capture/ClickCaptionsTests`, extended with every 2.9.1 row and the hotkey captions). New: `JsMathTests.NoMathRoundInCapture`, `JsMathTests.NoJsMathCopyInCapture` (no type named `JsMath` outside `ShotAI.Core.Json`, INV-CAP-18), `DownscalePolicyTests`, `ShotNamingTests`, `UiaControlTypesTests`, `ElementMappingTests`, `ShieldedCaptureTests`, `CaptureFunnelSourceTests.NoGraphicsCaptureUsage`. As built in WP-B1: 02 8.2's `EngineDependsOnlyOnShieldedCapture` is `CaptureFunnelSourceTests.OnlyTheFunnelHoldsTheRawCapture`, a reflection check of every Core type, which covers `CaptureEngine` when WP-B2 adds it; new `Capture/CaptureConstantsTests` (02 section 3, read from the Electron sources); the shared fakes are `Capture/CaptureFakes.cs` |
...
| Size | M |
| Risks and de-risking | Races between a job, stop and teardown (EDGE-CAP-28, EDGE-CAP-45): the gate lock never spans I/O or an await, and each await is followed by the generation check. Whether the store renumbers the caller's `ProjectStep` in place (02 verifier doubt): the engine sets `Order` from the store result itself. Outcome in WP-B2: the store returns the manifest as written and the engine reads the landed step from it by id. The races are pinned by `AStaleClickLandsInNoLaterSession`, `CaptureBailsWhenSessionClearedMidFlight`, `CaptureBailsWhenSessionClearedDuringTheElementQuery`, `TeardownDuringAStartLeavesNoSession`, `TeardownDuringAScreenshotHidesNothing`, `ConcurrentStartsCannotBothInstall` and `AFailedStartFailsItsWaiterToo`. 313 mutations of the engine, its session, the clock and the messages: 311 caught, 4 of them rewritten because their first text did not build, and 2 equivalent; the first runs found 63 gaps, each closed with a test. The Platform seams are faked, so nothing here runs against Windows yet |
| Demo | tests |
...
| Deliverables | Core `AreaSelectionMath`, `DipRect`, `PixelRect` (landed in WP-A13 with `WindowStyles`); App `AreaOverlayWindow` (`AllowsTransparency`, `#01000000` fill so it receives clicks, sized with `SetWindowPos` in physical px, re-applied on `DpiChanged`, overlay under the cursor activated, Esc on every overlay), `AreaSelectionService : IAreaSelectionService` (newest request wins, cancellation, requester restored). As built in WP-B8: Core `AreaSelectionMath` also has `OverlayUnder` (the overlay under the cursor, EDGE-SHELL-27), and the overlay's constants and strings join `ShellConstants` and `ShellStrings`; App `AreaSelectionService` is the singleton `IAreaSelectionService`, and `AreaOverlayWindow(registration, ui, monitor, finish)` is placed in the two steps of 03 7.7 and posts its bounds back after a DPI change through `IUiDispatcher`; a requester of its own comes back when a newer selection is another window's (D23 as built); a press of another button ends the selection at its release, so the release reaches the overlay and not the window beneath it (03 7.4.4 corrected, D25, EDGE-SHELL-52); `XamlChromeGuard` exempts the `Overlay.` prefix |
| Tests | No Electron file. New: Core `Shell/AreaSelectionMathTests`; App `AreaOverlayTests`, `AreaSelectionServiceTests`. As built in WP-B8: 75 tests, 42 Core (`AreaSelectionMathTests` 37, three `ShellStringsTests` cases, `CoreReferencesTests.TheShellFolderNamesNoWindowsNamespace` for AC-SHELL-29, and the new XAML file's `XamlChromeGuardTests` case) and 33 App (`AreaOverlayTests` 17, `AreaSelectionServiceTests` 15 and `AllWindowsRegisteredTests.OverlaysAreExcludedBeforeTheyAreShown`). The overlays stand in for monitors, a band above the primary monitor's taskbar for the real input and places off every screen for the rest, because the Platform tests' windows fill the runners' screens at the same time (03 8.3 as built) |
...
| Tests | No Electron file. New: Core `Home/CaptureReadinessTests`, `HomeTextTests` (complete); App `Home/CaptureModePickerTests`, `Home/HomeViewModelTests.CaptureCreatesAndStartsWithCreatedThisSession`, `EmptyProjectOpensWithoutCapture`, `Shell/ShellViewModelTests` (recording rows), `Report/ProjectDetailStateTests.AdoptIntoOpenSessionIsDurable` and `.ResumeUsesCaptureTargetSelection` |
...
| Risks and de-risking | A popup HWND over the excluded main window (INV-HOME-43): the target dropdown is an element of 03's overlay layer, not a `Popup` (`CaptureModePickerTests.DropdownIsOverlayElement`); a mode or pick lost on navigation (EDGE-HOME-57): the picker is one singleton (`SurvivesNavigation`) |
...
| Deliverables | Core `ShotAI.Core.Report.Operations`: `ReportOperation` (with `AffectedStepIds`) and the eleven operations of 05 7.4 (zoom, pan, move, caption, instructions, delete, text step save, callout change, overview, add text step, document scale); Core `ShotAI.Core.Report`: `ReportEditState`, `ReportEditPolicy`, `ReorderPlanner`, `StepNumberEntry`, edit strings in `ReportStrings`. Added by WP-A18: `ReportOperation` landed there with P8's `SetProjectThemeOperation` and its tests, and `ReportStrings.RolledBack` with the first rollback notice; `OptimisticPolicyTests`' P8 row joins here |
...
| Size | M |
| Risks and de-risking | `RenderTargetBitmap` size limits, text mode and software-rendering speed on the ARM64 VM are unverified (Q-EDIT-16): `WpfOverlayRasterizerTests` pins them and AC-EDIT-28 is measured in M-C. Added by WP-A16: WPF's resource package is not safe to read from two UI threads at once (the STA harness serializes its tests for it); `StaRenderThread` is the app's second STA thread, so check that it cannot read a pack part (a font) while the UI thread reads the same one |
...
| Deliverables | Core `MergePlanner`, `MergePlan`; App `MergeCoordinator` (flatten through `IStepFlattener.FlattenStepAsync` off the UI thread, then `ApplyDurable(s => s.MergeStepsAsync(...))`; only the two steps are locked), the merge banner. Added by WP-A17: `StepContext`'s `canMergeInto`, the card's merge members (05 7.8) and the project view's view-level token for the merge flatten (05 7.3); `IImageSizeProbe` landed in WP-A17 |
...
| Tests | No further Electron file. New: Core `Export/PrintHtmlTests` (including `FaceOnlyInThePrintCopy` from `export-theme.test.ts`), `Threading/StaleProgressTests`; Platform `Export/PdfRendererTests` (including `ReputationCheckingIsOff` and `UserDataFolderIsUnderLocalData`); App `Architecture/SingleWebViewTests` (landed in WP-A15 with the package and the version probe; this WP adds the PDF host's cases) |
...
| Deliverables | Core `ShotAI.Core.Export.Package`: `PackageWriter` (whitelisted, reset fields, no zip64), `PackageReader` (declared sizes before inflating, bounded reads, JSZip name cleanup, duplicate and backslash rules, marker and manifest validation), `PackageLimits`, `JsZipNames`, `PackageImport`; `ExportService.ExportPackageAsync`, `ImportPackageAsync`; App `PackageExportDialog` (redacted-only on every open), File, Import Project (`Ctrl+O`) and the Home import flow; added by WP-A16: the Home Import button's `HomeViewModel.ImportRequested` connected to the same flow |
| Tests | No Electron file. New: Core `Export/Package/PackageWriterTests`, `PackageReaderTests`, `PackageRoundTripTests`; Platform `Export/PackageWriterTests.OriginalsSkipLinkedReferences`; App `Report/ExportFlowTests.PackageDialogOpensRedacted`, `DialogKeyboardTests.PackageDialogEscapeCloses`, `Shell/AppMenuViewModelTests.ImportAndSettingsIgnoredWhileRecording` (INV-SHELL-18: WP-A15 raises the two requests; the guard joins with their handlers here) |
...
| X10 | Mixed-DPI placement errors (03 R3) | M / M | WP-B7, WP-B8, WP-B11 | physical-px positioning after `SourceInitialized`; the 100% plus 150% rig. Outcome in WP-B8: the overlays are placed in two steps and put their bounds back after a DPI change; no runner can change a DPI, so the rig checks it (AC-SHELL-15, AC-SHELL-16) |
...
| X21 | Archivo renders at one weight (06 Q-HOME-2) | M / M | WP-A14 | static instances; `ArchivoRenderingTests`. Closed in WP-A14: every weight and the condensed labels resolve their own static file on both runners |
| X22 | A `StaticResource` stops following the brand (06) | M / M | WP-A14 | Linux source guards. In place since WP-A14: `XamlResourceKeyGuardTests.NoStaticResourceOfAThemeKey` on Linux |
...
| Q-MODEL-17 | listing cost | WP-A16 | parity; cache summaries only if PB-8 fails (parity taken in WP-A16: every refresh lists; an unchanged listing edits no row) |
...
| Q-CAP-24 | incomplete targets | WP-B9a | the picker cannot submit incomplete; D23 warning |
...
| Q-SHELL-9 | window click-pick | WP-B9a | not added |
...
| Q-SHELL-14 | initial monitor | WP-A15 | the cursor's monitor, the primary without a cursor position (decided in WP-A15) |
...
| Q | Topic | Owner | Decision or mitigation |
|---|---|---|---|
| Q-HOME-1 | where source guards run | WP-A14 | `ShotAI.Core.Tests/SourceGuards` on Linux. Built in WP-A14 as the default |
| Q-HOME-2 | Archivo in WPF | WP-A14 | upstream static instances with `SOURCES.md`. Decided in WP-A14: the unmodified statics of upstream commit `555fa4a`; the `wdth` 62 files are their own family, so the labels read `Font.label-stack` |
...
| Q-HOME-15 | STA test harness | WP-A12 | closed (ARCHITECTURE 15.4): the in-repo helper of ARCHITECTURE 2.1 and 12.1; built in WP-A12 (`tests/ShotAI.App.Tests/Support/Sta.cs`); since WP-A16 it runs one test body at a time (WPF's resource package is not safe for two UI threads at once) |
...
| Q-SOP-13 | `SOP ready` badge when off | WP-A16 | keep (taken in WP-A16: the badge reads the project's guide only) |
...
| Q-INFRA-9 | WPF and the variable Archivo | WP-A14 | static instances. Decided in WP-A14: WPF lists the variable file's 9 named weight instances and no condensed face, so the statics stay |
...
| Q-INFRA-16 | environment self-test triggers | WP-A12 | keep both, switches primary (done in WP-A12: `StartupModeParser`) |
...
- [ ] WP-A12. App host and composition root: merged in #134, manual pending: AC-SHELL-25, AC-INFRA-14
- [ ] WP-A13. Window base, popup exclusion and single instance: merged in #135, manual pending: AC-SHELL-4
- [ ] WP-A14. Theme resources and fonts: merged in #136, manual pending: AC-HOME-21, AC-INFRA-30
- [ ] WP-A15. Main window, app menu and About: merged in #137, manual pending: AC-SHELL-5, AC-SHELL-23, AC-SHELL-24
