...
/**
 * True if `title` is one this app generated rather than a name a person chose.
 *
 * Kept beside `defaultTitle` on purpose: the two must describe the same format,
 * and a detector that drifts from its generator silently stops matching. An
 * empty title counts — there is nothing to preserve.
 */
export function isAutoGeneratedTitle(title: string): boolean {
  const t = title.trim();
  if (t === '') return true;
  return /^Project \d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}$/.test(t);
}
