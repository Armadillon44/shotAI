...
/** Max image box at zoom 1. Width matches macOS ReportPresentation.baseWidth. */
export const REPORT_BASE_W = 820;
...
export function reportFit(
  ...
  const budgetW =
    Math.max(1, Math.min(REPORT_BASE_W, availW ?? REPORT_BASE_W) - WRAP_BORDER * 2);
  const budgetH = Math.max(1, REPORT_BASE_H - WRAP_BORDER * 2);

  const scale = Math.min(budgetW / dims.w, budgetH / dims.h, 1);
  ...
  const baseW = Math.floor(dims.w * scale);
  const baseH = Math.floor(dims.h * scale);
